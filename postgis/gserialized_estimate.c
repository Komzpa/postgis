...
pg_get_nd_stats(const Oid table_oid, AttrNumber att_num, int mode, bool only_parent)
{
  ...
  {
    POSTGIS_DEBUGF(
        2, "searching whole tree stats for \"%s\"", get_rel_name(table_oid) ? get_rel_name(table_oid) : "NULL");
    stats_tuple =
        SearchSysCache3(STATRELATTINH, ObjectIdGetDatum(table_oid), Int16GetDatum(att_num), BoolGetDatum(true));
    if (stats_tuple)
      POSTGIS_DEBUGF(
          2, "found whole tree stats for \"%s\"", get_rel_name(table_oid) ? get_rel_name(table_oid) : "NULL");
  }
  /* Fall-back to main table stats only, if not found for whole tree or explicitly ignored */
  if (only_parent || !stats_tuple)
  {
    POSTGIS_DEBUGF(
        2, "searching parent table stats for \"%s\"", get_rel_name(table_oid) ? get_rel_name(table_oid) : "NULL");
    stats_tuple =
        SearchSysCache3(STATRELATTINH, ObjectIdGetDatum(table_oid), Int16GetDatum(att_num), BoolGetDatum(false));
    if (stats_tuple)
      POSTGIS_DEBUGF(
          2, "found parent table stats for \"%s\"", get_rel_name(table_oid) ? get_rel_name(table_oid) : "NULL");
  }
  if (!stats_tuple)
  {
  ...
  if (!nd_stats)
  {
    POSTGIS_DEBUGF(2, "histogram for attribute %d of table \"%s\" does not exist?", att_num, get_rel_name(table_oid));
  }
...
Datum gserialized_gist_joinsel(PG_FUNCTION_ARGS)
{
  ...
  {
    POSTGIS_DEBUGF(
        3, "unable to retrieve stats for \"%s\" Oid(%d)", get_rel_name(relid1) ? get_rel_name(relid1) : "NULL", relid1);
    PG_RETURN_FLOAT8(DEFAULT_ND_JOINSEL);
  }
  else if (!stats2)
  {
    POSTGIS_DEBUGF(
        3, "unable to retrieve stats for \"%s\" Oid(%d)", get_rel_name(relid2) ? get_rel_name(relid2) : "NULL", relid2);
    PG_RETURN_FLOAT8(DEFAULT_ND_JOINSEL);
  }
                               ...
                               int mode)
{
  ...
  else
  {
    ...
    for (d = 0; d < ndims; d++)
    {
      if (sample_distribution[d] == 0) /* Uninteresting dimensions don't get any room */ { histo_size[d] = 1; }
...
Datum _postgis_gserialized_stats(PG_FUNCTION_ARGS)
{
  ...
  if (!nd_stats) elog(ERROR, "stats for \"%s.%s\" do not exist", get_rel_name(table_oid), text_to_cstring(att_text));
...
Datum _postgis_gserialized_sel(PG_FUNCTION_ARGS)
{
  ...
  /* Calculate the gbox */
  if (!gserialized_datum_get_gbox_p(geom_datum, &gbox)) elog(ERROR, "unable to calculate bounding box from geometry");
...
Datum _postgis_gserialized_joinsel(PG_FUNCTION_ARGS)
{
  ...
  if (!nd_stats1) elog(ERROR, "stats for \"%s.%s\" do not exist", get_rel_name(table_oid1), text_to_cstring(att_text1));

  if (!nd_stats2) elog(ERROR, "stats for \"%s.%s\" do not exist", get_rel_name(table_oid2), text_to_cstring(att_text2));
...
Datum geometry_estimated_extent(PG_FUNCTION_ARGS)
{
  ...
  {
    PG_RETURN_DATUM(
        DirectFunctionCall3(gserialized_estimated_extent, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1), PG_GETARG_DATUM(2)));
  }
  else if (PG_NARGS() == 2)
  {
    PG_RETURN_DATUM(DirectFunctionCall2(gserialized_estimated_extent, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1)));
  }
...
table_get_spatial_index(Oid tbl_oid, text *col, int *key_type)
{
  ...
  foreach (lc, idx_list)
  {
    ...
    if (!HeapTupleIsValid(idx_tup)) elog(ERROR, "%s: unable to lookup index %u in syscache", __func__, idx_oid);
