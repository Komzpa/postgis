...
#define HANDLE_GEOS_ERROR(label) \
    { \
        if (!strstr(lwgeom_geos_errmsg, "InterruptedException")) lwpgerror(label ": %s", lwgeom_geos_errmsg); \
        PG_RETURN_NULL(); \
    }
...
pip_short_circuit(RTREE_POLY_CACHE *poly_cache, LWPOINT *point, GSERIALIZED *gpoly)
{
    ...
    {
        result =
            point_in_multipolygon_rtree(poly_cache->ringIndices, poly_cache->polyCount, poly_cache->ringCounts, point);
    }
    else
    {
...
Datum hausdorffdistance(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) PG_RETURN_NULL();
...
Datum boundary(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom1)) PG_RETURN_POINTER(geom1);
...
Datum ST_GeneratePoints(PG_FUNCTION_ARGS)
{
    ...
    /* Return null as null */
    if (!lwgeom_result) PG_RETURN_NULL();
...
Datum ST_OffsetCurve(PG_FUNCTION_ARGS)
{
    ...
    lwgeom_input = lwgeom_from_gserialized(gser_input);
    if (!lwgeom_input) lwpgerror("ST_OffsetCurve: lwgeom_from_gserialized returned NULL");

    /* For empty inputs, just echo them back */
    if (lwgeom_is_empty(lwgeom_input)) PG_RETURN_POINTER(gser_input);
    ...
    if (!lwgeom_result) lwpgerror("ST_OffsetCurve: lwgeom_offsetcurve returned NULL");
...
/**
        @example pointonsurface - {@link #pointonsurface} SELECT pointonsurface('POLYGON((0 0, 10 0, 10 10, 0 10, 0
   0))');
*/
...
Datum pointonsurface(PG_FUNCTION_ARGS)
{
    ...
    {
        LWPOINT *lwp =
            lwpoint_construct_empty(gserialized_get_srid(geom), gserialized_has_z(geom), gserialized_has_m(geom));
...
Datum centroid(PG_FUNCTION_ARGS)
{
    ...
    if (!geosgeom) HANDLE_GEOS_ERROR("First argument geometry could not be converted to GEOS");
 ...
 */
void
errorIfGeometryCollection(GSERIALIZED *g1, GSERIALIZED *g2)
{
...
Datum isvalid(PG_FUNCTION_ARGS)
{
    ...
    /* Empty.IsValid() == TRUE */
    if (gserialized_is_empty(geom1)) PG_RETURN_BOOL(true);

    initGEOS(lwpgnotice, lwgeom_geos_error);

    lwgeom = lwgeom_from_gserialized(geom1);
    if (!lwgeom) { lwpgerror("unable to deserialize input"); }
    g1 = LWGEOM2GEOS(lwgeom, 0);
    lwgeom_free(lwgeom);
...
Datum isvaliddetail(PG_FUNCTION_ARGS)
{
    ...
    if (PG_NARGS() > 1 && !PG_ARGISNULL(1)) { flags = PG_GETARG_INT32(1); }
...
Datum overlaps(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) PG_RETURN_BOOL(false);
    ...
    {
        if (!gbox_overlaps_2d(&box1, &box2)) { PG_RETURN_BOOL(false); }
    }
...
Datum contains(PG_FUNCTION_ARGS)
{
    ...
    */
    if (is_poly(geom1) && is_point(geom2))
    {
        ...
        else if (gserialized_get_type(gpoint) == MULTIPOINTTYPE)
        {
            ...
            for (i = 0; i < mpoint->ngeoms; i++)
            {
                ...
                if (pip_result == 1) found_completely_inside = LW_TRUE;
    ...
    if (prep_cache && prep_cache->prepared_geom && prep_cache->gcache.argnum == 1)
    {
        ...
        if (!g1) HANDLE_GEOS_ERROR("Geometry could not be converted to GEOS");
...
Datum coveredby(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_get_gbox_p(geom1, &box1) && gserialized_get_gbox_p(geom2, &box2))
    {
        if (!gbox_contains_2d(&box2, &box1)) { PG_RETURN_BOOL(false); }
...
Datum crosses(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_get_gbox_p(geom1, &box1) && gserialized_get_gbox_p(geom2, &box2))
    {
        if (gbox_overlaps_2d(&box1, &box2) == LW_FALSE) { PG_RETURN_BOOL(false); }
    }
...
Datum disjoint(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) PG_RETURN_BOOL(true);
    ...
    if (gserialized_get_gbox_p(geom1, &box1) && gserialized_get_gbox_p(geom2, &box2))
    {
        if (gbox_overlaps_2d(&box1, &box2) == LW_FALSE) { PG_RETURN_BOOL(true); }
    }
...
Datum relate_pattern(PG_FUNCTION_ARGS)
{
    ...
    patt = DatumGetCString(DirectFunctionCall1(textout, PointerGetDatum(PG_GETARG_DATUM(2))));
...
Datum relate_full(PG_FUNCTION_ARGS)
{
    ...
    if (PG_NARGS() > 2) { bnr = PG_GETARG_INT32(2); }
...
Datum ST_Equals(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_get_gbox_p(geom1, &box1) && gserialized_get_gbox_p(geom2, &box2))
    {
        if (gbox_same_2d_float(&box1, &box2) == LW_FALSE) { PG_RETURN_BOOL(false); }
    }
     ...
     */
    if (VARSIZE(geom1) == VARSIZE(geom2) && !memcmp(geom1, geom2, VARSIZE(geom1))) { PG_RETURN_BOOL(true); }
...
Datum issimple(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom)) PG_RETURN_BOOL(true);
    ...
    if (result == -1) { PG_RETURN_NULL(); /*never get here */ }
...
Datum isring(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(geom)) PG_RETURN_BOOL(false);
...
GEOS2POSTGIS(GEOSGeom geom, char want3d)
{
    ...
    if (lwgeom_needs_bbox(lwgeom) == LW_TRUE) { lwgeom_add_bbox(lwgeom); }
...
Datum ST_MinimumClearance(PG_FUNCTION_ARGS)
{
    ...
    if (!input_geos) HANDLE_GEOS_ERROR("Geometry could not be converted to GEOS");
...
Datum ST_MinimumClearanceLine(PG_FUNCTION_ARGS)
{
    ...
    GEOSGeom_destroy(input_geos);
    if (!result_geos) HANDLE_GEOS_ERROR("Error computing minimum clearance");
...
Datum ST_OrientedEnvelope(PG_FUNCTION_ARGS)
{
    ...
    GEOSGeom_destroy(input_geos);
    if (!result_geos) HANDLE_GEOS_ERROR("Error computing oriented envelope");
