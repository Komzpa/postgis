...
Datum LWGEOM_simplify2d(PG_FUNCTION_ARGS)
{
    ...
    if (PG_NARGS() > 2 && !PG_ARGISNULL(2)) preserve_collapsed = true;

    /* Can't simplify points! */
    if (type == POINTTYPE || type == MULTIPOINTTYPE) PG_RETURN_POINTER(geom);
...
Datum LWGEOM_SetEffectiveArea(PG_FUNCTION_ARGS)
{
    ...
    if ((PG_NARGS() > 1) && (!PG_ARGISNULL(1))) area = PG_GETARG_FLOAT8(1);

    if ((PG_NARGS() > 2) && (!PG_ARGISNULL(2))) set_area = PG_GETARG_INT32(2);
...
Datum LWGEOM_snaptogrid(PG_FUNCTION_ARGS)
{
    ...
    if (gserialized_is_empty(in_geom)) { PG_RETURN_POINTER(in_geom); }

    /* Return input geometry if input grid is meaningless */
    if (grid.xsize == 0 && grid.ysize == 0 && grid.zsize == 0 && grid.msize == 0) { PG_RETURN_POINTER(in_geom); }
...
Datum LWGEOM_snaptogrid_pointoff(PG_FUNCTION_ARGS)
{
    ...
    if (in_lwpoint == NULL) { lwpgerror("Offset geometry must be a point"); }
    ...
    if (FLAGS_GET_Z(in_lwpoint->flags))
        grid.ipz = offsetpoint.z;
    else
        grid.ipz = 0;
    if (FLAGS_GET_M(in_lwpoint->flags))
        grid.ipm = offsetpoint.m;
    else
        grid.ipm = 0;
...
Datum LWGEOM_line_substring(PG_FUNCTION_ARGS)
{
    ...
    else if (type == MULTILINETYPE)
    {
        ...
        for (i = 0; i < iline->ngeoms; i++)
        {
            ...
            if (subline->points && subline->points->npoints > 1) length += ptarray_length_2d(subline->points);
        }
        ...
        for (i = 0; i < iline->ngeoms; i++)
        {
            ...
            if (subline->points && subline->points->npoints > 1) sublength += ptarray_length_2d(subline->points);
            ...
            if (from > maxprop || to < minprop) continue;

            if (from <= minprop) subfrom = 0.0;
            if (to >= maxprop) subto = 1.0;

            if (from > minprop && from <= maxprop) subfrom = (from - minprop) / (maxprop - minprop);
...
point_in_ring_rtree(RTREE_NODE *root, const POINT2D *point)
{
    ...
    for (i = 0; i < lines->ngeoms; i++)
    {
        ...
        POSTGIS_DEBUGF(3,
                       "counterclockwise wrap %d, clockwise wrap %d",
                       FP_CONTAINS_BOTTOM(seg1->y, point->y, seg2->y),
                       FP_CONTAINS_BOTTOM(seg2->y, point->y, seg1->y));
    ...
    for (p = 0; p < polyCount; p++)
    {
        ...
        else
        {
            ...
            for (r = 1; r < ringCounts[p]; r++)
            {
                ...
                POSTGIS_DEBUGF(
                    4, "point_in_multipolygon_rtree: interior ring (%d), point_in_ring returned %d", r, in_ring);
...
Datum ST_MinimumBoundingCircle(PG_FUNCTION_ARGS)
{
    ...
    else
    {
        ...
        if (mbc->radius == 0)
            ...
            lwcircle = lwpoly_as_lwgeom(lwpoly_construct_circle(
                input->srid, mbc->center->x, mbc->center->y, mbc->radius, segs_per_quarter, LW_TRUE));
