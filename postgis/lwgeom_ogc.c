...
Datum LWGEOM_getTYPE(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_has_m(gser) && !gserialized_has_z(gser)) strcat(result, "M");
...
Datum LWGEOM_numpoints_linestring(PG_FUNCTION_ARGS)
{
  ...
  if (type == LINETYPE || type == CIRCSTRINGTYPE || type == COMPOUNDTYPE) count = lwgeom_count_vertices(lwgeom);
...
Datum LWGEOM_geometryn_collection(PG_FUNCTION_ARGS)
{
  ...
  if (type == POINTTYPE || type == LINETYPE || type == CIRCSTRINGTYPE || type == COMPOUNDTYPE || type == POLYGONTYPE ||
...
Datum LWGEOM_interiorringn_polygon(PG_FUNCTION_ARGS)
{
  ...
  if (wanted_index < 1) { PG_RETURN_NULL(); /* index out of range */ }
...
Datum LWGEOM_x_point(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POINTTYPE) lwpgerror("Argument to ST_X() must be a point");
...
Datum LWGEOM_y_point(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POINTTYPE) lwpgerror("Argument to ST_Y() must be a point");
...
Datum LWGEOM_z_point(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POINTTYPE) lwpgerror("Argument to ST_Z() must be a point");
...
Datum LWGEOM_m_point(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POINTTYPE) lwpgerror("Argument to ST_M() must be a point");
...
Datum LWGEOM_from_text(PG_FUNCTION_ARGS)
{
  ...
  if (lwgeom_parse_wkt(&lwg_parser_result, wkt, LW_PARSER_CHECK_ALL) == LW_FAILURE) PG_PARSER_ERROR(lwg_parser_result);

  lwgeom = lwg_parser_result.geom;

  if (lwgeom->srid != SRID_UNKNOWN) { elog(WARNING, "OGC WKT expected, EWKT provided - use GeomFromEWKT() for this"); }
