...
Datum geometry_to_point(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POINTTYPE) elog(ERROR, "geometry_to_point only accepts Points");
...
Datum geometry_to_path(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != LINETYPE) elog(ERROR, "geometry_to_path only accepts LineStrings");
...
Datum geometry_to_polygon(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_get_type(geom) != POLYGONTYPE) elog(ERROR, "geometry_to_polygon only accepts Polygons");
...
Datum polygon_to_geometry(PG_FUNCTION_ARGS)
{
  ...
  if (!polygon) PG_RETURN_NULL();

  /* Are first and last points different? If so we need to close this ring */
  if (memcmp(polygon->p, polygon->p + polygon->npts - 1, sizeof(Point))) { unclosed = 1; }
