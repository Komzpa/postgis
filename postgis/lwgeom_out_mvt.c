...
Datum ST_AsMVTGeom(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(1)) elog(ERROR, "%s: parameter bounds cannot be null", __func__);
    ...
    if (lwgeom_out == NULL) PG_RETURN_NULL();
...
Datum pgis_asmvt_transfn(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0))
    {
        ...
        ctx->extent = 4096;
        if (PG_NARGS() > 3 && !PG_ARGISNULL(3)) ctx->extent = PG_GETARG_INT32(3);
        ctx->geom_name = NULL;
