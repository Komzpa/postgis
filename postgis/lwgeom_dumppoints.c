    ...
    if (!lwgeom_is_collection(lwgeom))
    {
      ...
      case TRIANGLETYPE:
        ...
        if (state->pt <= 3)
        {
          ...
          lwpoint = lwpoint_make(tri->srid, FLAGS_GET_Z(tri->points->flags), FLAGS_GET_M(tri->points->flags), &pt);
        }
      ...
      else
      {
        ...
        pathpt[0] = PointerGetDatum(
            construct_array(state->path, state->pathlen + 1, INT4OID, state->typlen, state->byval, state->align));
