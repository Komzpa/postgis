...
double
distance_ellipse(double lat1, double long1, double lat2, double long2, SPHEROID *sphere)
{
    double result = 0;
    ...
    if ((lat1 == lat2) && (long1 == long2)) { return 0.0; /* same point, therefore zero distance */ }
    ...
    POSTGIS_DEBUGF(4, "delta = %lf, skae says: %.15lf,2 circle says: %.15lf", (result2 - result), result, result2);
 ...
 */
double
distance_ellipse_calculation(double lat1, double long1, double lat2, double long2, SPHEROID *sphere)
{
    /*
     * Code is taken from David Skea
     * Geographic Data BC, Province of British Columbia, Canada.
...
double
distance_sphere_method(double lat1, double long1, double lat2, double long2, SPHEROID *sphere)
{
...
Datum LWGEOM_distance_ellipsoid(PG_FUNCTION_ARGS)
{
    PG_RETURN_DATUM(DirectFunctionCall4(
        geometry_distance_spheroid, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1), PG_GETARG_DATUM(2), BoolGetDatum(true)));
}
...
Datum LWGEOM_distance_sphere(PG_FUNCTION_ARGS)
{
    ...
    PG_RETURN_DATUM(DirectFunctionCall4(
        geometry_distance_spheroid, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1), PointerGetDatum(&s), BoolGetDatum(false)));
}
