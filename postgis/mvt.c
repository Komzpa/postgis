...
parse_jsonb(mvt_agg_context *ctx, Jsonb *jb, uint32_t *tags)
{
    ...
    while ((r = JsonbIteratorNext(&it, &v, skipNested)) != WJB_DONE)
    {
        ...
        if (r == WJB_KEY && v.type != jbvNull)
        {
            ...
            {
                MVT_PARSE_VALUE(
                    v.val.boolean, mvt_kv_bool_value, bool_values_hash, bool_value, sizeof(protobuf_c_boolean));
                ctx->c++;
            }
            else if (v.type == jbvNumeric)
            {
                ...
                if ((long)d != l)
                { MVT_PARSE_VALUE(d, mvt_kv_double_value, double_values_hash, double_value, sizeof(double)); }
                else
                {
...
parse_values(mvt_agg_context *ctx)
{
    ...
    for (i = 0; i < natts; i++)
    {
        ...
        case FLOAT8OID:
            MVT_PARSE_DATUM(
                double, mvt_kv_double_value, double_values_hash, double_value, DatumGetFloat8, sizeof(double));
            break;
        default:
