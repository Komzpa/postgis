...
SELECT '#2038.precision_exact.start', CreateTopology('t2038_precision_exact', 4326, 1) > 0;

SELECT '#2038.precision_exact.edge.1', count(*) FROM TopoGeo_AddLineString(
  't2038_precision_exact',
  'SRID=4326;LINESTRING(0 0,10 0)'::geometry, -1.0::float8, -1);

SELECT '#2038.precision_exact.edge.2', count(*) FROM TopoGeo_AddLineString(
  't2038_precision_exact',
  'SRID=4326;LINESTRING(5 0,5 5)'::geometry, -1.0::float8, -1);

SELECT '#2038.precision_exact.nodes', count(*), string_agg(ST_AsText(geom), ',' ORDER BY node_id)
  FROM t2038_precision_exact.node;

SELECT '#2038.precision_exact.edges', count(*) FROM t2038_precision_exact.edge_data;

SELECT '#2038.precision_exact.invalidity', count(*) FROM ValidateTopology('t2038_precision_exact');

SELECT DropTopology('t2038_precision_exact');
