...
wkt_dimensionality(char *dimensionality)
{
    ...
    if (!dimensionality) return flags;
...
wkt_pointarray_dimensionality(POINTARRAY *pa, uint8_t flags)
{
    ...
    if (ndims > 2)
    {
        ...
        if (FLAGS_NDIMS(pa->flags) != ndims) return LW_FALSE;
 ...
 */
POINT
wkt_parser_coord_2(double c1, double c2)
{
    POINT p;
    p.flags = 0;
    p.x = c1;
 ...
 */
POINT
wkt_parser_coord_3(double c1, double c2, double c3)
{
    POINT p;
    p.flags = 0;
    p.x = c1;
 ...
 */
POINT
wkt_parser_coord_4(double c1, double c2, double c3, double c4)
{
    POINT p;
    p.flags = 0;
    p.x = c1;
...
wkt_parser_ptarray_add_coord(POINTARRAY *pa, POINT p)
{
    ...
    pt.x = p.x;
    pt.y = p.y;
    if (FLAGS_GET_Z(pa->flags)) pt.z = p.z;
    if (FLAGS_GET_M(pa->flags)) pt.m = p.m;
    /* If the destination is XYM, we'll write the third coordinate to m */
    if (FLAGS_GET_M(pa->flags) && !FLAGS_GET_Z(pa->flags)) pt.m = p.z;
...
wkt_parser_point_new(POINTARRAY *pa, char *dimensionality)
{
    ...
    if (!pa) return lwpoint_as_lwgeom(lwpoint_construct_empty(SRID_UNKNOWN, FLAGS_GET_Z(flags), FLAGS_GET_M(flags)));
...
wkt_parser_linestring_new(POINTARRAY *pa, char *dimensionality)
{
    ...
    /* No pointarray means it is empty */
    if (!pa) return lwline_as_lwgeom(lwline_construct_empty(SRID_UNKNOWN, FLAGS_GET_Z(flags), FLAGS_GET_M(flags)));
...
wkt_parser_polygon_finalize(LWGEOM *poly, char *dimensionality)
{
    ...
    if (!poly) return lwpoly_as_lwgeom(lwpoly_construct_empty(SRID_UNKNOWN, FLAGS_GET_Z(flags), FLAGS_GET_M(flags)));
...
wkt_parser_curvepolygon_add_ring(LWGEOM *poly, LWGEOM *ring)
{
    ...
    if ((global_parser_result.parser_check_flags & LW_PARSER_CHECK_MINPOINTS))
    {
        ...
        if (ring->type == LINETYPE) vertices_needed = 4;
