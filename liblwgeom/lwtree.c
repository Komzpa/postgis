...
{
  if (pt->y < node->ymin || pt->y > node->ymax || pt->x < node->xmin || pt->x > node->xmax)
    return LW_FALSE;
  else
    return LW_TRUE;
}
...
rect_node_leaf_new(const POINTARRAY *pa, int seg_num, int geom_type)
{
  ...
  case RECT_NODE_SEG_CIRCULAR:
  {
    ...
    /* Zero length edge, doesn't get a node */
    if ((p1->x == p2->x) && (p2->x == p3->x) && (p1->y == p2->y) && (p2->y == p3->y)) return NULL;
...
{
  if (rect_node_is_leaf(node)) lwerror("%s: call on leaf node", __func__);
...
rect_tree_from_lwcollection(const LWGEOM *lwgeom)
{
  ...
  for (i = 0; i < lwcol->ngeoms; i++)
  {
    ...
    if (node)
    {
      ...
      /* correctly */
      if (lwgeom->type == CURVEPOLYTYPE) node->i.ring_type = i ? RECT_NODE_RING_INTERIOR : RECT_NODE_RING_EXTERIOR;
      nodes[j++] = node;
    }
  }
  ...
  /* sorting needed */
  if (lwgeom->type != COMPOUNDTYPE) qsort(nodes, j, sizeof(RECT_NODE *), rect_node_cmp);
...
rect_node_to_str(const RECT_NODE *n)
{
  ...
  snprintf(buf, 256, "(%.9g %.9g,%.9g %.9g)", n->xmin, n->ymin, n->xmax, n->ymax);
  return buf;
}
#endif
  ...
  if (rect_node_intersects(n1, n2))
  {
    ...
    else
    {
      for (j = 0; j < n1->i.num_nodes; j++)
      {
        for (i = 0; i < n2->i.num_nodes; i++)
        {
          if (rect_tree_intersects_tree_recursive(n2->i.nodes[i], n1->i.nodes[j])) return LW_TRUE;
        }
      }
    }
  }
...
rect_tree_intersects_tree(RECT_NODE *n1, RECT_NODE *n2)
{
   ...
   */
  if (rect_tree_is_area(n1) && rect_tree_contains_point(n1, rect_tree_get_point(n2))) { return LW_TRUE; }

  if (rect_tree_is_area(n2) && rect_tree_contains_point(n2, rect_tree_get_point(n1))) { return LW_TRUE; }
...
rect_tree_node_sort(RECT_NODE *n1, RECT_NODE *n2)
{
  ...
  if (!rect_node_is_leaf(n2) && !n2->i.sorted)
  {
    ...
    /* Sort the children by distance */
    n2->i.sorted = 1;
    qsort(n2->i.nodes, n2->i.num_nodes, sizeof(RECT_NODE *), rect_tree_node_sort_cmp);
  }
}
...
rect_tree_distance_tree_recursive(RECT_NODE *n1, RECT_NODE *n2, RECT_TREE_DISTANCE_STATE *state)
{
  ...
  if (state->min_dist < state->threshold || state->min_dist == 0.0) return state->min_dist;
...
double
rect_tree_distance_tree(RECT_NODE *n1, RECT_NODE *n2, double threshold)
{
  double distance;
   ...
   * so we do a quick point-in-poly test first for those cases
   */
  if (rect_tree_is_area(n1) && rect_tree_contains_point(n1, rect_tree_get_point(n2))) { return 0.0; }

  if (rect_tree_is_area(n2) && rect_tree_contains_point(n2, rect_tree_get_point(n1))) { return 0.0; }
