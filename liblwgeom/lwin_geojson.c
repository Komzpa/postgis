...
findMemberByName(json_object *poObj, const char *pszName)
{
  ...
  if (NULL == pszName || NULL == poObj) return NULL;
  ...
  if (NULL != json_object_get_object(poTmp))
  {
    ...
    {
      if (strcasecmp((char *)it.key, pszName) == 0) return it.val;
    }
  }
...
parse_geojson_polygon(json_object *geojson, int *hasz, int root_srid)
{
  ...
  if (!nRings) { return (LWGEOM *)lwpoly_construct_empty(root_srid, 0, 0); }
  ...
  for (i = 0; i < nRings; i++)
  {
    ...
    if (!ppa) ppa = (POINTARRAY **)lwalloc(sizeof(POINTARRAY *) * nRings);
  ...
  if (!ppa) return (LWGEOM *)lwpoly_construct_empty(root_srid, 0, 0);
...
parse_geojson_multipoint(json_object *geojson, int *hasz, int root_srid)
{
  ...
  if (json_type_array == json_object_get_type(poObjPoints))
  {
    ...
    for (i = 0; i < nPoints; ++i)
    {
      ...
      geom = (LWGEOM *)lwmpoint_add_lwpoint((LWMPOINT *)geom, (LWPOINT *)lwpoint_construct(root_srid, NULL, pa));
    }
  }
  ...
  if (json_type_array == json_object_get_type(poObjLines))
  {
    ...
    for (i = 0; i < nLines; ++i)
    {
      ...
      if (json_type_array == json_object_get_type(poObjLine))
      {
        ...
        geom = (LWGEOM *)lwmline_add_lwline((LWMLINE *)geom, (LWLINE *)lwline_construct(root_srid, NULL, pa));
      }
    }
  }
...
parse_geojson_geometrycollection(json_object *geojson, int *hasz, int root_srid)
{
  ...
  if (json_type_array == json_object_get_type(poObjGeoms))
  {
    ...
    for (i = 0; i < nGeoms; ++i)
    {
      ...
      geom = (LWGEOM *)lwcollection_add_lwgeom((LWCOLLECTION *)geom, parse_geojson(poObjGeom, hasz, root_srid));
    }
  }
...
parse_geojson(json_object *geojson, int *hasz, int root_srid)
{
  ...
  if (strcasecmp(name, "Point") == 0) return parse_geojson_point(geojson, hasz, root_srid);

  if (strcasecmp(name, "LineString") == 0) return parse_geojson_linestring(geojson, hasz, root_srid);

  if (strcasecmp(name, "Polygon") == 0) return parse_geojson_polygon(geojson, hasz, root_srid);

  if (strcasecmp(name, "MultiPoint") == 0) return parse_geojson_multipoint(geojson, hasz, root_srid);

  if (strcasecmp(name, "MultiLineString") == 0) return parse_geojson_multilinestring(geojson, hasz, root_srid);

  if (strcasecmp(name, "MultiPolygon") == 0) return parse_geojson_multipolygon(geojson, hasz, root_srid);

  if (strcasecmp(name, "GeometryCollection") == 0) return parse_geojson_geometrycollection(geojson, hasz, root_srid);
