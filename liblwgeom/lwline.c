...
lwline_free(LWLINE *line)
{
    ...
    if (line->bbox) lwfree(line->bbox);
    if (line->points) ptarray_free(line->points);
    lwfree(line);
}
...
lwline_from_lwmpoint(int srid, const LWMPOINT *mpoint)
{
    ...
    if (lwgeom_is_empty(lwgeom)) { return lwline_construct_empty(srid, hasz, hasm); }
...
lwline_measured_from_lwline(const LWLINE *lwline, double m_start, double m_end)
{
    ...
    for (i = 0; i < npoints; i++)
    {
        ...
        if (length > 0.0) m = m_start + m_range * length_so_far / length;
...
lwline_is_trajectory(const LWLINE *line)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        if (p.m <= m)
        {
            lwnotice("Measure of vertex %d (%g) not bigger than measure of vertex %d (%g)", i, p.m, i - 1, m);
            return LW_FALSE;
        }
        m = p.m;
    }
