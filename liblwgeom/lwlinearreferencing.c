...
segment_locate_along(const POINT4D *p1, const POINT4D *p2, double m, double offset, POINT4D *pn)
{
  ...
  if ((m < FP_MIN(m1, m2)) || (m > FP_MAX(m1, m2))) { return LW_FALSE; }
...
ptarray_locate_along(const POINTARRAY *pa, double m, double offset)
{
  ...
  for (i = 1; i < pa->npoints; i++)
  {
    ...
    if (segment_locate_along(&p1, &p2, m, offset, &pn) == LW_FALSE) continue;

    /* No pointarray, make a fresh one */
    if (dpa == NULL) dpa = ptarray_construct_empty(ptarray_has_z(pa), ptarray_has_m(pa), 8);
...
lwline_locate_along(const LWLINE *lwline, double m, double offset)
{
  ...
  if (!opa) return lwmpoint_construct_empty(srid, hasz, hasm);
...
lwmpoint_locate_along(const LWMPOINT *lwin, double m, __attribute__((__unused__)) double offset)
{
  ...
  for (i = 0; i < lwin->ngeoms; i++)
  {
    ...
    if (FP_EQUALS(m, point_m)) { lwmpoint_add_lwpoint(lwout, lwpoint_clone(lwin->geoms[i])); }
  }
...
lwpoint_get_ordinate(const POINT4D *p, char ordinate)
{
  ...
  if (ordinate == 'X') return p->x;
  if (ordinate == 'Y') return p->y;
  if (ordinate == 'Z') return p->z;
  if (ordinate == 'M') return p->m;
                  ...
                  double interpolation_value)
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    LWDEBUGF(4,
             "   clip ordinate(%c) p1_value(%g) p2_value(%g) proportion(%g) newordinate(%g) ",
             dims[i],
             p1_value,
             p2_value,
             proportion,
...
lwpoint_clip_to_ordinate_range(const LWPOINT *point, char ordinate, double from, double to)
{
  ...
  if (!point) lwerror("Null input geometry.");
...
lwmpoint_clip_to_ordinate_range(const LWMPOINT *mpoint, char ordinate, double from, double to)
{
  ...
  /* Nothing to do with NULL */
  if (!mpoint) lwerror("Null input geometry.");
  ...
  else
  {
    ...
    for (i = 0; i < mline->ngeoms; i++)
    {
      ...
      if (col)
      {
        ...
        {
          geoms_size += 16;
          if (lwgeom_out->geoms) { lwgeom_out->geoms = lwrealloc(lwgeom_out->geoms, geoms_size * sizeof(LWGEOM *)); }
          else
          {
        ...
        if (col->type != mline->type) { homogeneous = 0; }
  ...
  for (i = 0; i < pa_in->npoints; i++)
  {
    ...
    if (ordinate_value_p >= from && ordinate_value_p <= to)
    {
      ...
      if (!added_last_point)
      {
                      ...
                      (ordinate_value_p == to && ordinate_value_q < from)))  /* Hopping from below */
        {
          ...
          LWDEBUGF(4,
                   "[0] interpolating between (%g, %g) with interpolation point (%g)",
                   ordinate_value_q,
                   ordinate_value_p,
                   interpolation_value);
        }
      ...
      if (ordinate_value_p == from || ordinate_value_p == to) { added_last_point = 2; /* Added on boundary. */ }
      else
      {
    ...
    else
    {
      ...
      if (added_last_point == 1)
      {
        ...
        ptarray_append_point(dp, r, LW_FALSE);
        LWDEBUGF(4,
                 " [1] interpolating between (%g, %g) with interpolation point (%g)",
                 ordinate_value_q,
                 ordinate_value_p,
                 interpolation_value);
      }
      else if (added_last_point == 2)
      {
                           ...
                           (ordinate_value_q == to && ordinate_value_p < to)))
        {
          ...
          ptarray_append_point(dp, r, LW_FALSE);
          LWDEBUGF(4,
                   " [2] interpolating between (%g, %g) with interpolation point (%g)",
                   ordinate_value_q,
                   ordinate_value_p,
                   interpolation_value);
        }
...
lwgeom_clip_to_ordinate_range(const LWGEOM *lwin, char ordinate, double from, double to, double offset)
{
  ...
  if (out_col == NULL) lwerror("lwgeom_clip_to_ordinate_range clipping routine returned NULL");

  /* Return if we aren't going to offset the result */
  if (FP_IS_ZERO(offset) || lwgeom_is_empty(lwcollection_as_lwgeom(out_col))) return out_col;
...
lwgeom_interpolate_point(const LWGEOM *lwin, const LWPOINT *lwpt)
{
  ...
  if (!lwin) lwerror("lwgeom_interpolate_point: null input geometry!");

  if (!lwgeom_has_m(lwin)) lwerror("Input geometry does not have a measure dimension");

  if (lwgeom_is_empty(lwin) || lwpoint_is_empty(lwpt)) lwerror("Input geometry is empty");
...
ptarray_collect_mvals(const POINTARRAY *pa, double tmin, double tmax, double *mvals)
{
  ...
  for (i = 0; i < pa->npoints; ++i)
  {
    ...
    if (pbuf.m >= tmin && pbuf.m <= tmax) mvals[n++] = pbuf.m;
  }
  return n;
}
...
ptarray_locate_along_linear(const POINTARRAY *pa, double m, POINT4D *p, uint32_t from)
{
  ...
  for (i = from + 1; i < pa->npoints; i++)
  {
    ...
    if (segment_locate_along(&p1, &p2, m, 0, p) == LW_TRUE) return i - 1; /* found */
...
lwgeom_tcpa(const LWGEOM *g1, const LWGEOM *g2, double *mindist)
{
  ...
  if (mindist) { *mindist = sqrt(mindist2); }
