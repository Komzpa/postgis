...
lwgeom_make_geos_friendly(LWGEOM *geom)
{
    ...
    default:
        lwerror(
            "lwgeom_make_geos_friendly: unsupported input geometry type: %s (%d)", lwtype_name(geom->type), geom->type);
        break;
    }
    return 0;
}
...
LWGEOM_GEOS_makeValidPolygon(const GEOSGeometry *gin)
{
    ...
    while (GEOSGetNumGeometries(geos_cut_edges))
    {
        ...
        if (!new_area_bound)
        {
            ...
            lwnotice(
                "GEOSBoundary('%s') threw an error: %s", lwgeom_to_ewkt(GEOS2LWGEOM(new_area, 0)), lwgeom_geos_errmsg);
            GEOSGeom_destroy(new_area);
            GEOSGeom_destroy(geos_area);
            return NULL;
        }
