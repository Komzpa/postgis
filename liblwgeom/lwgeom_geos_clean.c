...
#if 1
    /* Duplicate point */
    line->points = ptarray_addPoint(
        line->points, getPoint_internal(line->points, 0), FLAGS_NDIMS(line->points->flags), line->points->npoints);
    ret = (LWGEOM *)line;
#else
    /* Turn into a point */
...
LWGEOM_GEOS_makeValidPolygon(const GEOSGeometry *gin)
{
  ...
  while (GEOSGetNumGeometries(geos_cut_edges))
  {
    ...
    if (!new_area_bound)
    {
      ...
      lwnotice("GEOSBoundary('%s') threw an error: %s", lwgeom_to_ewkt(GEOS2LWGEOM(new_area, 0)), lwgeom_geos_errmsg);
      GEOSGeom_destroy(new_area);
      GEOSGeom_destroy(geos_area);
      return NULL;
    }
...
lwgeom_make_valid(LWGEOM *lwgeom_in)
{
  ...
  if (lwgeom_is_collection(lwgeom_in) && !lwgeom_is_collection(lwgeom_out))
  {
    ...
    ogeoms[0] = lwgeom_out;
    ogeom =
        (LWGEOM *)lwcollection_construct(MULTITYPE[lwgeom_out->type], lwgeom_out->srid, lwgeom_out->bbox, 1, ogeoms);
