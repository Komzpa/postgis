...
twkb_parse_state_varint_skip(twkb_parse_state *s)
{
    ...
    if (!size) lwerror("%s: no varint to skip", __func__);
...
lwpoint_from_twkb_state(twkb_parse_state *s)
{
    ...
    if (s->is_empty) return lwpoint_construct_empty(SRID_UNKNOWN, s->has_z, s->has_m);
...
lwline_from_twkb_state(twkb_parse_state *s)
{
    ...
    if (s->is_empty) return lwline_construct_empty(SRID_UNKNOWN, s->has_z, s->has_m);

    /* Read number of points */
    npoints = twkb_parse_state_uvarint(s);

    if (npoints == 0) return lwline_construct_empty(SRID_UNKNOWN, s->has_z, s->has_m);
...
lwpoly_from_twkb_state(twkb_parse_state *s)
{
    ...
    if (s->is_empty) return lwpoly_construct_empty(SRID_UNKNOWN, s->has_z, s->has_m);
...
header_from_twkb_state(twkb_parse_state *s)
{
    ...
    if (s->has_size) { s->size = twkb_parse_state_uvarint(s); }
...
lwgeom_from_twkb_state(twkb_parse_state *s)
{
    ...
    if (has_bbox) { geom->bbox = gbox_clone(&bbox); }
