...
asx3d3_mline_coordindex_sb(const LWMLINE *mgeom, stringbuffer_t *sb)
{
  ...
  for (i = 0; i < mgeom->ngeoms; i++)
  {
    ...
    if (i < (mgeom->ngeoms - 1)) { stringbuffer_aprintf(sb, " -1 "); /* separator for each linestring */ }
  }
  return LW_SUCCESS;
}
...
asx3d3_mpoly_coordindex_sb(const LWMPOLY *psur, stringbuffer_t *sb)
{
  ...
  for (i = 0; i < psur->ngeoms; i++)
  {
    ...
    if (i < (psur->ngeoms - 1)) { stringbuffer_aprintf(sb, " -1 "); /* separator for each subgeom */ }
  }
  return LW_SUCCESS;
}
 ...
 */
static int
asx3d3_multi_sb(const LWCOLLECTION *col,
                __attribute__((__unused__)) char *srs,
                int precision,
                int opts,
                const char *defid,
              ...
              stringbuffer_t *sb)
{
  ...
  for (i = 0; i < tin->ngeoms; i++)
  {
    ...
    if (i < (tin->ngeoms - 1)) { stringbuffer_aprintf(sb, " "); }
    k += 3;
  }
  ...
  for (i = 0; i < tin->ngeoms; i++)
  {
    asx3d3_triangle_sb(tin->geoms[i], 0, precision, opts, defid, sb);
    if (i < (tin->ngeoms - 1)) { stringbuffer_aprintf(sb, " "); }
  }
