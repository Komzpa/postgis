			 ...
			 LWLINE *newedge)
{
	...
	nodes = lwt_be_getNodeWithinBox2D(topo, edgebox, &numnodes, LWT_COL_NODE_ALL, 0);
	...
	for (i = 0; i < numnodes; ++i)
	{
		...
		if (!node->geom || !node->geom->point || !node->geom->point->npoints)
		{
			...
			lwerror("Internal error: lwt_be_getNodeWithinBox2D returned node %" LWTFMT_ELEMID
				" with no vertices",
				node_id);
			return -1;
		}
	...
	if (motion_range)
	{
		...
		edges = lwt_be_getEdgeWithinBox2D(topo, &mbox, &numedges, LWT_COL_EDGE_ALL, 0);
...
static int
_lwt_EdgeMoveWillHitStagedEdges(LWLINE *oldedge, LWLINE *newedge, LWLINE **staged_edges, int staged_edge_count)
{
	POINTARRAY *motion_range = NULL;
	LWGEOM *motion_poly;
	GEOSGeometry *motion_poly_g;
...
static int
_lwt_EdgeMoveWillCrossEdge(LWT_TOPOLOGY *topo, LWT_ELEMID edge_id, LWLINE *newedge)
{
...
static int _lwt_RingWindingWithReplacement(LWT_TOPOLOGY *topo,
...
static int
_lwt_EdgeMoveWillChangeAdjacentEdges(LWT_TOPOLOGY *topo,
				     LWT_ELEMID edge_id,
				     LWT_ELEMID start_node,
				     LWT_ELEMID end_node,
				     ...
				     LWLINE *newedge)
{
	...
	if (_lwt_InitEdgeEndByLine(&span_pre, &epan_pre, oldedge, &p1, &p2))
		return -1;
	if (_lwt_FindAdjacentEdges(topo, start_node, &span_pre, isclosed ? &epan_pre : NULL, edge_id) < 0)
		return -1;
	if (_lwt_FindAdjacentEdges(topo, end_node, &epan_pre, isclosed ? &span_pre : NULL, edge_id) < 0)
		return -1;

	if (isclosed && ptarray_isccw(oldedge->points) != ptarray_isccw(newedge->points))
		return 1;

	if (!isclosed && face_left != face_right && span_pre.nextCW == span_pre.nextCCW &&
	    epan_pre.nextCW == epan_pre.nextCCW)
	{
		...
		if (_lwt_RingWindingWithReplacement(
			topo, edge_id, oldedge, signed_edge_ids, num_signed_edge_ids, &old_isccw) < 0)
		{
			lwfree(signed_edge_ids);
			return -1;
		}
		if (_lwt_RingWindingWithReplacement(
			topo, edge_id, newedge, signed_edge_ids, num_signed_edge_ids, &new_isccw) < 0)
		{
			lwfree(signed_edge_ids);
			return -1;
		}
	...
	if (_lwt_InitEdgeEndByLine(&span_post, &epan_post, newedge, &p1, &p2))
		return -1;
	if (_lwt_FindAdjacentEdges(topo, start_node, &span_post, isclosed ? &epan_post : NULL, edge_id) < 0)
		return -1;
	if (_lwt_FindAdjacentEdges(topo, end_node, &epan_post, isclosed ? &span_post : NULL, edge_id) < 0)
		return -1;

	if (span_pre.nextCW != span_post.nextCW || span_pre.nextCCW != span_post.nextCCW ||
	    epan_pre.nextCW != epan_post.nextCW || epan_pre.nextCCW != epan_post.nextCCW)
		return 1;
				...
				int *isccw)
{
	...
	{
		_lwt_release_edges(ring_edges, i);
		lwerror("Unexpected error: %" LWTFMT_ELEMID " edges found when expecting %" PRIu64, i, numedges);
		return -1;
	}
	...
	for (i = 0; i < num_signed_edge_ids; ++i)
	{
		...
		if (full_ring_pa == NULL)
		{
			...
			if (eid < 0)
				ptarray_reverse_in_place(full_ring_pa);
		}
		else if (eid < 0)
		{
	...
	{
		ptarray_free(full_ring_pa);
		lwerror("Corrupted topology: ring of edge %" LWTFMT_ELEMID " is geometrically not-closed",
			signed_edge_ids[0]);
		return -1;
	}
	     ...
	     int snapExistingEdges)
{
  ...
  *nedges = -1; /* error condition, by default */
  ...
  {{
    ...
    if (snapExistingEdges)
    {
	    ...
	    for (int i = 0; i < nearbyedgecount; i++)
	    {
		    ...
		    move_collision = _lwt_EdgeMoveWillHitNode(
			topo, edge_id, nearby_start_node[i], nearby_end_node[i], edgeline, snappedline);
		    if (move_collision)
		    {
			    lwgeom_free(snapped);
			    if (move_collision < 0)
		    ...
		    if (!edge_collision)
		    {
			    edge_collision = _lwt_EdgeMoveWillHitStagedEdges(edgeline, snappedline, snapped_edges, i);
		    }
		    if (edge_collision)
		    {
			    lwgeom_free(snapped);
  ...
  if (nearbyid)
	  lwfree(nearbyid);
  if (nearby_start_node)
	  lwfree(nearby_start_node);
  if (nearby_end_node)
	  lwfree(nearby_end_node);
  if (nearby_face_left)
	  lwfree(nearby_face_left);
  if (nearby_face_right)
	  lwfree(nearby_face_right);
