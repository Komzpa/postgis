...
lw_arc_length(const POINT2D *A1, const POINT2D *A2, const POINT2D *A3)
{
  ...
  if (lw_arc_is_pt(A1, A2, A3)) return 0.0;
  ...
  if (p2d_same(A1, A3)) return circumference_A;
...
lw_arc_center(const POINT2D *p1, const POINT2D *p2, const POINT2D *p3, POINT2D *result)
{
  ...
  LWDEBUGF(
      2, "lw_arc_center called (%.16f,%.16f), (%.16f,%.16f), (%.16f,%.16f).", p1->x, p1->y, p2->x, p2->y, p3->x, p3->y);
  ...
  if (fabs(d) < EPSILON_SQLMM) return -1.0;
...
lw_seg_interact(const POINT2D *p1, const POINT2D *p2, const POINT2D *q1, const POINT2D *q2)
{
  ...
  if (FP_GT(minp, maxq) || FP_LT(maxp, minq)) return LW_FALSE;
...
/**
** @brief returns the kind of #CG_SEGMENT_INTERSECTION_TYPE  behavior of lineseg 1 (constructed from p1 and p2) and
*lineseg 2 (constructed from q1 and q2) *	@param p1 start point of first straight linesegment *	@param p2 end
*point of first straight linesegment *	@param q1 start point of second line segment *	@param q2 end point of second
*line segment *	@return a #CG_SEGMENT_INTERSECTION_TYPE
** 	Returns one of
**		SEG_ERROR = -1,
...
lw_segment_intersects(const POINT2D *p1, const POINT2D *p2, const POINT2D *q1, const POINT2D *q2)
{
  ...
  if (!lw_seg_interact(p1, p2, q1, p2)) { return SEG_NO_INTERSECTION; }
  ...
  if ((qp1 > 0.0 && qp2 > 0.0) || (qp1 < 0.0 && qp2 < 0.0)) { return SEG_NO_INTERSECTION; }

  /* Nobody is on one side or another? Must be colinear. */
  if (pq1 == 0.0 && pq2 == 0.0 && qp1 == 0.0 && qp2 == 0.0) { return SEG_COLINEAR; }
...
lwline_crossing_direction(const LWLINE *l1, const LWLINE *l2)
{
  ...
  if (!cross_left && !cross_right) return LINE_NO_CROSS;

  if (!cross_left && cross_right == 1) return LINE_CROSS_RIGHT;

  if (!cross_right && cross_left == 1) return LINE_CROSS_LEFT;

  if (cross_left - cross_right == 1) return LINE_MULTICROSS_END_LEFT;

  if (cross_left - cross_right == -1) return LINE_MULTICROSS_END_RIGHT;

  if (cross_left - cross_right == 0 && first_cross == SEG_CROSS_LEFT) return LINE_MULTICROSS_END_SAME_FIRST_LEFT;

  if (cross_left - cross_right == 0 && first_cross == SEG_CROSS_RIGHT) return LINE_MULTICROSS_END_SAME_FIRST_RIGHT;
...
lwgeom_geohash(const LWGEOM *lwgeom, int precision)
{
  ...
  {
    lwerror(
        "Geohash requires inputs in decimal degrees, got (%g %g, %g %g).", gbox.xmin, gbox.ymin, gbox.xmax, gbox.ymax);
    return NULL;
  }
  ...
  ** approximate location. */
  ...
  if (precision <= 0) { precision = lwgeom_geohash_precision(gbox, &gbox_bounds); }
