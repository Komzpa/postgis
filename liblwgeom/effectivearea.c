...
tune_areas(EFFECTIVE_AREAS *ea, int avoid_collaps, int set_area, double trshld)
{
  ...
  while (go_on)
  {
    ...
    if (ea->res_arealist[current] < check_order_min_area)
      lwerror(
          "Oh no, this is a bug. For some reason the minHeap returned our points in the wrong order. "
          "Please file a ticket in PostGIS ticket system, or send a mial at the mailing "
          "list.Returned area = %lf, and last area = %lf",
          ea->res_arealist[current],
          check_order_min_area);

    check_order_min_area = ea->res_arealist[current];

    /*The found smallest area point is now regarded as elimnated and we have to recalculate the area the
     * adjacent (ignoring earlier elimnated points) points gives*/
    ...
    if ((!set_area && ea->res_arealist[current] > trshld) || (ea->initial_arealist[0].next == (npoints - 1))) go_on = 0;
...
lwline_set_effective_area(const LWLINE *iline, int set_area, double trshld)
{
  ...
  if (lwline_is_empty(iline) || iline->points->npoints < 3) return lwline_clone(iline);
...
lwpoly_set_effective_area(const LWPOLY *ipoly, int set_area, double trshld)
{
  ...
  if (lwpoly_is_empty(opoly)) return NULL;
