...
circ_node_leaf_new(const POINTARRAY *pa, int i)
{
    ...
    if (FP_EQUALS(diameter, 0.0)) return NULL;
                      ...
                      GEOGRAPHIC_POINT *center)
{
    ...
    if (isnan(dir)) return LW_FAILURE;
...
circ_node_internal_new(CIRC_NODE **c, uint32_t num_nodes)
{
    ...
    for (i = 1; i < num_nodes; i++)
    {
        ...
        LWDEBUGF(3,
                 "distance between new (%g %g) and %i (%g %g) is %g",
                 c1.lon,
                 c1.lat,
                 i,
                 c[i]->center.lon,
...
circ_tree_new(const POINTARRAY *pa)
{
    ...
    if (pa->npoints == 1) return circ_node_leaf_point_new(pa);
...
circ_nodes_merge(CIRC_NODE **nodes, int num_nodes)
{
     ...
     * TODO, change the distance algorithm to drive down to simple types first, test pip on poly/other cases, then
     * test edges
     */
    ...
    while (num_children > 1)
    {
        for (j = 0; j < num_children; j++)
        {
            ...
            if (inode_num == CIRC_NODE_SIZE - 1) nodes[num_parents++] = circ_node_internal_new(inodes, CIRC_NODE_SIZE);
        }
...
circ_tree_contains_point(const CIRC_NODE *node, const POINT2D *pt, const POINT2D *pt_outside, int *on_boundary)
{
    ...
    LWDEBUGF(3,
             "working on node %p, edge_num %d, radius %g, center POINT(%g %g)",
             node,
             node->edge_num,
             node->radius,
             rad2deg(node->center.lon),
             rad2deg(node->center.lat));
                                 ...
                                 GEOGRAPHIC_POINT *closest2)
{
    ...
    LWDEBUGF(4,
             "entered, min_dist=%.8g max_dist=%.8g, type1=%d, type2=%d",
             *min_dist,
             *max_dist,
             n1->geom_type,
             n2->geom_type);
    ...
    */
    ...
    if (*min_dist < threshold || *min_dist == 0.0) return *min_dist;
...
circ_tree_print(const CIRC_NODE *node, int depth)
{
    ...
    {
        printf("%*s C(%.5g %.5g) R(%.5g)", 3 * depth + 6, "NODE", node->center.lon, node->center.lat, node->radius);
        if (node->geom_type) { printf(" %s", lwtype_name(node->geom_type)); }
        if (node->geom_type == POLYGONTYPE) { printf(" O(%.5g %.5g)", node->pt_outside.x, node->pt_outside.y); }
        printf("\n");
    }
