...
#define CHECKCB(be, method) \
  do \
  { \
    if (!(be)->cb || !(be)->cb->method) lwerror("Callback " #method " not registered by backend"); \
  } while (0)
...
_lwt_toposnap(LWGEOM *src, LWGEOM *tgt, double tol)
{
  ...
  do
  {
    ...
    LWDEBUGF(2,
             "After iteration %d, geometry changed ? %d (%d vs %d vertices)",
             iterations,
             changed,
             lwgeom_count_vertices(tmp2),
             lwgeom_count_vertices(tmp));
    if (tmp != src) lwgeom_free(tmp);
    tmp = tmp2;
  } while (changed && iterations <= maxiterations);

  LWDEBUGF(1, "It took %d/%d iterations to properly snap", iterations, maxiterations);
                       ...
                       LWT_ELEMID myself)
{
  ...
  for (i = 0; i < num_edges; ++i)
  {
    ...
    if (match)
    {
      ...
      GEOSGeom_destroy(eegg);
      GEOSFree(relate);
      if (match == 2) { lwerror("GEOSRelatePatternMatch error: %s", lwgeom_geos_errmsg); }
      else
      {
        lwerror("SQL/MM Spatial exception - coincident edge %" LWTFMT_ELEMID, edge_id);
      }
      return -1;
    }
    ...
    if (match)
    {
      ...
      {
        lwerror("Spatial exception - geometry intersects edge %" LWTFMT_ELEMID, edge_id);
      }
      return -1;
    }
    ...
    if (match)
    {
      ...
      {
        lwerror("SQL/MM Spatial exception - geometry crosses edge %" LWTFMT_ELEMID, edge_id);
      }
      return -1;
    }
...
lwt_AddIsoEdge(LWT_TOPOLOGY *topo, LWT_ELEMID startNode, LWT_ELEMID endNode, const LWLINE *geom)
{
  ...
  for (i = 0; i < num_nodes; ++i)
  {
    ...
    if (containing_face == -1)
      containing_face = n->containing_face;
  ...
  ret = lwt_be_updateNodesById(topo, updated_nodes, 2, LWT_COL_NODE_CONTAINING_FACE);
  if (ret == -1)
  {
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
    return -1;
...
lwt_ModEdgeSplit(LWT_TOPOLOGY *topo, LWT_ELEMID edge, LWPOINT *pt, int skipISOChecks)
{
  ...
  newedge1.next_left = oldedge->next_left == -oldedge->edge_id ? -newedge1.edge_id : oldedge->next_left;
  ...
  ret = lwt_be_updateEdges(topo,
                           &seledge,
                           LWT_COL_EDGE_NEXT_RIGHT | LWT_COL_EDGE_START_NODE,
                           &updedge,
  ...
  ret = lwt_be_updateEdges(topo,
                           &seledge,
                           LWT_COL_EDGE_NEXT_LEFT | LWT_COL_EDGE_END_NODE,
                           &updedge,
...
lwt_NewEdgesSplit(LWT_TOPOLOGY *topo, LWT_ELEMID edge, LWPOINT *pt, int skipISOChecks)
{
  ...
  ret = lwt_be_updateEdges(
      topo, &seledge, LWT_COL_EDGE_NEXT_RIGHT | LWT_COL_EDGE_START_NODE, &updedge, LWT_COL_EDGE_NEXT_RIGHT, NULL, 0);
  if (ret == -1)
  {
    _lwt_release_edges(oldedge, 1);
    lwcollection_free(split_col);
  ...
  ret = lwt_be_updateEdges(
      topo, &seledge, LWT_COL_EDGE_NEXT_LEFT | LWT_COL_EDGE_END_NODE, &updedge, LWT_COL_EDGE_NEXT_LEFT, NULL, 0);
  if (ret == -1)
  {
    _lwt_release_edges(oldedge, 1);
    lwcollection_free(split_col);
...
_lwt_InitEdgeEndByLine(edgeend *fee, edgeend *lee, LWLINE *edge, POINT2D *fp, POINT2D *lp)
{
  ...
  fee->cwFace = fee->ccwFace = lee->cwFace = lee->ccwFace = -1;
  ...
  {
    lwerror("error computing azimuth of first edgeend [%.15g %.15g,%.15g %.15g]", fp->x, fp->y, pt.x, pt.y);
    return -2;
  }
  LWDEBUGF(1, "azimuth of first edge end [%.15g %.15g,%.15g %.15g] is %g", fp->x, fp->y, pt.x, pt.y, fee->myaz);
  ...
  {
    lwerror("error computing azimuth of last edgeend [%.15g %.15g,%.15g %.15g]", lp->x, lp->y, pt.x, pt.y);
    return -2;
  }
  LWDEBUGF(1, "azimuth of last edge end [%.15g %.15g,%.15g %.15g] is %g", lp->x, lp->y, pt.x, pt.y, lee->myaz);
...
_lwt_FindAdjacentEdges(LWT_TOPOLOGY *topo, LWT_ELEMID node, edgeend *data, edgeend *other, int myedge_id)
{
  ...
  if (other)
  {
    ...
    LWDEBUGF(1, "Other edge end has cwFace=%d and ccwFace=%d", other->cwFace, other->ccwFace);
  }
  else
  {
    minaz = maxaz = -1;
  ...
  LWDEBUGF(1, "Looking for edges incident to node %" LWTFMT_ELEMID " and adjacent to azimuth %g", node, data->myaz);
  ...
  LWDEBUGF(1, "getEdgeByNode returned %d edges, minaz=%g, maxaz=%g", numedges, minaz, maxaz);
  ...
  for (i = 0; i < numedges; ++i)
  {
    ...
    {
      {
        ...
        lwerror("corrupted topology: edge %" LWTFMT_ELEMID " does not have two distinct points", id);
        return -1;
      }
    }
    ...
    if (edge->start_node == node)
    {
      ...
      {
        lwerror("Edge %d has no distinct vertices: [%.15g %.15g,%.15g %.15g]: ", edge->edge_id, p1.x, p1.y, p2.x, p2.y);
        return -1;
      }
      LWDEBUGF(1,
               "edge %" LWTFMT_ELEMID " starts on node %" LWTFMT_ELEMID ", edgeend is %g,%g-%g,%g",
               edge->edge_id,
               node,
               p1.x,
               p1.y,
      ...
      {
        {
          ...
          lwgeom_free(cleangeom);
          lwerror(
              "error computing azimuth of edge %d first edgeend [%.15g %.15g,%.15g "
              "%.15g]",
              id,
              p1.x,
              p1.y,
      ...
      azdif = az - data->myaz;
      LWDEBUGF(1, "azimuth of edge %" LWTFMT_ELEMID ": %g (diff: %g)", edge->edge_id, az, azdif);
    ...
    if (edge->end_node == node)
    {
      ...
      LWDEBUGF(1,
               "edge %" LWTFMT_ELEMID " ends on node %" LWTFMT_ELEMID ", edgeend is %g,%g-%g,%g",
               edge->edge_id,
               node,
               p1.x,
               p1.y,
      ...
      {
        {
          ...
          lwgeom_free(cleangeom);
          lwerror(
              "error computing azimuth of edge %d last edgeend [%.15g %.15g,%.15g %.15g]", id, p1.x, p1.y, p2.x, p2.y);
          return -1;
        }
      }
      azdif = az - data->myaz;
...
_lwt_MakeRingShell(LWT_TOPOLOGY *topo, LWT_ELEMID *signed_edge_ids, int num_signed_edge_ids)
{
  ...
  i = numedges;
  ring_edges = lwt_be_getEdgeById(topo, edge_ids, &i, LWT_COL_EDGE_EDGE_ID | LWT_COL_EDGE_GEOM);
  lwfree(edge_ids);
  if (i == -1)
  {
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
...
_lwt_AddFaceSplit(LWT_TOPOLOGY *topo, LWT_ELEMID sedge, LWT_ELEMID face, int mbr_only)
{
  ...
  if (face != 0 && !isccw)
  { /* ring is clockwise in a real face */
  ...
  int fields = LWT_COL_EDGE_EDGE_ID | LWT_COL_EDGE_FACE_LEFT | LWT_COL_EDGE_FACE_RIGHT | LWT_COL_EDGE_GEOM;
  numfaceedges = 1;
  ...
  if (numfaceedges)
  {
    ...
    for (i = 0; i < numfaceedges; ++i)
    {
      ...
      if (!_lwt_GetInteriorEdgePoint(e->geom, &ep))
      {
        ...
        _lwt_release_edges(edges, numfaceedges);
        lwerror("Could not find interior point for edge %d: %s", e->edge_id, lwgeom_geos_errmsg);
        return -2;
      }
      ...
      LWDEBUGF(1, "Edge %d %scontained in new ring", e->edge_id, (contains ? "" : "not "));
      ...
      if (newface_outside)
      {
        if (contains)
        {
          LWDEBUGF(1, "Edge %d contained in an hole of the new face", e->edge_id);
          continue;
        }
      }
      else
      {
        if (!contains)
        {
          LWDEBUGF(1, "Edge %d not contained in the face shell", e->edge_id);
          continue;
        }
      }
    ...
    {
      ret = lwt_be_updateEdgesById(topo, forward_edges, forward_edges_count, LWT_COL_EDGE_FACE_LEFT);
      if (ret == -1)
      {
        lwfree(signed_edge_ids);
        lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
      ...
      {
        lwfree(signed_edge_ids);
        lwerror("Unexpected error: %d edges updated when expecting %d", ret, forward_edges_count);
        return -2;
      }
    }
    ...
    {
      ret = lwt_be_updateEdgesById(topo, backward_edges, backward_edges_count, LWT_COL_EDGE_FACE_RIGHT);
      if (ret == -1)
      {
        lwfree(signed_edge_ids);
        lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
      ...
      {
        lwfree(signed_edge_ids);
        lwerror("Unexpected error: %d edges updated when expecting %d", ret, backward_edges_count);
        return -2;
      }
    }
  ...
  if (numisonodes)
  {
    ...
    for (i = 0; i < numisonodes; ++i)
    {
      ...
      if (!ngg)
      {
        ...
        lwfree(signed_edge_ids);
        lwpoly_free(shell);
        lwerror("Could not convert node geometry to GEOS: %s", lwgeom_geos_errmsg);
        return -2;
      }
      ...
      if (newface_outside)
      {
        if (contains)
        {
          LWDEBUGF(1, "Node %d contained in an hole of the new face", n->node_id);
          continue;
        }
      }
      else
      {
        if (!contains)
        {
          LWDEBUGF(1, "Node %d not contained in the face shell", n->node_id);
          continue;
        }
      }
      updated_nodes[nodes_to_update].node_id = n->node_id;
      updated_nodes[nodes_to_update++].containing_face = newface.face_id;
      LWDEBUGF(1, "Node %d will be updated", n->node_id);
    }
    _lwt_release_nodes(nodes, numisonodes);
    if (nodes_to_update)
    {
      int ret = lwt_be_updateNodesById(topo, updated_nodes, nodes_to_update, LWT_COL_NODE_CONTAINING_FACE);
      if (ret == -1)
      {
        lwfree(signed_edge_ids);
        lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
...
_lwt_AddEdge(LWT_TOPOLOGY *topo, LWT_ELEMID start_node, LWT_ELEMID end_node, LWLINE *geom, int skipChecks, int modFace)
{
  ...
  span.cwFace = span.ccwFace = epan.cwFace = epan.ccwFace = -1;
  ...
  {
    lwgeom_free(cleangeom);
    lwerror("error computing azimuth of first edgeend [%.15g %.15g,%.15g %.15g]", p1.x, p1.y, pn.x, pn.y);
    return -1;
  }
  ...
  {
    lwerror("error computing azimuth of last edgeend [%.15g %.15g,%.15g %.15g]", p2.x, p2.y, pn.x, pn.y);
    return -1;
  }
  ...
  for (i = 0; i < num_nodes; ++i)
  {
    ...
    LWDEBUGF(1, "Node %d, with geom %p (looking for %d and %d)", node->node_id, node->geom, start_node, end_node);
    if (node->node_id == start_node) { start_node_geom = node->geom; }
    if (node->node_id == end_node) { end_node_geom = node->geom; }
  }
  ...
  if (!skipChecks)
  {
    ...
    if (_lwt_CheckEdgeCrossing(topo, start_node, end_node, geom, 0)) return -1;
  ...
  int found;
  found = _lwt_FindAdjacentEdges(topo, start_node, &span, isclosed ? &epan : NULL, -1);
  if (found)
  {
    span.was_isolated = 0;
    ...
    LWDEBUGF(1,
             ...
             newedge.next_right,
             prev_left);
    if (newedge.face_right == -1) { newedge.face_right = span.cwFace; }
    if (newedge.face_left == -1) { newedge.face_left = span.ccwFace; }
  }
  else
  {
    span.was_isolated = 1;
  ...
  found = _lwt_FindAdjacentEdges(topo, end_node, &epan, isclosed ? &span : NULL, -1);
  if (found)
  {
    epan.was_isolated = 0;
    ...
    LWDEBUGF(1,
             ...
             newedge.next_left,
             prev_right);
    if (newedge.face_right == -1) { newedge.face_right = span.ccwFace; }
    ...
    if (newedge.face_left == -1) { newedge.face_left = span.cwFace; }
  ...
  if (llabs(prev_left) != newedge.edge_id)
  {
    ...
    ret = lwt_be_updateEdges(topo, &seledge, LWT_COL_EDGE_EDGE_ID, &updedge, updfields, NULL, 0);
    if (ret == -1)
    {
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
      return -1;
  ...
  if (span.was_isolated)
  {
    ...
    ret = lwt_be_updateNodes(topo, &selnode, LWT_COL_NODE_NODE_ID, &updnode, LWT_COL_NODE_CONTAINING_FACE, NULL, 0);
    if (ret == -1)
    {
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
      return -1;
  ...
  if (modFace > -1)
  {
    ...
    int newface = _lwt_AddFaceSplit(topo, newedge.edge_id, newedge.face_left, 0);
    if (modFace)
    {
      if (newface == 0)
      {
      ...
      if (newface < 0)
      {
        ...
        newface = _lwt_AddFaceSplit(topo, -newedge.edge_id, newedge.face_left, 0);
    ...
    {
      ret = lwt_be_updateTopoGeomFaceSplit(topo, newedge.face_left, newface, newface1);
      if (ret == 0)
      {
        lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
        return -1;
...
_lwt_FaceByEdges(LWT_TOPOLOGY *topo, LWT_ISO_EDGE *edges, int numfaceedges)
{
  ...
  if (!validedges)
  {
    ...
    return lwpoly_as_lwgeom(lwpoly_construct_empty(topo->srid, topo->hasZ, 0));
  }
...
lwt_GetFaceGeometry(LWT_TOPOLOGY *topo, LWT_ELEMID faceid)
{
  ...
  numfaceedges = 1;
  fields = LWT_COL_EDGE_GEOM | LWT_COL_EDGE_FACE_LEFT | LWT_COL_EDGE_FACE_RIGHT;
  ...
  if (numfaceedges == 0)
  {
    ...
    if (i > 1)
    {
      lwerror("Corrupted topology: multiple face records have face_id=%" LWTFMT_ELEMID, faceid);
      return NULL;
    }
...
_lwt_FindNextRingEdge(const POINTARRAY *ring, int from, const LWT_ISO_EDGE *edges, int numedges)
{
  ...
  for (i = 0; i < numedges; ++i)
  {
    ...
    {
      LWDEBUGF(3,
               "_lwt_FindNextRingEdge: edge %" LWTFMT_ELEMID " has same face (%" LWTFMT_ELEMID
    ...
    {
      LWDEBUGF(
          3, "_lwt_FindNextRingEdge: edge %" LWTFMT_ELEMID " has only %" PRIu32 " points", isoe->edge_id, epa->npoints);
      continue;
    }
    ...
    LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " 'first' point is %g,%g", isoe->edge_id, p2.x, p2.y);
    LWDEBUGF(1, "Rings's 'from' point is still %g,%g", p1.x, p1.y);
    if (p2d_same(&p1, &p2))
    {
      LWDEBUG(1, "p2d_same(p1,p2) returned true");
      LWDEBUGF(1, "First point of edge %" LWTFMT_ELEMID " matches ring vertex %d", isoe->edge_id, from);
      /* first point matches, let's check next non-equal one */
      for (j = 1; j < epa->npoints; ++j)
      {
        getPoint2d_p(epa, j, &p2);
        LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " 'next' point %d is %g,%g", isoe->edge_id, j, p2.x, p2.y);
        ...
        LWDEBUGF(1, "Ring's point %d is %g,%g", from + 1, pt.x, pt.y);
        match = p2d_same(&pt, &p2);
        break; /* we want to check a single non-equal next vertex */
      }
#if POSTGIS_DEBUG_LEVEL > 0
      if (match)
      { LWDEBUGF(1, "Prev point of edge %" LWTFMT_ELEMID " matches ring vertex %d", isoe->edge_id, from + 1); }
      else
      {
        LWDEBUGF(1, "Prev point of edge %" LWTFMT_ELEMID " does not match ring vertex %d", isoe->edge_id, from + 1);
      }
#endif
    }
    ...
    if (!match)
    {
      LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " did not match as forward", isoe->edge_id);
      getPoint2d_p(epa, epa->npoints - 1, &p2);
      LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " 'last' point is %g,%g", isoe->edge_id, p2.x, p2.y);
      if (p2d_same(&p1, &p2))
      {
        LWDEBUGF(1, "Last point of edge %" LWTFMT_ELEMID " matches ring vertex %d", isoe->edge_id, from);
        /* last point matches, let's check next non-equal one */
        for (j = 2; j <= epa->npoints; j++)
        {
          getPoint2d_p(epa, epa->npoints - j, &p2);
          LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " 'prev' point %d is %g,%g", isoe->edge_id, epa->npoints - j, p2.x, p2.y);
          /* we won't check duplicated edge points */
          if (p2d_same(&p1, &p2)) continue;
          /* we assume there are no duplicated points in ring */
          getPoint2d_p(ring, from + 1, &pt);
...
lwt_GetFaceEdges(LWT_TOPOLOGY *topo, LWT_ELEMID face_id, LWT_ELEMID **out)
{
  ...
  numfaceedges = 1;
  fields = LWT_COL_EDGE_EDGE_ID | LWT_COL_EDGE_GEOM | LWT_COL_EDGE_FACE_LEFT | LWT_COL_EDGE_FACE_RIGHT;
  ...
  for (i = 0; i < facepoly->nrings; ++i)
  {
    ...
    {
      LWDEBUGF(1, "Looking for edge covering ring %d from vertex %d", i, j);
      ...
      if (edgeno == -1)
      {
        ...
        lwgeom_free(face);
        lwfree(seid);
        lwerror("No edge (among %d) found to be defining geometry of face %" LWTFMT_ELEMID, numfaceedges, face_id);
        return -1;
      }
      ...
      LWDEBUGF(1,
               "Edge %" LWTFMT_ELEMID " covers ring %d from vertex %d to %d",
               nextedge->edge_id,
               i,
               j,
      ...
      seid[nseid++] = nextedge->face_left == face_id ? nextedge->edge_id : -nextedge->edge_id;
    ...
    {
      {
        ...
        LWDEBUGF(1, "Smallest id is %" LWTFMT_ELEMID " at position %d", minid, minidx);
        if (minidx != prevseid) { _lwt_RotateElemidArray(seid, prevseid, nseid, minidx); }
      }
    }
...
_lwt_EdgeMotionArea(LWLINE *geom, int isclosed)
{
  ...
  if (!gg)
  {
    lwerror("Could not convert old edge area geometry to GEOS: %s", lwgeom_geos_errmsg);
    return NULL;
  }
  return gg;
}
...
lwt_ChangeEdgeGeom(LWT_TOPOLOGY *topo, LWT_ELEMID edge_id, LWLINE *geom)
{
  ...
  if (!oldedge)
  {
    LWDEBUGF(1,
             "lwt_ChangeEdgeGeom: "
             "lwt_be_getEdgeById returned NULL and set i=%d",
             i);
    if (i == -1)
    {
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
  ...
  {
    _lwt_release_edges(oldedge, 1);
    lwerror("Corrupted topology: edge %" LWTFMT_ELEMID " has less than 2 vertices", oldedge->edge_id);
    return -1;
  }
  ...
  if (oldedge->start_node == oldedge->end_node)
  {
    ...
    if (ptarray_isccw(oldedge->geom->points) != ptarray_isccw(geom->points))
    {
      _lwt_release_edges(oldedge, 1);
  ...
  if (_lwt_CheckEdgeCrossing(topo, oldedge->start_node, oldedge->end_node, geom, edge_id))
  {
    /* would have called lwerror already, leaking :( */
    _lwt_release_edges(oldedge, 1);
    return -1;
  ...
  i = _lwt_InitEdgeEndByLine(&span_pre, &epan_pre, oldedge->geom, &p1, &p2);
  if (i) return -1; /* lwerror should have been raised */
  _lwt_FindAdjacentEdges(topo, oldedge->start_node, &span_pre, isclosed ? &epan_pre : NULL, edge_id);
  _lwt_FindAdjacentEdges(topo, oldedge->end_node, &epan_pre, isclosed ? &span_pre : NULL, edge_id);

  LWDEBUGF(1,
           "edges adjacent to old edge are %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID " (first point), %" LWTFMT_ELEMID
  ...
  i = _lwt_InitEdgeEndByLine(&span_post, &epan_post, geom, &p1, &p2);
  if (i) return -1; /* lwerror should have been raised */
  _lwt_FindAdjacentEdges(topo, oldedge->start_node, &span_post, isclosed ? &epan_post : NULL, edge_id);
  _lwt_FindAdjacentEdges(topo, oldedge->end_node, &epan_post, isclosed ? &span_post : NULL, edge_id);

  LWDEBUGF(1,
           "edges adjacent to new edge are %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID " (first point), %" LWTFMT_ELEMID
           " and %" LWTFMT_ELEMID " (last point)",
           span_pre.nextCW,
           span_pre.nextCCW,
           epan_pre.nextCW,
  ...
  if (span_pre.nextCW != span_post.nextCW || span_pre.nextCCW != span_post.nextCCW)
  {
    {
      LWT_ELEMID nid = oldedge->start_node;
      _lwt_release_edges(oldedge, 1);
      lwerror("Edge changed disposition around start node %" LWTFMT_ELEMID, nid);
      return -1;
    }
  }

  /* Bail out if next CW or CCW edge on end node changed */
  if (epan_pre.nextCW != epan_post.nextCW || epan_pre.nextCCW != epan_post.nextCCW)
  {
    {
      LWT_ELEMID nid = oldedge->end_node;
      _lwt_release_edges(oldedge, 1);
      lwerror("Edge changed disposition around end node %" LWTFMT_ELEMID, nid);
      return -1;
    }
  }
  ...
  {
    {
      ...
      if (oldedge->face_left > 0)
      {
        ...
        if (!nface1)
        {
          lwerror("lwt_ChangeEdgeGeom could not construct face %" LWTFMT_ELEMID ", on the left of edge %" LWTFMT_ELEMID,
                  oldedge->face_left,
                  edge_id);
          return -1;
        }
          ...
          && oldedge->face_right != oldedge->face_left)
      {
        ...
        if (!nface2)
        {
          lwerror("lwt_ChangeEdgeGeom could not construct face %" LWTFMT_ELEMID
...
lwt_MoveIsoNode(LWT_TOPOLOGY *topo, LWT_ELEMID nid, LWPOINT *pt)
{
  ...
  node->node_id = nid;
  node->geom = pt;
  ret = lwt_be_updateNodesById(topo, node, 1, LWT_COL_NODE_GEOM);
  if (ret == -1)
  {
    lwfree(node);
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
...
lwt_RemIsoEdge(LWT_TOPOLOGY *topo, LWT_ELEMID id)
{
  ...
  edge = lwt_be_getEdgeById(topo,
                            &id,
                            &n,
                            LWT_COL_EDGE_START_NODE | LWT_COL_EDGE_END_NODE | LWT_COL_EDGE_FACE_LEFT |
  ...
  {
    lwfree(edge);
    lwerror("Corrupted topology: more than a single edge have id %" LWTFMT_ELEMID, id);
    return -1;
  }
  ...
  n = lwt_be_updateNodesById(topo, upd_node, n, LWT_COL_NODE_CONTAINING_FACE);
  if (n == -1)
  {
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
    return -1;
...
_lwt_UpdateEdgeFaceRef(LWT_TOPOLOGY *topo, LWT_ELEMID of, LWT_ELEMID nf)
{
  ...
  ret = lwt_be_updateEdges(topo, &sel_edge, LWT_COL_EDGE_FACE_LEFT, &upd_edge, LWT_COL_EDGE_FACE_LEFT, NULL, 0);
  if (ret == -1) return -1;

  /* Update face_right for all edges still referencing old face */
  sel_edge.face_right = of;
  upd_edge.face_right = nf;
  ret = lwt_be_updateEdges(topo, &sel_edge, LWT_COL_EDGE_FACE_RIGHT, &upd_edge, LWT_COL_EDGE_FACE_RIGHT, NULL, 0);
  if (ret == -1) return -1;
...
_lwt_UpdateNodeFaceRef(LWT_TOPOLOGY *topo, LWT_ELEMID of, LWT_ELEMID nf)
{
  ...
  ret = lwt_be_updateNodes(topo, &sel, LWT_COL_NODE_CONTAINING_FACE, &upd, LWT_COL_NODE_CONTAINING_FACE, NULL, 0);
  if (ret == -1) return -1;
...
_lwt_RemEdge(LWT_TOPOLOGY *topo, LWT_ELEMID edge_id, int modFace)
{
  ...
  if (!lwt_be_checkTopoGeomRemEdge(topo, edge_id, edge->face_left, edge->face_right))
  {
  ...
  if (edge->end_node != edge->start_node) { node_ids[nedges++] = edge->end_node; }
  fields = LWT_COL_EDGE_EDGE_ID | LWT_COL_EDGE_START_NODE | LWT_COL_EDGE_END_NODE | LWT_COL_EDGE_NEXT_LEFT |
           LWT_COL_EDGE_NEXT_RIGHT;
  ...
  for (i = 0; i < nedges; ++i)
  {
    ...
    if (e->next_left == -edge_id)
    {
      ...
      upd_edge_left[nedge_left++].next_left = edge->next_left != edge_id ? edge->next_left : edge->next_right;
    }
    else if (e->next_left == edge_id)
    {
      upd_edge_left[nedge_left].edge_id = e->edge_id;
      upd_edge_left[nedge_left++].next_left = edge->next_right != -edge_id ? edge->next_right : edge->next_left;
    }

    if (e->next_right == -edge_id)
    {
      upd_edge_right[nedge_right].edge_id = e->edge_id;
      upd_edge_right[nedge_right++].next_right = edge->next_left != edge_id ? edge->next_left : edge->next_right;
    }
    else if (e->next_right == edge_id)
    {
      upd_edge_right[nedge_right].edge_id = e->edge_id;
      upd_edge_right[nedge_right++].next_right = edge->next_right != -edge_id ? edge->next_right : edge->next_left;
    }
  }
  ...
  if (nedge_left)
  {
    ...
    i = lwt_be_updateEdgesById(topo, &(upd_edge_left[0]), nedge_left, LWT_COL_EDGE_NEXT_LEFT);
    if (i == -1)
    {
  ...
  if (nedge_right)
  {
    ...
    i = lwt_be_updateEdgesById(topo, &(upd_edge_right[0]), nedge_right, LWT_COL_EDGE_NEXT_RIGHT);
    if (i == -1)
    {
      _lwt_release_edges(edge, 1);
      lwfree(upd_edge);
  ...
  if (edge->face_left == edge->face_right) { floodface = edge->face_right; }
  else
  {
    /* Two faces healed */
    ...
    else
    {
      ...
      for (i = 0; i < nfaces; ++i)
      {
        if (faces[i].face_id == edge->face_left)
        {
          ...
          else
          {
            ...
            lwerror("corrupted topology: more than 1 face have face_id=%" LWTFMT_ELEMID, i);
            return -1;
          }
        }
        ...
        else
        {
          ...
          lwerror(
              ...
              "with non-requested id %" LWTFMT_ELEMID,
              i);
          return -1;
        }
      }
      if (!box1)
      {
        ...
        _lwt_release_edges(edge, 1);
        _lwt_release_faces(faces, nfaces);
        lwerror("corrupted topology: no face have face_id=%" LWTFMT_ELEMID " (left face for edge %" LWTFMT_ELEMID ")",
                i,
                edge_id);
        return -1;
      }
      if (!box2)
      {
        ...
        _lwt_release_edges(edge, 1);
        _lwt_release_faces(faces, nfaces);
        lwerror("corrupted topology: no face have face_id=%" LWTFMT_ELEMID " (right face for edge %" LWTFMT_ELEMID ")",
                i,
                edge_id);
        return -1;
      }
    ...
    if (!lwt_be_updateTopoGeomFaceHeal(topo, edge->face_right, edge->face_left, floodface))
    {
      _lwt_release_edges(edge, 1);
      lwerror("%s", lwt_be_lastErrorMessage(topo->be_iface));
      return -1;
  ...
  if (nnode)
  {
    i = lwt_be_updateNodesById(topo, upd_node, nnode, LWT_COL_NODE_CONTAINING_FACE);
    if (i == -1)
    {
      _lwt_release_edges(edge, 1);
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
  ...
  if (edge->face_left != edge->face_right) /* or there'd be no face to remove */
  {
    LWT_ELEMID ids[2];
    int nids = 0;
    if (edge->face_right != floodface) ids[nids++] = edge->face_right;
    if (edge->face_left != floodface) ids[nids++] = edge->face_left;
...
_lwt_HealEdges(LWT_TOPOLOGY *topo, LWT_ELEMID eid1, LWT_ELEMID eid2, int modEdge)
{
  ...
  if (eid1 == eid2)
  {
    lwerror("Cannot heal edge %" LWTFMT_ELEMID " with itself, try with another", eid1);
    return -1;
  }
  ids[0] = eid1;
  ids[1] = eid2;
  ...
  for (i = 0; i < nedges; ++i)
  {
    if (edges[i].edge_id == eid1)
    {
      if (e1)
      {
        ...
        lwerror("Corrupted topology: multiple edges have id %" LWTFMT_ELEMID, eid1);
        return -1;
      }
      e1 = &(edges[i]);
    }
    else if (edges[i].edge_id == eid2)
    {
      ...
      {
        _lwt_release_edges(edges, nedges);
        lwerror("Corrupted topology: multiple edges have id %" LWTFMT_ELEMID, eid2);
        return -1;
      }
      e2 = &(edges[i]);
    }
  ...
  {
    _lwt_release_edges(edges, nedges);
    lwerror("SQL/MM Spatial exception - non-existent edge %" LWTFMT_ELEMID, eid1);
    return -1;
  }
  if (!e2)
  {
    _lwt_release_edges(edges, nedges);
    lwerror("SQL/MM Spatial exception - non-existent edge %" LWTFMT_ELEMID, eid2);
    return -1;
  }
  ...
  {
    _lwt_release_edges(edges, nedges);
    lwerror("Edge %" LWTFMT_ELEMID " is closed, cannot heal to edge %" LWTFMT_ELEMID, eid1, eid2);
    return -1;
  }
  if (e2->start_node == e2->end_node)
  {
    _lwt_release_edges(edges, nedges);
    lwerror("Edge %" LWTFMT_ELEMID " is closed, cannot heal to edge %" LWTFMT_ELEMID, eid2, eid1);
    return -1;
  }
  ...
  {
    num_node_edges = 1;
    node_edges = lwt_be_getEdgeByNode(topo, &commonnode, &num_node_edges, LWT_COL_EDGE_EDGE_ID);
    if (num_node_edges == -1)
    {
      _lwt_release_edges(edges, nedges);
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
    ...
    for (i = 0; i < num_node_edges; ++i)
    {
      ...
      if (bufleft > 0)
      {
        r = snprintf(ptr, bufleft, "%s%" LWTFMT_ELEMID, (ptr == buf ? "" : ","), node_edges[i].edge_id);
        if (r >= (int)bufleft)
        {
          bufleft = 0;
          buf[252] = '.';
  ...
  {
    _lwt_release_edges(edges, nedges);
    if (ptr != buf) { lwerror("SQL/MM Spatial exception - other edges connected (%s)", buf); }
    else
    {
  ...
  if (!lwt_be_checkTopoGeomRemNode(topo, commonnode, eid1, eid2))
  {
    _lwt_release_edges(edges, nedges);
    lwerror("%s", lwt_be_lastErrorMessage(topo->be_iface));
    return -1;
  ...
  */
  ...
  i = lwt_be_updateEdges(topo, &seledge, LWT_COL_EDGE_NEXT_LEFT, &updedge, LWT_COL_EDGE_NEXT_LEFT, NULL, 0);
  if (i == -1)
  {
    _lwt_release_edges(edges, nedges);
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
  ...
  i = lwt_be_updateEdges(topo, &seledge, LWT_COL_EDGE_NEXT_RIGHT, &updedge, LWT_COL_EDGE_NEXT_RIGHT, NULL, 0);
  if (i == -1)
  {
    _lwt_release_edges(edges, nedges);
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
  ...
  */
  if (!lwt_be_updateTopoGeomEdgeHeal(topo, eid1, eid2, newedge.edge_id))
  {
    lwerror("%s", lwt_be_lastErrorMessage(topo->be_iface));
    return -1;
  }
...
lwt_GetFaceByPoint(LWT_TOPOLOGY *topo, LWPOINT *pt, double tol)
{
  ...
  for (i = 0; i < num; ++i)
  {
    ...
    {
      _lwt_release_edges(elem, num);
      lwnotice("Corrupted topology: edge %" LWTFMT_ELEMID " has null geometry", e->edge_id);
      continue;
    }
    ...
    {
      LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " is dangling, won't consider it", e->edge_id);
      continue;
    }
    ...
    LWDEBUGF(1, "Distance from edge %" LWTFMT_ELEMID " is %g (tol=%g)", e->edge_id, dist, tol);

    /* we won't consider edges too far */
    if (dist > tol) continue;
    if (e->face_left == 0) { eface = e->face_right; }
...
#define _LWT_MINTOLERANCE(topo, geom) (topo->precision ? topo->precision : _lwt_minTolerance(geom))
    ...
    {
      {
        ...
        for (j = 0, i = 0; i < num; ++i)
        {
          if (sorted[i].score == sorted[0].score) { edges2[j++] = *((LWT_ISO_EDGE *)sorted[i].ptr); }
          else
          {
      ...
      {
        {
          ...
          LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " does not contain projected point to it", edge_id);
...
_lwt_AddLineEdge(LWT_TOPOLOGY *topo, LWLINE *edge, double tol, int handleFaceSplit)
{
  ...
  nid[0] = _lwt_AddPoint(topo, start_point, _lwt_minTolerance(lwpoint_as_lwgeom(start_point)), handleFaceSplit, &mm);
  ...
  nid[1] = _lwt_AddPoint(topo, end_point, _lwt_minTolerance(lwpoint_as_lwgeom(end_point)), handleFaceSplit, &mm);
  moved += mm;
  ...
  */
  if (moved)
  {
    ...
    nn = nid[0] == nid[1] ? 1 : 2;
    node = lwt_be_getNodeById(topo, nid, &nn, LWT_COL_NODE_NODE_ID | LWT_COL_NODE_GEOM);
    if (nn == -1)
    {
      lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
      return -1;
    ...
    if (!start_point || !end_point)
    {
      ...
      lwerror("Could not find just-added nodes % " LWTFMT_ELEMID " and %" LWTFMT_ELEMID, nid[0], nid[1]);
      return -1;
    }
    ...
    else
    {
      ...
      if (!edge)
      {
        LWDEBUGF(1, "Made-valid snapped edge collapsed to %s", lwtype_name(lwgeom_get_type(tmp)));
        lwgeom_free(tmp);
        return 0;
      }
    }
...
_lwt_AddLine(LWT_TOPOLOGY *topo, LWLINE *line, double tol, int *nedges, int handleFaceSplit)
{
  ...
  *nedges = -1; /* error condition, by default */
  ...
  LWDEBUGF(1, "Line BOX is %.15g %.15g, %.15g %.15g", qbox.xmin, qbox.ymin, qbox.xmax, qbox.ymax);
  ...
  LWDEBUGF(1, "Line has %d points, its bbox intersects %d edges bboxes", line->points->npoints, numedges);
  if (numedges)
  {
    {
  ...
  {
    {
      ...
      nearby = nearby ? lwrealloc(nearby, nearbycount * sizeof(LWGEOM *)) : lwalloc(nearbycount * sizeof(LWGEOM *));
      int nn = 0;
      for (i = 0; i < numnodes; ++i)
      {
        LWT_ISO_NODE *n = &(nodes[i]);
  ...
  {
    {
      ...
      {
        {
          col = lwcollection_construct(
              MULTIPOINTTYPE, topo->srid, NULL, nearbyindex - nearbyedgecount, nearby + nearbyedgecount);
  ...
  for (i = 0; i < ngeoms; ++i)
  {
    ...
    LWDEBUGF(1, "Component %d of split line is edge %" LWTFMT_ELEMID, i, id);
...
lwt_AddPolygon(LWT_TOPOLOGY *topo, LWPOLY *poly, double tol, int *nfaces)
{
  ...
  */
  ...
  gbox_expand(&qbox, tol);
  faces = lwt_be_getFaceWithinBox2D(topo, &qbox, &nfacesinbox, LWT_COL_FACE_ALL, 0);
  if (nfacesinbox == -1)
  {
    lwfree(ids);
    lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
  ...
  if (nfacesinbox)
  {
    ...
    for (j = 0; j < nfacesinbox; ++j)
    {
      ...
      if (!covers) { continue; /* we're not composed by this face */ }
...
_lwt_UpdateEdgeRingSideFace(LWT_TOPOLOGY *topo, LWT_EDGERING *ring, LWT_ELEMID face)
{
  ...
  if (forward_edges_count)
  {
    ...
    if (ret != forward_edges_count)
    {
      lwfree(forward_edges);
      lwfree(backward_edges);
      lwerror("Unexpected error: %d edges updated when expecting %d (forward)", ret, forward_edges_count);
      return -1;
    }
  }
  ...
  if (backward_edges_count)
  {
    ...
    if (ret != backward_edges_count)
    {
      lwfree(forward_edges);
      lwfree(backward_edges);
      lwerror("Unexpected error: %d edges updated when expecting %d (backward)", ret, backward_edges_count);
      return -1;
    }
  }
                   ...
                   int side)
{
  ...
  do
  {
    ...
    if (elem->left)
      cur->face_left = LWT_HOLES_FACE_PLACEHOLDER;
    else
      cur->face_right = LWT_HOLES_FACE_PLACEHOLDER;
...
_lwt_EdgeRingCrossingCount(const POINT2D *p, LWT_EDGERING_POINT_ITERATOR *it)
{
  ...
  {
    lwerror("_lwt_EdgeRingCrossingCount: V[n] != V[0] (%g %g != %g %g)", v1.x, v1.y, v0.x, v0.y);
    return -1;
  }
#endif
                            ...
                            LWT_ELEMID *registered)
{
  ...
  if (isccw)
  {
    ...
    *registered = newface.face_id;
    ...
    if (ret)
    {
      lwerror("Errors updating edgering side face: %s", lwt_be_lastErrorMessage(iface));
      return -1;
    }
  }
...
_lwt_FindFaceContainingRing(LWT_TOPOLOGY *topo, LWT_EDGERING *ring, LWT_EDGERING_ARRAY *shells)
{
  ...
  LWDEBUGF(1,
           ...
           candidates.size,
           ring->elems[0]->edge->edge_id * (ring->elems[0]->left ? 1 : -1));
  ...
  for (i = 0; i < candidates.size; ++i)
  {
    ...
    {
      LWDEBUGF(1, "Shell %d is on other side of ring", _lwt_EdgeRingGetFace(sring));
      continue;
    }
    ...
    {
      LWDEBUGF(1, "Bbox of shell %d equals that of hole ring", _lwt_EdgeRingGetFace(sring));
      continue;
    }
    ...
    {
      LWDEBUGF(1, "Bbox of shell %d does not contain bbox of ring point", _lwt_EdgeRingGetFace(sring));
      continue;
    }
    ...
    if (contains)
    {
      ...
      LWDEBUGF(1, "Shell %d contains hole of edge %d", _lwt_EdgeRingGetFace(sring), ring->elems[0]->edge->edge_id);
      minenv = shellbox;
...
lwt_Polygonize(LWT_TOPOLOGY *topo)
{
  ...
  */
  ...
  while (1)
  {
    ...
    if (edge->face_left == -1)
    {
      err = _lwt_RegisterFaceOnEdgeSide(topo, edge, 1, &edgetable, &holes, &shells, &newface);
      if (err) break;
      LWDEBUGF(1, "New face on the left of edge %d is %d", edge->edge_id, newface);
    ...
    if (edge->face_right == -1)
    {
      err = _lwt_RegisterFaceOnEdgeSide(topo, edge, -1, &edgetable, &holes, &shells, &newface);
      if (err) break;
      LWDEBUGF(1, "New face on the right of edge %d is %d", edge->edge_id, newface);
  ...
  if (err)
  {
    ...
    lwerror("Errors fetching or registering face-missing edges: %s", lwt_be_lastErrorMessage(iface));
    return -1;
  }
  ...
  for (i = 0; i < holes.size; ++i)
  {
    ...
    if (containing_face == -1)
    {
      ...
      LWT_EDGERING_ARRAY_CLEAN(&holes);
      LWT_EDGERING_ARRAY_CLEAN(&shells);
      lwerror("Errors finding face containing ring: %s", lwt_be_lastErrorMessage(iface));
      return -1;
    }
