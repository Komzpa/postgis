...
_lwt_FindNextRingEdge(const POINTARRAY *ring, int from, const LWT_ISO_EDGE *edges, int numedges)
{
    ...
    for (i = 0; i < numedges; ++i)
    {
        ...
        {
            LWDEBUGF(3,
                     "_lwt_FindNextRingEdge: edge %" LWTFMT_ELEMID " has only %" PRIu32 " points",
                     isoe->edge_id,
                     epa->npoints);
            continue;
        }
    ...
    LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " is %s",
        ...
        if (p2d_same(&p1, &p2))
        {
            ...
            {
                LWDEBUGF(
                    1, "Prev point of edge %" LWTFMT_ELEMID " does not match ring vertex %d", isoe->edge_id, from + 1);
            }
#endif
        }
        ...
        if (!match)
        {
            ...
            if (p2d_same(&p1, &p2))
            {
                ...
                for (j = 2; j <= epa->npoints; j++)
                {
                    ...
                    LWDEBUGF(1,
                             "Edge %" LWTFMT_ELEMID " 'prev' point %d is %g,%g",
                             isoe->edge_id,
                             epa->npoints - j,
                             p2.x,
                             p2.y);
    ...
    for (i = 0; i < facepoly->nrings; ++i)
    {
        ...
        while (j < (int32_t)ring->npoints - 1)
        {
            ...
            if (edgeno == -1)
            {
                ...
                lwfree(seid);
                lwerror(
                    "No edge (among %d) found to be defining geometry of face %" LWTFMT_ELEMID, numfaceedges, face_id);
                return -1;
            }
    ...
    else
    {
        ...
        else
        {
            ...
            if (!box1)
            {
                ...
                _lwt_release_edges(edge, 1);
                _lwt_release_faces(faces, nfaces);
                lwerror("corrupted topology: no face have face_id=%" LWTFMT_ELEMID
                        " (left face for edge %" LWTFMT_ELEMID ")",
                        i,
                        edge_id);
                return -1;
            }
            if (!box2)
            {
                ...
                _lwt_release_faces(faces, nfaces);
                lwerror("corrupted topology: no face have face_id=%" LWTFMT_ELEMID
                        " (right face for edge %" LWTFMT_ELEMID ")",
                        i,
                        edge_id);
                return -1;
            }
    ...
    {
        {
            ...
            nearby =
                nearby ? lwrealloc(nearby, nearbycount * sizeof(LWGEOM *)) : lwalloc(nearbycount * sizeof(LWGEOM *));
            int nn = 0;
            for (i = 0; i < numnodes; ++i)
            {
                LWT_ISO_NODE *n = &(nodes[i]);
...
_lwt_FindFaceContainingRing(LWT_TOPOLOGY *topo, LWT_EDGERING *ring, LWT_EDGERING_ARRAY *shells)
{
    ...
    for (i = 0; i < candidates.size; ++i)
    {
        ...
        if (contains)
        {
            ...
            LWDEBUGF(
                1, "Shell %d contains hole of edge %d", _lwt_EdgeRingGetFace(sring), ring->elems[0]->edge->edge_id);
            minenv = shellbox;
