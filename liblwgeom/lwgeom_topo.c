...
_lwt_FindAdjacentEdges(LWT_TOPOLOGY *topo, LWT_ELEMID node, edgeend *data, edgeend *other, int myedge_id)
{
	...
	LWDEBUGF(
	    1, "Looking for edges incident to node %" LWTFMT_ELEMID " and adjacent to azimuth %g", node, data->myaz);
		...
		{
			{
				...
				lwerror("corrupted topology: edge %" LWTFMT_ELEMID " does not have two distinct points",
					id);
				return -1;
			}
		}
	...
	if (numfaceedges)
	{
		...
		for (i = 0; i < numfaceedges; ++i)
		{
			...
			if (!_lwt_GetInteriorEdgePoint(e->geom, &ep))
			{
				...
				_lwt_release_edges(edges, numfaceedges);
				lwerror(
				    "Could not find interior point for edge %d: %s", e->edge_id, lwgeom_geos_errmsg);
				return -2;
			}
		...
		if (forward_edges_count)
		{
			...
			if (ret != forward_edges_count)
			{
				lwfree(signed_edge_ids);
				lwerror(
				    "Unexpected error: %d edges updated when expecting %d", ret, forward_edges_count);
				return -2;
			}
		}
		...
		{
			ret =
			    lwt_be_updateEdgesById(topo, backward_edges, backward_edges_count, LWT_COL_EDGE_FACE_RIGHT);
			if (ret == -1)
			{
				lwfree(signed_edge_ids);
				lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
			...
			if (ret != backward_edges_count)
			{
				lwfree(signed_edge_ids);
				lwerror(
				    "Unexpected error: %d edges updated when expecting %d", ret, backward_edges_count);
				return -2;
			}
		}
...
_lwt_AddEdge(LWT_TOPOLOGY *topo, LWT_ELEMID start_node, LWT_ELEMID end_node, LWLINE *geom, int skipChecks, int modFace)
{
	...
	if (span.was_isolated)
	{
		...
		ret = lwt_be_updateNodes(
		    topo, &selnode, LWT_COL_NODE_NODE_ID, &updnode, LWT_COL_NODE_CONTAINING_FACE, NULL, 0);
		if (ret == -1)
		{
			lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
			return -1;
...
_lwt_FindNextRingEdge(const POINTARRAY *ring, int from, const LWT_ISO_EDGE *edges, int numedges)
{
    ...
    LWDEBUGF(1, "Edge %" LWTFMT_ELEMID " is %s",
		...
		if (p2d_same(&p1, &p2))
		{
			...
			LWDEBUGF(
			    1, "First point of edge %" LWTFMT_ELEMID " matches ring vertex %d", isoe->edge_id, from);
			/* first point matches, let's check next non-equal one */
			for (j = 1; j < epa->npoints; ++j)
			{
				getPoint2d_p(epa, j, &p2);
				LWDEBUGF(1,
					 "Edge %" LWTFMT_ELEMID " 'next' point %d is %g,%g",
					 isoe->edge_id,
					 j,
					 p2.x,
					 p2.y);
			...
			{
				LWDEBUGF(1,
					 "Prev point of edge %" LWTFMT_ELEMID " matches ring vertex %d",
					 isoe->edge_id,
					 from + 1);
			}
			else
			{
				LWDEBUGF(1,
					 "Prev point of edge %" LWTFMT_ELEMID " does not match ring vertex %d",
					 isoe->edge_id,
					 from + 1);
			}
#endif
		...
		if (!match)
		{
			...
			{
				LWDEBUGF(1,
					 "Last point of edge %" LWTFMT_ELEMID " matches ring vertex %d",
					 isoe->edge_id,
					 from);
	...
	for (i = 0; i < facepoly->nrings; ++i)
	{
		...
		while (j < (int32_t)ring->npoints - 1)
		{
			...
			if (edgeno == -1)
			{
				...
				lwgeom_free(face);
				lwfree(seid);
				lwerror("No edge (among %d) found to be defining geometry of face %" LWTFMT_ELEMID,
					numfaceedges,
					face_id);
				return -1;
			}
...
lwt_ChangeEdgeGeom(LWT_TOPOLOGY *topo, LWT_ELEMID edge_id, LWLINE *geom)
{
	...
	LWDEBUGF(1,
		 "edges adjacent to old edge are %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID
		 " (first point), %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID " (last point)",
		 span_pre.nextCW,
		 span_pre.nextCCW,
		 epan_pre.nextCW,
		 epan_pre.nextCCW);
	...
	LWDEBUGF(1,
		 "edges adjacent to new edge are %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID
		 " (first point), %" LWTFMT_ELEMID " and %" LWTFMT_ELEMID " (last point)",
		 span_pre.nextCW,
		 span_pre.nextCCW,
		 epan_pre.nextCW,
			...
			for (i = 0; i < nfaces; ++i)
			{
				if (faces[i].face_id == edge->face_left)
				{
					...
					else
					{
						...
						lwerror(
						    "corrupted topology: more than 1 face have face_id=%" LWTFMT_ELEMID,
						    i);
						return -1;
					}
				}
...
_lwt_HealEdges(LWT_TOPOLOGY *topo, LWT_ELEMID eid1, LWT_ELEMID eid2, int modEdge)
{
	...
	if (commonnode != -1)
	{
		...
		for (i = 0; i < num_node_edges; ++i)
		{
			...
			{
				r = snprintf(
				    ptr, bufleft, "%s%" LWTFMT_ELEMID, (ptr == buf ? "" : ","), node_edges[i].edge_id);
				if (r >= (int)bufleft)
				{
					bufleft = 0;
					buf[252] = '.';
	...
	*/
	...
	if (!modEdge)
	{
		...
		i = lwt_be_updateEdges(
		    topo, &seledge, LWT_COL_EDGE_NEXT_LEFT, &updedge, LWT_COL_EDGE_NEXT_LEFT, NULL, 0);
		if (i == -1)
		{
			_lwt_release_edges(edges, nedges);
			lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
		...
		i = lwt_be_updateEdges(
		    topo, &seledge, LWT_COL_EDGE_NEXT_RIGHT, &updedge, LWT_COL_EDGE_NEXT_RIGHT, NULL, 0);
		if (i == -1)
		{
			_lwt_release_edges(edges, nedges);
			lwerror("Backend error: %s", lwt_be_lastErrorMessage(topo->be_iface));
			...
			{
				{
					...
					LWDEBUGF(1,
						 "Edge %" LWTFMT_ELEMID " does not contain projected point to it",
						 edge_id);
...
_lwt_AddLineEdge(LWT_TOPOLOGY *topo, LWLINE *edge, double tol, int handleFaceSplit)
{
	...
	nid[0] =
	    _lwt_AddPoint(topo, start_point, _lwt_minTolerance(lwpoint_as_lwgeom(start_point)), handleFaceSplit, &mm);
	...
	*/
	if (moved)
	{
		...
		if (!start_point || !end_point)
		{
			...
			lwerror(
			    "Could not find just-added nodes % " LWTFMT_ELEMID " and %" LWTFMT_ELEMID, nid[0], nid[1]);
			return -1;
		}
		...
		else
		{
			...
			{
				LWDEBUGF(
				    1, "Made-valid snapped edge collapsed to %s", lwtype_name(lwgeom_get_type(tmp)));
				lwgeom_free(tmp);
				return 0;
			}
		}
			...
			{
				{
					...
					col = lwcollection_construct(
					    COLLECTIONTYPE, topo->srid, NULL, nearbyindex, nearby);
	...
	{
		{
			...
			{
				{
					col = lwcollection_construct(MULTIPOINTTYPE,
								     topo->srid,
								     NULL,
...
_lwt_UpdateEdgeRingSideFace(LWT_TOPOLOGY *topo, LWT_EDGERING *ring, LWT_ELEMID face)
{
	...
	if (forward_edges_count)
	{
		...
		if (ret != forward_edges_count)
		{
			...
			lwfree(backward_edges);
			lwerror(
			    "Unexpected error: %d edges updated when expecting %d (forward)", ret, forward_edges_count);
			return -1;
		}
	}
...
_lwt_FindFaceContainingRing(LWT_TOPOLOGY *topo, LWT_EDGERING *ring, LWT_EDGERING_ARRAY *shells)
{
	...
	for (i = 0; i < candidates.size; ++i)
	{
		...
		{
			LWDEBUGF(
			    1, "Bbox of shell %d does not contain bbox of ring point", _lwt_EdgeRingGetFace(sring));
			continue;
		}
