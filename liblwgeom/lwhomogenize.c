...
lwcollection_homogenize(const LWCOLLECTION *col)
{
    ...
    if (ntypes == 0)
    {
        LWCOLLECTION *outcol;
        outcol =
            lwcollection_construct_empty(COLLECTIONTYPE, col->srid, FLAGS_GET_Z(col->flags), FLAGS_GET_M(col->flags));
    ...
    if (lwgeom_is_empty(geom))
    {
        if (lwgeom_is_collection(geom))
        {
            return lwcollection_as_lwgeom(
                lwcollection_construct_empty(geom->type, geom->srid, lwgeom_has_z(geom), lwgeom_has_m(geom)));
        }
