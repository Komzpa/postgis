...
LWGEOM2GEOS(const LWGEOM *lwgeom, uint8_t autofix)
{
    ...
    case LINETYPE:
        ...
        if (lwl->points->npoints == 1)
        {
            ...
            lwl->points = ptarray_addPoint(
                lwl->points, getPoint_internal(lwl->points, 0), FLAGS_NDIMS(lwl->points->flags), lwl->points->npoints);
        }
...
lwgeom_geos_noop(const LWGEOM *geom)
{
    ...
    if (!output_geos_as_lwgeom(&g, &result, srid, is3d, __func__)) return geos_clean_and_fail(g, NULL, NULL, __func__);
