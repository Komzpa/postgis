...
lw_dist3d_distanceline(const LWGEOM *lw1, const LWGEOM *lw2, int srid, int mode)
{
  ...
  if (!lwgeom_has_z(lw1) || !lwgeom_has_z(lw2))
  {
    ...
    lwnotice(
        "One or both of the geometries is missing z-value. The unknown z-value will be regarded as \"any "
        "value\"");

    if (!lwgeom_has_z(lw1) && !lwgeom_has_z(lw2)) return lw_dist2d_distanceline(lw1, lw2, srid, mode);
...
lw_dist3d_distancepoint(const LWGEOM *lw1, const LWGEOM *lw2, int srid, int mode)
{
  ...
  if (!lwgeom_has_z(lw1) || !lwgeom_has_z(lw2))
  {
    ...
    if (!lwgeom_has_z(lw1) && !lwgeom_has_z(lw2)) return lw_dist2d_distancepoint(lw1, lw2, srid, mode);
...
lwgeom_maxdistance3d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance)
{
  ...
  if (lw_dist3d_recursive(lw1, lw2, &thedl)) { return thedl.distance; }
  /*should never get here. all cases ought to be error handled earlier*/
  lwerror("Some unspecified error.");
  return -1;
}
...
lw_dist3d_point_poly(LWPOINT *point, LWPOLY *poly, DISTPTS3D *dl)
{
  ...
  if (!define_plane(poly->rings[0], &plane)) return LW_FALSE;
...
lw_dist3d_line_poly(LWLINE *line, LWPOLY *poly, DISTPTS3D *dl)
{
  ...
  if (dl->mode == DIST_MAX) { return lw_dist3d_ptarray_ptarray(line->points, poly->rings[0], dl); }
...
lw_dist3d_poly_poly(LWPOLY *poly1, LWPOLY *poly2, DISTPTS3D *dl)
{
  ...
  if (dl->mode == DIST_MAX) { return lw_dist3d_ptarray_ptarray(poly1->rings[0], poly2->rings[0], dl); }

  if (!define_plane(poly2->rings[0], &plane)) return LW_FALSE;
  ...
  dl->twisted = 1;
  if (!lw_dist3d_ptarray_poly(poly1->rings[0], poly2, &plane, dl)) return LW_FALSE;
  if (dl->distance == 0.0) /*Just check if the answer already is given*/
    return LW_TRUE;

  if (!define_plane(poly1->rings[0], &plane)) return LW_FALSE;
  dl->twisted = -1; /*because we swithc the order of geometries we swithch "twisted" to -1 which will give the
...
lw_dist3d_pt_seg(POINT3DZ *p, POINT3DZ *A, POINT3DZ *B, DISTPTS3D *dl)
{
  ...
  if ((A->x == B->x) && (A->y == B->y) && (A->z == B->z)) { return lw_dist3d_pt_pt(p, A, dl); }
...
lw_dist3d_seg_seg(POINT3DZ *s1p1, POINT3DZ *s1p2, POINT3DZ *s2p1, POINT3DZ *s2p2, DISTPTS3D *dl)
{
  ...
  */
  ...
  if (!get_3dvector_from_points(s1p1, s1p2, &v1)) return LW_FALSE;

  if (!get_3dvector_from_points(s2p1, s2p2, &v2)) return LW_FALSE;

  if (!get_3dvector_from_points(s2p1, s1p1, &vl)) return LW_FALSE;
  ...
  {            /* the lines are almost parallel*/
    s1k = 0.0; /*If the lines are paralell we try by using the startpoint of first segment. If that gives a
  ...
  { /*Find the closest point on the edges of both segments*/
    ...
    if (!lw_dist3d_pt_pt(&p1, &p2, dl)) /* Send the closest points to point-point calculation*/ { return LW_FALSE; }
  }
  return LW_TRUE;
}
...
lw_dist3d_pt_poly(POINT3DZ *p, LWPOLY *poly, PLANE3D *plane, POINT3DZ *projp, DISTPTS3D *dl)
{
  ...
  {
    return lw_dist3d_pt_ptarray(p, poly->rings[0], dl); /*If the projected point is outside the polygon we search
                                                           for the closest distance against the boundarry instead*/
  }
...
lw_dist3d_ptarray_poly(POINTARRAY *pa, LWPOLY *poly, PLANE3D *plane, DISTPTS3D *dl)
{
  ...
  s1 =
      project_point_on_plane(&p1, plane, &projp1); /*the sign of s1 tells us on which side of the plane the point is. */
...
define_plane(POINTARRAY *pa, PLANE3D *pl)
{
  ...
  for (j = pointsinslice; j < pa->npoints; j += pointsinslice)
  {
    ...
    if (!get_3dcross_product(&v1, &v2, &v)) return LW_FALSE;
    vl = VECTORLENGTH(v);
    sumx += (v.x / vl);
    sumy += (v.y / vl);
    sumz += (v.z / vl);
...
project_point_on_plane(POINT3DZ *p, PLANE3D *pl, POINT3DZ *p0)
{
  ...
  */
  ...
  if (!get_3dvector_from_points(&(pl->pop), p, &v1)) return LW_FALSE;
...
pt_in_ring_3d(const POINT3DZ *p, const POINTARRAY *ring, PLANE3D *plane)
{
  ...
  {
    lwerror("pt_in_ring_3d: V[n] != V[0] (%g %g %g!= %g %g %g)", first.x, first.y, first.z, last.x, last.y, last.z);
    return LW_FALSE;
  }
