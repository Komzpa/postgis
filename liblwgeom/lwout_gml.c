...
gbox_to_gml2(const GBOX *bbox, const char *srs, int precision, const char *prefix)
{
  ...
  if (srs)
    ptr += sprintf(ptr, "<%sBox srsName=\"%s\">", prefix, srs);
  else
    ptr += sprintf(ptr, "<%sBox>", prefix);
...
static size_t
asgml2_line_buf(const LWLINE *line, const char *srs, char *output, int precision, const char *prefix)
{
  char *ptr = output;
...
asgml2_poly_size(const LWPOLY *poly, const char *srs, int precision, const char *prefix)
{
  ...
  if (srs) size += strlen(srs) + sizeof(" srsName=..");
  if (lwpoly_is_empty(poly)) return size;
...
static size_t
asgml2_poly_buf(const LWPOLY *poly, const char *srs, char *output, int precision, const char *prefix)
{
  uint32_t i;
  char *ptr = output;
  ...
  ptr += sprintf(ptr, ">");
  ptr += sprintf(ptr, "<%souterBoundaryIs><%sLinearRing><%scoordinates>", prefix, prefix, prefix);
 ...
 */
static size_t
asgml2_multi_size(const LWCOLLECTION *col, const char *srs, int precision, const char *prefix)
{
  uint32_t i;
  size_t size;
  size_t prefixlen = strlen(prefix);
 ...
 */
static size_t
asgml2_multi_buf(const LWCOLLECTION *col, const char *srs, char *output, int precision, const char *prefix)
{
  int type = col->type;
  char *ptr, *gmltype;
  uint32_t i;
  ...
  if (type == MULTIPOINTTYPE)
    ...
    gmltype = "MultiLineString";
  else if (type == MULTIPOLYGONTYPE)
    gmltype = "MultiPolygon";
 ...
 */
static size_t
asgml2_collection_size(const LWCOLLECTION *col, const char *srs, int precision, const char *prefix)
{
  uint32_t i;
  size_t size;
  size_t prefixlen = strlen(prefix);
...
asgml2_collection_buf(const LWCOLLECTION *col, const char *srs, char *output, int precision, const char *prefix)
{
  ...
  for (i = 0; i < col->ngeoms; i++)
  {
    ...
    if (subgeom->type == POINTTYPE) { ptr += asgml2_point_buf((LWPOINT *)subgeom, 0, ptr, precision, prefix); }
    else if (subgeom->type == LINETYPE)
    {
      ptr += asgml2_line_buf((LWLINE *)subgeom, 0, ptr, precision, prefix);
    }
...
static size_t
asgml3_point_buf(const LWPOINT *point,
                 const char *srs,
                 char *output,
                 int precision,
                 int opts,
...
static size_t
asgml3_line_buf(const LWLINE *line,
                const char *srs,
                char *output,
                int precision,
                int opts,
...
static size_t
asgml3_circstring_size(const LWCIRCSTRING *circ,
                       const char *srs,
                       int precision,
                       int opts,
                       const char *prefix,
...
static size_t
asgml3_circstring_buf(const LWCIRCSTRING *circ,
                      const char *srs,
                      char *output,
                      int precision,
                      int opts,
                      ...
                      const char *id)
{
  ...
  if (FLAGS_GET_Z(circ->flags)) { dimension = 3; }
...
static size_t
asgml3_poly_buf(const LWPOLY *poly,
                const char *srs,
                char *output,
                int precision,
                int opts,
                ...
                const char *id)
{
  ...
  ptr += sprintf(ptr, "</%sposList></%sLinearRing></%sexterior>", prefix, prefix, prefix);
  for (i = 1; i < poly->nrings; i++)
  {
    ...
    ptr += sprintf(ptr, "</%sposList></%sLinearRing></%sinterior>", prefix, prefix, prefix);
  }
  if (is_patch)
...
static size_t
asgml3_compound_size(const LWCOMPOUND *col,
                     const char *srs,
                     int precision,
                     int opts,
                     const char *prefix,
...
static size_t
asgml3_compound_buf(const LWCOMPOUND *col,
                    const char *srs,
                    char *output,
                    int precision,
                    int opts,
                    ...
                    const char *id)
{
  ...
  {
    subgeom = col->geoms[i];
    if (subgeom->type != LINETYPE && subgeom->type != CIRCSTRINGTYPE) { continue; }
...
static size_t
asgml3_curvepoly_size(const LWCURVEPOLY *poly,
                      const char *srs,
                      int precision,
                      int opts,
                      const char *prefix,
...
static size_t
asgml3_curvepoly_buf(const LWCURVEPOLY *poly,
                     const char *srs,
                     char *output,
                     int precision,
                     int opts,
...
static size_t
asgml3_triangle_size(const LWTRIANGLE *triangle,
                     const char *srs,
                     int precision,
                     int opts,
                     const char *prefix,
...
static size_t
asgml3_triangle_buf(const LWTRIANGLE *triangle,
                    const char *srs,
                    char *output,
                    int precision,
                    int opts,
 ...
 */
static size_t
asgml3_multi_buf(const LWCOLLECTION *col,
                 const char *srs,
                 char *output,
                 int precision,
                 int opts,
...
static size_t
asgml3_psurface_size(const LWPSURFACE *psur,
                     const char *srs,
                     int precision,
                     int opts,
                     const char *prefix,
 ...
 */
static size_t
asgml3_psurface_buf(const LWPSURFACE *psur,
                    const char *srs,
                    char *output,
                    int precision,
                    int opts,
                    ...
                    const char *id)
{
  ...
  /* Close outmost tag */
  ptr += sprintf(ptr, "</%spolygonPatches></%sPolyhedralSurface>", prefix, prefix);
 ...
 */
static size_t
asgml3_tin_buf(const LWTIN *tin,
               const char *srs,
               char *output,
               int precision,
               int opts,
               ...
               const char *id)
{
  ...
  if (id)
    ...
    ptr += sprintf(ptr, "><%strianglePatches>", prefix);

  for (i = 0; i < tin->ngeoms; i++)
  {
    ptr += asgml3_triangle_buf(tin->geoms[i], 0, ptr, precision, opts, prefix, id);
  }
...
static size_t
asgml3_collection_size(const LWCOLLECTION *col,
                       const char *srs,
                       int precision,
                       int opts,
                       const char *prefix,
                       const char *id)
{
  ...
  for (i = 0; i < col->ngeoms; i++)
  {
    subgeom = col->geoms[i];
    size += (sizeof("<geometryMember>/") + prefixlen) * 2;
    if (subgeom->type == POINTTYPE) { size += asgml3_point_size((LWPOINT *)subgeom, 0, precision, opts, prefix, id); }
    else if (subgeom->type == LINETYPE)
    {
      size += asgml3_line_size((LWLINE *)subgeom, 0, precision, opts, prefix, id);
    }
...
static size_t
asgml3_collection_buf(const LWCOLLECTION *col,
                      const char *srs,
                      char *output,
                      int precision,
                      int opts,
...
static size_t
asgml3_multicurve_size(const LWMCURVE *cur,
                       const char *srs,
                       int precision,
                       int opts,
                       const char *prefix,
                       const char *id)
{
  ...
  for (i = 0; i < cur->ngeoms; ++i)
  {
    size += sizeof("<curveMember></curveMember>") + 2 * prefixlen;
    subgeom = cur->geoms[i];
    if (subgeom->type == LINETYPE) { size += asgml3_line_size((LWLINE *)subgeom, srs, precision, opts, prefix, id); }
    else if (subgeom->type == CIRCSTRINGTYPE)
    {
      size += asgml3_circstring_size((LWCIRCSTRING *)subgeom, srs, precision, opts, prefix, id);
    }
...
static size_t
asgml3_multicurve_buf(const LWMCURVE *cur,
                      const char *srs,
                      char *output,
                      int precision,
                      int opts,
                      ...
                      const char *id)
{
  ...
  for (i = 0; i < cur->ngeoms; ++i)
  {
    ptr += sprintf(ptr, "<%scurveMember>", prefix);
    subgeom = cur->geoms[i];
    if (subgeom->type == LINETYPE) { ptr += asgml3_line_buf((LWLINE *)subgeom, srs, ptr, precision, opts, prefix, id); }
    else if (subgeom->type == CIRCSTRINGTYPE)
    {
      ptr += asgml3_circstring_buf((LWCIRCSTRING *)subgeom, srs, ptr, precision, opts, prefix, id);
    }
...
static size_t
asgml3_multisurface_size(const LWMSURFACE *sur,
                         const char *srs,
                         int precision,
                         int opts,
                         const char *prefix,
...
static size_t
asgml3_multisurface_buf(const LWMSURFACE *sur,
                        const char *srs,
                        char *output,
                        int precision,
                        int opts,
...
{
  if (FLAGS_NDIMS(pa->flags) == 2) return (OUT_MAX_DIGS_DOUBLE + precision + sizeof(", ")) * 2 * pa->npoints;
