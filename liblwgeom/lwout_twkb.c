                    ...
                    uint32_t minpoints)
{
  ...
  for (i = 0; i < pa->npoints; i++)
  {
    ...
    if (i > minpoints && diff == 0) continue;
    ...
    if (globals->variant & TWKB_BBOX)
    {
      for (j = 0; j < ndims; j++)
      {
        if (ts->accum_rels[j] > ts->bbox_max[j]) ts->bbox_max[j] = ts->accum_rels[j];

        if (ts->accum_rels[j] < ts->bbox_min[j]) ts->bbox_min[j] = ts->accum_rels[j];
      }
    }
  }
  ...
  if (pa->npoints > 127)
  {
    ...
    /* First the npoints */
    if (register_npoints) bytebuffer_append_uvarint(ts->geom_buf, npoints);
    /* Now the coordinates */
...
lwmulti_to_twkb_buf(const LWCOLLECTION *col, TWKB_GLOBALS *globals, TWKB_STATE *ts)
{
  ...
  if (col->type == MULTIPOINTTYPE)
  {
    for (i = 0; i < col->ngeoms; i++)
      if (lwgeom_is_empty(col->geoms[i])) nempty++;
  }
  ...
  if (ts->idlist)
  {
    for (i = 0; i < col->ngeoms; i++)
    {
      ...
      if (col->type == MULTIPOINTTYPE && lwgeom_is_empty(col->geoms[i])) continue;
...
lwgeom_write_to_buffer(const LWGEOM *geom, TWKB_GLOBALS *globals, TWKB_STATE *parent_state)
{
  ...
  if (has_z) globals->factor[2] = pow(10, globals->prec_z);
  if (has_m) globals->factor[2 + has_z] = pow(10, globals->prec_m);
  ...
  /* TYPE/PRECISION BYTE */
  if (abs(globals->prec_xy) > 7) lwerror("%s: X/Z precision cannot be greater than 7 or less than -7", __func__);
  ...
  if (is_empty)
  {
    ...
    /* there is none */
    if (globals->variant & TWKB_SIZE) bytebuffer_append_byte(child_state.header_buf, 0);
  ...
  if ((globals->variant & TWKB_BBOX) && parent_state->header_buf)
  {
    ...
    for (i = 0; i < ndims; i++)
    {
      if (child_state.bbox_min[i] < parent_state->bbox_min[i]) parent_state->bbox_min[i] = child_state.bbox_min[i];
      if (child_state.bbox_max[i] > parent_state->bbox_max[i]) parent_state->bbox_max[i] = child_state.bbox_max[i];
    }
  }
  ...
  if (globals->variant & TWKB_BBOX) write_bbox(&child_state, ndims);
