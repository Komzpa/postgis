...
lwgeom_maxdistance2d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance)
{
  ...
  if (lw_dist2d_comp(lw1, lw2, &thedl)) { return thedl.distance; }
  /*should never get here. all cases ought to be error handled earlier*/
  lwerror("Some unspecified error.");
  return -1;
}
...
lw_dist2d_check_overlap(LWGEOM *lwg1, LWGEOM *lwg2)
{
  ...
  if (!lwg1->bbox) lwgeom_calculate_gbox(lwg1, lwg1->bbox);
  if (!lwg2->bbox) lwgeom_calculate_gbox(lwg2, lwg2->bbox);
...
and not in holes so we check wich one is inside.
 */
int
lw_dist2d_poly_poly(LWPOLY *poly1, LWPOLY *poly2, DISTPTS *dl)
{
  ...
  /*1	if we are looking for maxdistance, just check the outer rings.*/
  if (dl->mode == DIST_MAX) { return lw_dist2d_ptarray_ptarray(poly1->rings[0], poly2->rings[0], dl); }
...
lw_dist2d_curvepoly_curvepoly(LWCURVEPOLY *poly1, LWCURVEPOLY *poly2, DISTPTS *dl)
{
  ...
  /*1	if we are looking for maxdistance, just check the outer rings.*/
  if (dl->mode == DIST_MAX) { return lw_dist2d_recursive(poly1->rings[0], poly2->rings[0], dl); }
...
lw_dist2d_pt_ptarrayarc(const POINT2D *p, const POINTARRAY *pa, DISTPTS *dl)
{
  ...
  for (t = 1; t < pa->npoints; t += 2)
  {
    ...
    if (lw_dist2d_pt_arc(p, A1, A2, A3, dl) == LW_FALSE) return LW_FALSE;
                  ...
                  DISTPTS *dl)
{
  ...
  if (dl->mode < 0) lwerror("lw_dist2d_seg_arc does not support maxdistance mode");

  /* What if the "arc" is a point? */
  if (lw_arc_is_pt(B1, B2, B3)) return lw_dist2d_pt_seg(B1, A1, A2, dl);
  ...
  if (radius_C < 0.0) return lw_dist2d_seg_seg(A1, A2, B1, B3, dl);

  /* Calculate distance between the line and circle center */
  lw_dist2d_distpts_init(&dltmp, DIST_MIN);
  if (lw_dist2d_pt_seg(&C, A1, A2, &dltmp) == LW_FALSE) lwerror("lw_dist2d_pt_seg failed in lw_dist2d_seg_arc");
  ...
  else
  {
    ...
    if (pt_in_arc && pt_in_seg) return lw_dist2d_pt_pt(&D, &G, dl);
  }
...
lw_dist2d_pt_arc(const POINT2D *P, const POINT2D *A1, const POINT2D *A2, const POINT2D *A3, DISTPTS *dl)
{
  ...
  if (dl->mode < 0) lwerror("lw_dist2d_pt_arc does not support maxdistance mode");

  /* What if the arc is a point? */
  if (lw_arc_is_pt(A1, A2, A3)) return lw_dist2d_pt_pt(P, A1, dl);
  ...
  if (radius_A < 0.0) return lw_dist2d_pt_seg(P, A1, A3, dl);
                  ...
                  DISTPTS *dl)
{
  ...
  if (dl->mode != DIST_MIN) lwerror("lw_dist2d_arc_arc only supports mindistance");
  ...
  if (radius_A < 0 && radius_B < 0) return lw_dist2d_seg_seg(A1, A3, B1, B3, dl);

  /* A is co-linear, delegate to lw_dist_seg_arc here. */
  if (radius_A < 0) return lw_dist2d_seg_arc(A1, A3, B1, B2, B3, dl);

  /* B is co-linear, delegate to lw_dist_seg_arc here. */
  if (radius_B < 0) return lw_dist2d_seg_arc(B1, B3, A1, A2, A3, dl);
  ...
  else if (d > (radius_A + radius_B) /* Disjoint */ || d < (radius_A - radius_B) /* Contained */)
  {
    ...
    if (pt_in_arc_A && pt_in_arc_B) { return lw_dist2d_pt_pt(&XA, &XB, dl); }
  }
...
lw_dist2d_seg_seg(const POINT2D *A, const POINT2D *B, const POINT2D *C, const POINT2D *D, DISTPTS *dl)
{
  ...
  LWDEBUGF(2, "lw_dist2d_seg_seg [%g,%g]->[%g,%g] by [%g,%g]->[%g,%g]", A->x, A->y, B->x, B->y, C->x, C->y, D->x, D->y);
...
lw_dist2d_fast_ptarray_ptarray(POINTARRAY *l1, POINTARRAY *l2, DISTPTS *dl, GBOX *box1, GBOX *box2)
{
  ...
  /*if the center-center line is horizontal the perpendicular line will be vertical. To eliminate problems with
   deviding by zero we are here mirroring the coordinate-system and we find it's crossing the X-axes with z =
   x-(1/k)y */
  else /*West or East*/
  {
    k = -deltaY / deltaX;
...
lw_dist2d_pre_seg_seg(POINTARRAY *l1, POINTARRAY *l2, LISTSTRUCT *list1, LISTSTRUCT *list2, double k, DISTPTS *dl)
{
  ...
  for (i = (n1 - 1); i >= 0; --i)
  {
    ...
    for (r = -1; r <= 1; r += 2) /*because we are not iterating in the original pointorder we have to check
                                    the segment before and after every point*/
    {
      pnr1 = list1[i].pnr;
      ...
      for (u = 0; u < n2; ++u)
      {
        ...
        maxmeasure = sqrt(dl->distance * dl->distance +
                          (dl->distance * dl->distance * k * k)); /*here we "translate" the found mindistance
                                                                     so it can be compared to our "z"-values*/
      }
    }
  }
...
lw_dist2d_pt_seg(const POINT2D *p, const POINT2D *A, const POINT2D *B, DISTPTS *dl)
{
  ...
  if ((A->x == B->x) && (A->y == B->y)) { return lw_dist2d_pt_pt(p, A, dl); }
  /*
  ...
  if (r >= 1) /*If p projected on the line is outside point B or on point B*/ { return lw_dist2d_pt_pt(p, B, dl); }
...
distance2d_pt_seg(const POINT2D *p, const POINT2D *A, const POINT2D *B)
{
  ...
  /*if start==end, then use pt distance */
  if ((A->x == B->x) && (A->y == B->y)) return distance2d_pt_pt(p, A);
...
distance2d_sqr_pt_seg(const POINT2D *p, const POINT2D *A, const POINT2D *B)
{
  ...
  if ((A->x == B->x) && (A->y == B->y)) return distance2d_sqr_pt_pt(p, A);
