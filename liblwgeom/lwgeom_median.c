    ...
    if (!lwgeom_is_empty(subg))
    {
      *input_empty = LW_FALSE;
      ...
      if (has_m)
      {
        ...
        {
          lwerror(
              "Geometric median input contains points with negative weights (POINT(%g %g "
              "%g %g), number %d of %d in input). Implementation can't guarantee global "
              "minimum convergence.",
              points[n].x,
              points[n].y,
              points[n].z,
