...
ptarray_append_point(POINTARRAY *pa, const POINT4D *pt, int repeated_points)
{
  ...
  if (repeated_points == LW_FALSE && pa->npoints > 0)
  {
    ...
    LWDEBUGF(4,
             "checking for duplicate end point (pt = POINT(%g %g) pa->npoints-q = POINT(%g %g))",
             pt->x,
             pt->y,
             tmp.x,
             tmp.y);
 ...
 */
POINTARRAY *
ptarray_construct_reference_data(char hasz, char hasm, uint32_t npoints, uint8_t *ptlist)
{
  POINTARRAY *pa = lwalloc(sizeof(POINTARRAY));
...
ptarray_free(POINTARRAY *pa)
{
  if (pa)
  {
    if (pa->serialized_pointlist && (!FLAGS_GET_READONLY(pa->flags))) lwfree(pa->serialized_pointlist);
    lwfree(pa);
...
ptarray_segmentize2d(const POINTARRAY *ipa, double dist)
{
  ...
  while (ipoff < ipa->npoints)
  {
    ...
    if (segdist > dist) /* add an intermediate point */
    {
      ...
      if (hasz) pbuf.z = p1.z + (p2.z - p1.z) / segdist * dist;
...
ptarray_same(const POINTARRAY *pa1, const POINTARRAY *pa2)
{
  ...
  {
    if (memcmp(getPoint_internal(pa1, i), getPoint_internal(pa2, i), ptsize)) return LW_FALSE;
...
ptarray_addPoint(const POINTARRAY *pa, uint8_t *p, size_t pdims, uint32_t where)
{
  ...
  LWDEBUGF(3, "pa %x p %x size %d where %d", pa, p, pdims, where);
  ...
  if (where) { memcpy(getPoint_internal(ret, 0), getPoint_internal(pa, 0), ptsize * where); }
...
ptarray_removePoint(POINTARRAY *pa, uint32_t which)
{
  ...
  {
    lwerror("%s [%d] offset (%d) out of range (%d..%d)", __FILE__, __LINE__, which, 0, pa->npoints - 1);
    return NULL;
  }
  ...
  /* copy initial part */
  if (which) { memcpy(getPoint_internal(ret, 0), getPoint_internal(pa, 0), ptsize * which); }

  /* copy final part */
  if (which < pa->npoints - 1)
  { memcpy(getPoint_internal(ret, which), getPoint_internal(pa, which + 1), ptsize * (pa->npoints - which - 1)); }
...
ptarray_merge(POINTARRAY *pa1, POINTARRAY *pa2)
{
  ...
  if (FLAGS_GET_ZM(pa1->flags) != FLAGS_GET_ZM(pa2->flags)) lwerror("ptarray_cat: Mixed dimension");

  pa = ptarray_construct(FLAGS_GET_Z(pa1->flags), FLAGS_GET_M(pa1->flags), pa1->npoints + pa2->npoints);

  memcpy(getPoint_internal(pa, 0), getPoint_internal(pa1, 0), ptsize * (pa1->npoints));

  memcpy(getPoint_internal(pa, pa1->npoints), getPoint_internal(pa2, 0), ptsize * (pa2->npoints));
...
ptarray_contains_point_partial(const POINTARRAY *pa, const POINT2D *pt, int check_closed, int *winding_number)
{
  ...
  if (check_closed && !p2d_same(seg1, seg2)) lwerror("ptarray_contains_point called on unclosed ring");
  ...
  for (i = 1; i < pa->npoints; i++)
  {
     ...
     */
    if ((side == 0) && lw_pt_in_seg(pt, seg1, seg2)) { return LW_BOUNDARY; }
  ...
  if (winding_number) *winding_number = wn;

  /* Outside */
  if (wn == 0) { return LW_OUTSIDE; }
...
ptarrayarc_contains_point_partial(const POINTARRAY *pa, const POINT2D *pt, int check_closed, int *winding_number)
{
  ...
  else if (p2d_same(seg1, seg3) && pa->npoints == 3)
  {
    ...
    if (lw_arc_is_pt(seg1, seg2, seg3)) return LW_OUTSIDE;
  ...
  for (i = 1; i < pa->npoints; i += 2)
  {
    ...
    if (p2d_same(seg3, pt)) return LW_BOUNDARY;
    ...
    /* On the boundary */
    if ((side == 0) && lw_pt_in_arc(pt, seg1, seg2, seg3)) { return LW_BOUNDARY; }

    /* Going "up"! Point to left of arc. */
    if (side < 0 && (seg1->y <= pt->y) && (pt->y < seg3->y)) { wn++; }

    /* Going "down"! */
    if (side > 0 && (seg2->y <= pt->y) && (pt->y < seg1->y)) { wn--; }
    ...
    if (pt->x <= gbox.xmax && pt->x >= gbox.xmin)
    {
      ...
      /* On the boundary! */
      if (d == radius) return LW_BOUNDARY;
...
ptarray_force_dims(const POINTARRAY *pa, int hasz, int hasm)
{
  ...
  for (i = 0; i < pa->npoints; i++)
  {
    ...
    if (hasz && !in_hasz) pt.z = 0.0;
    if (hasm && !in_hasm) pt.m = 0.0;
...
ptarray_substring(POINTARRAY *ipa, double from, double to, double tolerance)
{
  ...
  for (i = 0; i < nsegs; i++)
  {
    ...
    LWDEBUGF(3, "Segment %d: (%g,%g,%g,%g)-(%g,%g,%g,%g)", i, p1.x, p1.y, p1.z, p1.m, p2.x, p2.y, p2.z, p2.m);
...
ptarray_locate_point(const POINTARRAY *pa, const POINT4D *p4d, double *mindistout, POINT4D *proj4d)
{
  ...
  if (pa->npoints <= 1)
  {
    if (pa->npoints == 1)
    {
      ...
      if (mindistout) *mindistout = distance2d_pt_pt(&p, getPoint2d_cp(pa, 0));
    }
    return 0.0;
  }
  ...
  if ((seg >= (pa->npoints - 2)) && p2d_same(&proj, end)) { return 1.0; }
...
ptarray_remove_repeated_points_in_place(POINTARRAY *pa, double tolerance, uint32_t min_points)
{
  ...
  for (i = 1; i < n_points; i++)
  {
    ...
    if (n_points + n_points_out > min_points + i)
    {
      ...
      if (last_point && n_points_out > 1 && tolerance > 0.0 && dsq <= tolsq) { n_points_out--; }
    }
...
ptarray_dp_findsplit_in_place(const POINTARRAY *pts, int p1, int p2, int *split, double *dist)
{
  ...
  *split = p1;
  ...
  if (p1 + 1 < p2)
  {
    ...
    LWDEBUGF(4, "P%d(%f,%f) to P%d(%f,%f)", p1, pa->x, pa->y, p2, pb->x, pb->y);
...
ptarray_simplify_in_place(POINTARRAY *pa, double epsilon, uint32_t minpts)
{
  ...
  if (stack != stack_static) lwfree(stack);
  if (outlist != outlist_static) lwfree(outlist);
...
ptarray_arc_length_2d(const POINTARRAY *pts)
{
  ...
  if (pts->npoints % 2 != 1) lwerror("arc point array with even number of points");
...
ptarray_length(const POINTARRAY *pts)
{
  ...
  for (i = 1; i < pts->npoints; i++)
  {
    ...
    dist +=
        sqrt(((frm.x - to.x) * (frm.x - to.x)) + ((frm.y - to.y) * (frm.y - to.y)) + ((frm.z - to.z) * (frm.z - to.z)));
    frm = to;
  }
  return dist;
}
...
getPoint_internal(const POINTARRAY *pa, uint32_t n)
{
  ...
  if ((n > pa->npoints) || (n >= pa->maxpoints))
  {
    lwerror("%s [%d] called outside of ptarray range (n=%d, pa.npoints=%d, pa.maxpoints=%d)",
            __FILE__,
            __LINE__,
            n,
            pa->npoints,
...
ptarray_grid_in_place(POINTARRAY *pa, const gridspec *grid)
{
  ...
  for (i = 0; i < pa->npoints; i++)
  {
    ...
    if (grid->xsize > 0) { p->x = rint((p->x - grid->ipx) / grid->xsize) * grid->xsize + grid->ipx; }

    if (grid->ysize > 0) { p->y = rint((p->y - grid->ipy) / grid->ysize) * grid->ysize + grid->ipy; }

    /* Read and round this point */
    /* Z is always in third position */
    if (has_z)
    {
      if (grid->zsize > 0) p->z = rint((p->z - grid->ipz) / grid->zsize) * grid->zsize + grid->ipz;
    }
    /* M might be in 3rd or 4th position */
    if (has_m)
    {
      /* In POINT M, M is in 3rd position */
      if (grid->msize > 0 && !has_z) p->z = rint((p->z - grid->ipm) / grid->msize) * grid->msize + grid->ipm;
      /* In POINT ZM, M is in 4th position */
      if (grid->msize > 0 && has_z) p->m = rint((p->m - grid->ipm) / grid->msize) * grid->msize + grid->ipm;
    }

    /* Skip duplicates */
    if (p_out && FP_EQUALS(p_out->x, p->x) && FP_EQUALS(p_out->y, p->y) &&
        (ndims > 2 ? FP_EQUALS(p_out->z, p->z) : 1) && (ndims > 3 ? FP_EQUALS(p_out->m, p->m) : 1))
    { continue; }
    ...
    p_out->x = p->x;
    p_out->y = p->y;
    if (ndims > 2) p_out->z = p->z;
    if (ndims > 3) p_out->m = p->m;
  }
...
ptarray_npoints_in_rect(const POINTARRAY *pa, const GBOX *gbox)
{
  ...
  {
    pt = getPoint2d_cp(pa, i);
    if (gbox_contains_point2d(gbox, pt)) n++;
  }
  return n;
}
