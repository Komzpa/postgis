...
#define WORK_SIZE 1024
  ...
  for (index = 0; index < format_length; index++)
  {
    ...
    case 'D':
      ...
      else
      {
        ...
        if (deg_digits > 0) { lwerror("Bad format, cannot include degrees (DD.DDD) more than once."); }
        reading_deg = 1;
        reading_min = 0;
        reading_sec = 0;
        deg_digits++;
    ...
    case 'M':
      ...
      else
      {
        ...
        if (min_digits > 0) { lwerror("Bad format, cannot include minutes (MM.MMM) more than once."); }
        reading_deg = 0;
        reading_min = 1;
        reading_sec = 0;
        min_digits++;
    ...
    case 'S':
      ...
      else
      {
        ...
        if (sec_digits > 0) { lwerror("Bad format, cannot include seconds (SS.SSS) more than once."); }
        reading_deg = 0;
        reading_min = 0;
        reading_sec = 1;
        sec_digits++;
    ...
    case 'C':
      ...
      if (compass_dir_piece >= 0) { lwerror("Bad format, cannot include compass dir (C) more than once."); }
    ...
    default:
      ...
      if (next_char & 0x80)
      {
        ...
        {
          lwerror(
              "Bad format, invalid high-order byte found first, format string may not be "
              "UTF-8.");
        }
      }
      if (multibyte_char_width > 1)
      {
        ...
        {
          lwerror(
              "Bad format, UTF-8 character first byte found with insufficient following "
              "bytes, format string may not be UTF-8.");
        }
        for (following_byte_index = (index + 1); following_byte_index < (index + multibyte_char_width);
             following_byte_index++)
        {
          if ((format[following_byte_index] & 0xC0) != 0x80)
          {
            lwerror(
                "Bad format, invalid byte found following leading byte of "
                "multibyte character, format string may not be UTF-8.");
          }
        }
      }
    ...
    if (current_piece >= NUM_PIECES) { lwerror("Internal error, somehow needed more pieces than it should."); }
  }
  ...
  if (sec_digits > 0)
  {
    if (0 == min_digits) { lwerror("Bad format, cannot include seconds (SS.SSS) without including minutes (MM.MMM)."); }
  ...
  if (compass_dir_piece >= 0) { strcpy(pieces[compass_dir_piece], is_negative ? neg_dir_symbol : pos_dir_symbol); }
  else if (is_negative)
  {
    degrees *= -1;
  }
