 ...
 */
double
longitude_radians_normalize(double lon)
{
    if (lon == -1.0 * M_PI) return M_PI;
    if (lon == -2.0 * M_PI) return 0.0;

    if (lon > 2.0 * M_PI) lon = remainder(lon, 2.0 * M_PI);

    if (lon < -2.0 * M_PI) lon = remainder(lon, -2.0 * M_PI);

    if (lon > M_PI) lon = -2.0 * M_PI + lon;

    if (lon < -1.0 * M_PI) lon = 2.0 * M_PI + lon;

    if (lon == -2.0 * M_PI) lon *= -1.0;
 ...
 */
double
latitude_radians_normalize(double lat)
{

    if (lat > 2.0 * M_PI) lat = remainder(lat, 2.0 * M_PI);

    if (lat < -2.0 * M_PI) lat = remainder(lat, -2.0 * M_PI);

    if (lat > M_PI) lat = M_PI - lat;

    if (lat < -1.0 * M_PI) lat = -1.0 * M_PI - lat;

    if (lat > M_PI_2) lat = M_PI - lat;

    if (lat < -1.0 * M_PI_2) lat = -1.0 * M_PI - lat;
 ...
 */
double
longitude_degrees_normalize(double lon)
{
    if (lon > 360.0) lon = remainder(lon, 360.0);

    if (lon < -360.0) lon = remainder(lon, -360.0);

    if (lon > 180.0) lon = -360.0 + lon;

    if (lon < -180.0) lon = 360 + lon;

    if (lon == -180.0) return 180.0;

    if (lon == -360.0) return 0.0;
 ...
 */
double
latitude_degrees_normalize(double lat)
{

    if (lat > 360.0) lat = remainder(lat, 360.0);

    if (lat < -360.0) lat = remainder(lat, -360.0);

    if (lat > 180.0) lat = 180.0 - lat;
 ...
 */
void
point_shift(GEOGRAPHIC_POINT *p, double shift)
{
...
int
geographic_point_equals(const GEOGRAPHIC_POINT *g1, const GEOGRAPHIC_POINT *g2)
{
 ...
 */
void
geographic_point_init(double lon, double lat, GEOGRAPHIC_POINT *g)
{
 ...
 */
void
geog2cart(const GEOGRAPHIC_POINT *g, POINT3D *p)
{
 ...
 */
void
cart2geog(const POINT3D *p, GEOGRAPHIC_POINT *g)
{
    g->lon = atan2(p->y, p->x);
    g->lat = asin(p->z);
}
 ...
 */
void
ll2cart(const POINT2D *g, POINT3D *p)
{
 ...
 */
static double
dot_product(const POINT3D *p1, const POINT3D *p2)
{
 ...
 */
static void
cross_product(const POINT3D *a, const POINT3D *b, POINT3D *n)
{
    n->x = a->y * b->z - a->z * b->y;
    n->y = a->z * b->x - a->x * b->z;
    n->z = a->x * b->y - a->y * b->x;
 ...
 */
void
vector_sum(const POINT3D *a, const POINT3D *b, POINT3D *n)
{
    n->x = a->x + b->x;
    n->y = a->y + b->y;
    n->z = a->z + b->z;
 ...
 */
static void
vector_difference(const POINT3D *a, const POINT3D *b, POINT3D *n)
{
    n->x = a->x - b->x;
    n->y = a->y - b->y;
    n->z = a->z - b->z;
 ...
 */
static void
vector_scale(POINT3D *n, double scale)
{
    n->x *= scale;
    n->y *= scale;
    n->z *= scale;
 ...
 */
double
vector_angle(const POINT3D *v1, const POINT3D *v2)
{
    POINT3D v3, normal;
    double angle, x, y;
 ...
 */
void
unit_normal(const POINT3D *P1, const POINT3D *P2, POINT3D *normal)
{
 ...
 */
void
vector_rotate(const POINT3D *v1, const POINT3D *v2, double angle, POINT3D *n)
{
    POINT3D u;
 ...
 */
void
robust_cross_product(const GEOGRAPHIC_POINT *p, const GEOGRAPHIC_POINT *q, POINT3D *a)
{
...
int
crosses_dateline(const GEOGRAPHIC_POINT *s, const GEOGRAPHIC_POINT *e)
{
 ...
 */
int
edge_point_on_plane(const GEOGRAPHIC_EDGE *e, const GEOGRAPHIC_POINT *p)
{
    int side = edge_point_side(e, p);
    if (side == 0) return LW_TRUE;
 ...
 */
int
edge_point_in_cone(const GEOGRAPHIC_EDGE *e, const GEOGRAPHIC_POINT *p)
{
    ...
    if (vs.x == -1.0 * ve.x && vs.y == -1.0 * ve.y && vs.z == -1.0 * ve.z) return LW_TRUE;
    geog2cart(p, &vp);
 ...
 */
int
edge_contains_coplanar_point(const GEOGRAPHIC_EDGE *e, const GEOGRAPHIC_POINT *p)
{
    GEOGRAPHIC_EDGE g;
    GEOGRAPHIC_POINT q;
    ...
    if (FP_EQUALS(g.start.lon, g.end.lon))
    {
        ...
        if (!FP_EQUALS(q.lon, g.start.lon)) return LW_FALSE;
    ...
    if (FP_EQUALS(slon, M_PI) && (SIGNUM(g.start.lon) != SIGNUM(g.end.lon) || FP_EQUALS(dlon, M_PI)))
    {
        ...
        if (FP_EQUALS(slat, 0.0)) return LW_TRUE;

        /* Point *is* the north pole */
        if (slat > 0.0 && FP_EQUALS(q.lat, M_PI_2)) return LW_TRUE;

        /* Point *is* the south pole */
        if (slat < 0.0 && FP_EQUALS(q.lat, -1.0 * M_PI_2)) return LW_TRUE;
    ...
    if ((g.start.lon <= q.lon && q.lon <= g.end.lon) || (g.end.lon <= q.lon && q.lon <= g.start.lon))
    {
 ...
 */
double
sphere_distance(const GEOGRAPHIC_POINT *s, const GEOGRAPHIC_POINT *e)
{
 ...
 */
double
sphere_distance_cartesian(const POINT3D *s, const POINT3D *e)
{
 ...
 */
double
sphere_direction(const GEOGRAPHIC_POINT *s, const GEOGRAPHIC_POINT *e, double d)
{
    double heading = 0.0;
    double f;

    /* Starting from the poles? Special case. */
    if (FP_IS_ZERO(cos(s->lat))) return (s->lat > 0.0) ? M_PI : 0.0;
 ...
 */
int
edge_contains_point(const GEOGRAPHIC_EDGE *e, const GEOGRAPHIC_POINT *p)
{
 ...
 */
double
z_to_latitude(double z, int top)
{
 ...
 */
int
clairaut_cartesian(const POINT3D *start, const POINT3D *end, GEOGRAPHIC_POINT *g_top, GEOGRAPHIC_POINT *g_bottom)
{
    POINT3D t1, t2;
 ...
 */
int
clairaut_geographic(const GEOGRAPHIC_POINT *start,
 ...
 */
int
edge_intersection(const GEOGRAPHIC_EDGE *e1, const GEOGRAPHIC_EDGE *e2, GEOGRAPHIC_POINT *g)
{
    POINT3D ea, eb, v;
    ...
    LWDEBUGF(4,
             "e1 start(%.20g %.20g) end(%.20g %.20g)",
             rad2deg(e1->start.lon),
             rad2deg(e1->start.lat),
             rad2deg(e1->end.lon),
             rad2deg(e1->end.lat));
    LWDEBUGF(4,
             "e2 start(%.20g %.20g) end(%.20g %.20g)",
             rad2deg(e2->start.lon),
             rad2deg(e2->start.lat),
             rad2deg(e2->end.lon),
             rad2deg(e2->end.lat));
    ...
    else
    {
        ...
        g->lat = -1.0 * g->lat;
        g->lon = g->lon + M_PI;
        if (g->lon > M_PI) { g->lon = -1.0 * (2.0 * M_PI - g->lon); }
        if (edge_contains_point(e1, g) && edge_contains_point(e2, g)) { return LW_TRUE; }
    }
    return LW_FALSE;
}
...
double
edge_distance_to_point(const GEOGRAPHIC_EDGE *e, const GEOGRAPHIC_POINT *gp, GEOGRAPHIC_POINT *closest)
{
    ...
    normalize(&k);
    cart2geog(&k, &gk);
    if (edge_contains_point(e, &gk)) { d1 = sphere_distance(gp, &gk); }
 ...
 */
double
edge_distance_to_edge(const GEOGRAPHIC_EDGE *e1,
 ...
 */
int
sphere_project(const GEOGRAPHIC_POINT *r, double distance, double azimuth, GEOGRAPHIC_POINT *n)
{
    double d = distance;
    double lat1 = r->lat;
    double lon1 = r->lon;
    ...
    if (isnan(lat2) || isnan(lon2)) return LW_FAILURE;
...
int
edge_calculate_gbox_slow(const GEOGRAPHIC_EDGE *e, GBOX *gbox)
{
    int steps = 1000000;
    int i;
    double dx, dy, dz;
 ...
 */
int
edge_calculate_gbox(const POINT3D *A1, const POINT3D *A2, GBOX *gbox)
{
    POINT2D R1, R2, RX, O;
    POINT3D AN, A3;
    POINT3D X[6];
    ...
    if (p3d_same(A1, A2)) return LW_SUCCESS;
...
void
lwpoly_pt_outside(const LWPOLY *poly, POINT2D *pt_outside)
{
 ...
 */
void
gbox_pt_outside(const GBOX *gbox, POINT2D *pt_outside)
{
...
static int
ptarray_segmentize_sphere_edge_recursive(const POINT3D *p1,
                                         const POINT3D *p2, /* 3-space points we are interpolating beween */
                                         const POINT4D *v1,
                                         const POINT4D *v2, /* real values and z/m values */
                                         double d,
                                         double max_seg_length, /* current segment length and segment limit */
...
ptarray_segmentize_sphere(const POINTARRAY *pa_in, double max_seg_length)
{
    ...
    if (!pa_in) lwerror("%s: null input pointarray", __func__);
    if (max_seg_length <= 0.0) lwerror("%s: maximum segment length must be positive", __func__);
    ...
    for (i = 1; i < pa_in->npoints; i++)
    {
        ...
        if ((pa_in->npoints > 2) && p4d_same(&p1, &p2)) continue;
...
lwgeom_segmentize_sphere(const LWGEOM *lwg_in, double max_seg_length)
{
    ...
    /* Reflect NULL */
    if (!lwg_in) return NULL;

    /* Clone empty */
    if (lwgeom_is_empty(lwg_in)) return lwgeom_clone(lwg_in);
    ...
    case COLLECTIONTYPE:
        ...
        lwcol_out =
            lwcollection_construct_empty(lwg_in->type, lwg_in->srid, lwgeom_has_z(lwg_in), lwgeom_has_m(lwg_in));
...
static double
ptarray_distance_spheroid(const POINTARRAY *pa1,
                          ...
                          int check_intersection)
{
    ...
    if (pa1->npoints == 0 || pa2->npoints == 0) return -1.0;
    ...
    if (pa1->npoints == 1 || pa2->npoints == 1)
    {
        ...
        for (i = 1; i < pa_many->npoints; i++)
        {
            ...
            if (d < tolerance)
            {
                ...
                /* On a spheroid and near the tolerance? Confirm that we are *actually* closer than
                 * tolerance */
                else
                {
 ...
 */
double
lwgeom_area_sphere(const LWGEOM *lwgeom, const SPHEROID *spheroid)
{
    int type;
    ...
    /* No area in nothing */
    if (lwgeom_is_empty(lwgeom)) return 0.0;

    /* Read the geometry type number */
    type = lwgeom->type;

    /* Anything but polygons and collections returns zero */
    if (!(type == POLYGONTYPE || type == MULTIPOLYGONTYPE || type == COLLECTIONTYPE)) return 0.0;
 ...
 */
LWPOINT *
lwgeom_project_spheroid(const LWPOINT *r, const SPHEROID *spheroid, double distance, double azimuth)
{
 ...
 */
double
lwgeom_azumith_spheroid(const LWPOINT *r, const LWPOINT *s, const SPHEROID *spheroid)
{
    GEOGRAPHIC_POINT g1, g2;
    double x1, y1, x2, y2;
    ...
    if (FP_EQUALS(x1, x2) && FP_EQUALS(y1, y2)) { return NAN; }
 ...
 */
double
lwgeom_distance_spheroid(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2, const SPHEROID *spheroid, double tolerance)
{
    uint8_t type1, type2;
    ...
    if (lwgeom_is_empty(lwgeom1) || lwgeom_is_empty(lwgeom2)) { return -1.0; }
    ...
    if (gbox_overlaps(&gbox1, &gbox2)) check_intersection = LW_TRUE;

    /* Point/line combinations can all be handled with simple point array iterations */
    if ((type1 == POINTTYPE || type1 == LINETYPE) && (type2 == POINTTYPE || type2 == LINETYPE))
    {
        POINTARRAY *pa1, *pa2;
    ...
    if ((type1 == POLYGONTYPE && type2 == POINTTYPE) || (type2 == POLYGONTYPE && type1 == POINTTYPE))
    {
        const POINT2D *p;
        LWPOLY *lwpoly;
        LWPOINT *lwpt;
        ...
        if (lwpoly_covers_point2d(lwpoly, p)) { return 0.0; }

        /* Not inside, so what's the actual distance? */
        for (i = 0; i < lwpoly->nrings; i++)
        {
            double ring_distance =
                ptarray_distance_spheroid(lwpoly->rings[i], lwpt->point, spheroid, tolerance, check_intersection);
            if (ring_distance < distance) distance = ring_distance;
    ...
    if ((type1 == POLYGONTYPE && type2 == LINETYPE) || (type2 == POLYGONTYPE && type1 == LINETYPE))
    {
        const POINT2D *p;
        LWPOLY *lwpoly;
        LWLINE *lwline;
        ...
        {
            double ring_distance =
                ptarray_distance_spheroid(lwpoly->rings[i], lwline->points, spheroid, tolerance, check_intersection);
    ...
    if (type1 == POLYGONTYPE && type2 == POLYGONTYPE)
    {
        ...
        for (i = 0; i < lwpoly1->nrings; i++)
        {
            ...
            {
                double ring_distance = ptarray_distance_spheroid(
                    lwpoly1->rings[i], lwpoly2->rings[j], spheroid, tolerance, check_intersection);
                if (ring_distance < distance) distance = ring_distance;
                if (distance < tolerance) return distance;
            }
        }
    ...
    if (lwtype_is_collection(type1))
    {
        ...
        for (i = 0; i < col->ngeoms; i++)
        {
            ...
            if (geom_distance < distance) distance = geom_distance;
            if (distance < tolerance) return distance;
        }
        return distance;
    }
...
int
lwgeom_covers_lwgeom_sphere(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2)
{
    int type1, type2;
    GBOX gbox1, gbox2;
    ...
    if ((type1 == POINTTYPE && type2 == LINETYPE) || (type1 == POINTTYPE && type2 == POLYGONTYPE) ||
    ...
    if (lwtype_is_collection(type1))
    {
        ...
        for (i = 0; i < col->ngeoms; i++)
        {
            if (lwgeom_covers_lwgeom_sphere(col->geoms[i], lwgeom2)) { return LW_TRUE; }
        }
        return LW_FALSE;
    }
    ...
    if (lwtype_is_collection(type2))
    {
        ...
        for (i = 0; i < col->ngeoms; i++)
        {
            if (!lwgeom_covers_lwgeom_sphere(lwgeom1, col->geoms[i])) { return LW_FALSE; }
        }
        return LW_TRUE;
    }
 ...
 */
int
lwpoly_covers_point2d(const LWPOLY *poly, const POINT2D *pt_to_test)
{
    uint32_t i;
    int in_hole_count = 0;
    POINT3D p;
    ...
    for (i = 1; i < poly->nrings; i++)
    {
        ...
        if (ptarray_contains_point_sphere(poly->rings[i], &pt_outside, pt_to_test)) in_hole_count++;
    }
 ...
 */
int
lwpoly_covers_lwpoly(const LWPOLY *poly1, const LWPOLY *poly2)
{
    uint32_t i;
 ...
 */
int
lwpoly_covers_lwline(const LWPOLY *poly, const LWLINE *line)
{
    /* Nulls and empties don't contain anything! */
    if (!poly || lwgeom_is_empty((LWGEOM *)poly))
    {
 ...
 */
int
lwpoly_covers_pointarray(const LWPOLY *lwpoly, const POINTARRAY *pta)
{
    uint32_t i;
 ...
 */
int
lwpoly_intersects_line(const LWPOLY *lwpoly, const POINTARRAY *line)
{
    uint32_t i, j, k;
    ...
    for (i = 0; i < lwpoly->nrings; i++)
    {
        for (j = 0; j < lwpoly->rings[i]->npoints - 1; j++)
        {
            ...
            for (k = 0; k < line->npoints - 1; k++)
            {
                ...
                /* ignore same edges */
                if (inter & PIR_INTERSECTS && !(inter & PIR_B_TOUCH_RIGHT || inter & PIR_COLINEAR)) { return LW_TRUE; }
            }
        }
    }
 ...
 */
int
lwline_covers_lwpoint(const LWLINE *lwline, const LWPOINT *lwpoint)
{
    uint32_t i;
    GEOGRAPHIC_POINT p;
    GEOGRAPHIC_EDGE e;
    ...
    for (i = 0; i < lwline->points->npoints - 1; i++)
    {
        ...
        if (edge_contains_point(&e, &p)) { return LW_TRUE; }
    }
 ...
 */
int
lwline_covers_lwline(const LWLINE *lwline1, const LWLINE *lwline2)
{
    uint32_t i, j;
    GEOGRAPHIC_EDGE e1, e2;
    ...
    while (i < lwline1->points->npoints - 1 && j < lwline2->points->npoints - 1)
    {
        ...
        if (edge_contains_point(&e1, &p2)) { start = LW_TRUE; }
 ...
 */
int
getPoint2d_p_ro(const POINTARRAY *pa, uint32_t n, POINT2D **point)
{
    uint8_t *pa_ptr = NULL;
    assert(pa);
    assert(n < pa->npoints);
...
int
ptarray_calculate_gbox_geodetic(const POINTARRAY *pa, GBOX *gbox)
{
    uint32_t i;
    int first = LW_TRUE;
    const POINT2D *p;
...
static int
lwpoint_calculate_gbox_geodetic(const LWPOINT *point, GBOX *gbox)
{
    assert(point);
...
static int
lwline_calculate_gbox_geodetic(const LWLINE *line, GBOX *gbox)
{
    assert(line);
...
static int
lwpolygon_calculate_gbox_geodetic(const LWPOLY *poly, GBOX *gbox)
{
    GBOX ringbox;
    uint32_t i;
    int first = LW_TRUE;
    ...
    {
        if (ptarray_calculate_gbox_geodetic(poly->rings[i], &ringbox) == LW_FAILURE) return LW_FAILURE;
        if (first)
        {
...
static int
lwtriangle_calculate_gbox_geodetic(const LWTRIANGLE *triangle, GBOX *gbox)
{
    assert(triangle);
...
static int
lwcollection_calculate_gbox_geodetic(const LWCOLLECTION *coll, GBOX *gbox)
{
    GBOX subbox;
    uint32_t i;
    ...
    int first = LW_TRUE;
    assert(coll);
    if (coll->ngeoms == 0) return LW_FAILURE;
    ...
    for (i = 0; i < coll->ngeoms; i++)
    {
        if (lwgeom_calculate_gbox_geodetic((LWGEOM *)(coll->geoms[i]), &subbox) == LW_SUCCESS)
        {
            ...
            if (coll->geoms[i]->bbox) lwfree(coll->geoms[i]->bbox);
...
int
lwgeom_calculate_gbox_geodetic(const LWGEOM *geom, GBOX *gbox)
{
    int result = LW_FAILURE;
...
static int
ptarray_check_geodetic(const POINTARRAY *pa)
{
    uint32_t t;
    POINT2D pt;
    ...
    for (t = 0; t < pa->npoints; t++)
    {
        ...
        /* printf( "%d (%g, %g)\n", t, pt.x, pt.y); */
        if (pt.x < -180.0 || pt.y < -90.0 || pt.x > 180.0 || pt.y > 90.0) return LW_FALSE;
    }
...
static int
lwpoint_check_geodetic(const LWPOINT *point)
{
    assert(point);
...
static int
lwline_check_geodetic(const LWLINE *line)
{
    assert(line);
...
static int
lwpoly_check_geodetic(const LWPOLY *poly)
{
    uint32_t i = 0;
    assert(poly);
    ...
    for (i = 0; i < poly->nrings; i++)
    {
        if (ptarray_check_geodetic(poly->rings[i]) == LW_FALSE) return LW_FALSE;
    }
    return LW_TRUE;
}

static int
lwtriangle_check_geodetic(const LWTRIANGLE *triangle)
{
    assert(triangle);
...
static int
lwcollection_check_geodetic(const LWCOLLECTION *col)
{
    uint32_t i = 0;
    assert(col);
    ...
    for (i = 0; i < col->ngeoms; i++)
    {
        if (lwgeom_check_geodetic(col->geoms[i]) == LW_FALSE) return LW_FALSE;
    }
    return LW_TRUE;
}

int
lwgeom_check_geodetic(const LWGEOM *geom)
{
...
static int
ptarray_force_geodetic(POINTARRAY *pa)
{
    uint32_t t;
    int changed = LW_FALSE;
    POINT4D pt;
...
static int
lwpoint_force_geodetic(LWPOINT *point)
{
    assert(point);
...
static int
lwline_force_geodetic(LWLINE *line)
{
    assert(line);
...
static int
lwpoly_force_geodetic(LWPOLY *poly)
{
    uint32_t i = 0;
    int changed = LW_FALSE;
    assert(poly);
    ...
    for (i = 0; i < poly->nrings; i++)
    {
        if (ptarray_force_geodetic(poly->rings[i]) == LW_TRUE) changed = LW_TRUE;
    }
    return changed;
}

static int
lwcollection_force_geodetic(LWCOLLECTION *col)
{
    uint32_t i = 0;
    int changed = LW_FALSE;
    assert(col);
    ...
    for (i = 0; i < col->ngeoms; i++)
    {
        if (lwgeom_force_geodetic(col->geoms[i]) == LW_TRUE) changed = LW_TRUE;
    }
    return changed;
}

int
lwgeom_force_geodetic(LWGEOM *geom)
{
...
double
ptarray_length_spheroid(const POINTARRAY *pa, const SPHEROID *s)
{
    ...
    for (i = 1; i < pa->npoints; i++)
    {
        ...
        /* Special sphere case */
        if (s->a == s->b) seglength = s->radius * sphere_distance(&a, &b);
        /* Spheroid case */
        else
            seglength = spheroid_distance(&a, &b, s);

        /* Add in the vertical displacement if we're in 3D */
        if (hasz) seglength = sqrt((zb - za) * (zb - za) + seglength * seglength);
...
double
lwgeom_length_spheroid(const LWGEOM *geom, const SPHEROID *s)
{
    int type;
    uint32_t i = 0;
    double length = 0.0;
    ...
    /* No area in nothing */
    if (lwgeom_is_empty(geom)) return 0.0;

    type = geom->type;

    if (type == POINTTYPE || type == MULTIPOINTTYPE) return 0.0;

    if (type == LINETYPE) return ptarray_length_spheroid(((LWLINE *)geom)->points, s);
    ...
    if (type == TRIANGLETYPE) return ptarray_length_spheroid(((LWTRIANGLE *)geom)->points, s);
...
ptarray_nudge_geodetic(POINTARRAY *pa)
{
    ...
    if (!pa) lwerror("ptarray_nudge_geodetic called with null input");
...
lwgeom_nudge_geodetic(LWGEOM *geom)
{
    ...
    if (lwgeom_is_empty(geom)) return LW_FALSE;

    type = geom->type;

    if (type == POINTTYPE) return ptarray_nudge_geodetic(((LWPOINT *)geom)->point);

    if (type == LINETYPE) return ptarray_nudge_geodetic(((LWLINE *)geom)->points);
    ...
    if (type == TRIANGLETYPE) return ptarray_nudge_geodetic(((LWTRIANGLE *)geom)->points);
...
point_in_cone(const POINT3D *A1, const POINT3D *A2, const POINT3D *P)
{
    ...
    if (similarity > min_similarity || fabs(similarity - min_similarity) < 2e-16) { return LW_TRUE; }
    return LW_FALSE;
}
...
edge_intersects(const POINT3D *A1, const POINT3D *A2, const POINT3D *B1, const POINT3D *B2)
{
    ...
    if (a1_side != a2_side && (a1_side + a2_side) == 0 && b1_side != b2_side && (b1_side + b2_side) == 0)
    {
        ...
        if (point_in_cone(A1, A2, &VN) && point_in_cone(B1, B2, &VN)) { return PIR_INTERSECTS; }
 ...
 */
int
ptarray_contains_point_sphere(const POINTARRAY *pa, const POINT2D *pt_outside, const POINT2D *pt_to_test)
{
    ...
    if (!pa || pa->npoints < 4) return LW_FALSE;
    ...
    for (i = 1; i < pa->npoints; i++)
    {
        ...
        if (point3d_equals(&E1, &E2)) { continue; }

        /* Our test point is on an edge end! Point is "in ring" by our definition */
        if (point3d_equals(&S1, &E1)) { return LW_TRUE; }
        ...
        if (inter & PIR_INTERSECTS)
        {
            ...
            if ((inter & PIR_A_TOUCH_RIGHT) || (inter & PIR_A_TOUCH_LEFT)) { return LW_TRUE; }
