...
static void
test_mindistance2d_tolerance(void)
{
    /*
    ** Simple case.
    */
    ...
    */
    DIST2DTEST("GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(POINT(0 0)))",
               "GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(POINT(3 4)))",
               5.0);

    /*
    ** Geometry Collection Collection Multipoint vs Geometry Collection Collection Multipoint
    */
    DIST2DTEST("GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(MULTIPOINT(0 0)))",
               "GEOMETRYCOLLECTION(GEOMETRYCOLLECTION(MULTIPOINT(3 4)))",
               5.0);
...
test_rect_tree_contains_point(void)
{
     ...
     * curvepolygon
     */
    poly = lwgeom_from_wkt(
        "CURVEPOLYGON(COMPOUNDCURVE(CIRCULARSTRING(0 0,1 5,0 10),(0 10,10 10,10 0, 0 "
        "0)),COMPOUNDCURVE(CIRCULARSTRING(3 7,5 8,7 7),(7 7,7 3,3 3, 3 7)))",
     ...
     */
    poly = lwgeom_from_wkt("POLYGON((0 0,0 10,10 10,10 0,9 0,9 9,8 6,8 0,2 0,2 9,1 6,1 0,0 0),(4 4,4 6,6 6,6 4,4 4))",
                           LW_PARSER_CHECK_NONE);
    tree = rect_tree_from_lwgeom(poly);
...
static void
test_rect_tree_intersects_tree(void)
{
    /* total overlap, A == B */
    CU_ASSERT_EQUAL(tree_inter("POLYGON((0 0, 3 1, 0 2, 3 3, 0 4, 3 5, 0 6, 5 6, 5 0, 0 0))",
...
test_rect_tree_distance_tree(void)
{
    ...
    wkt =
        ...
        "48.4234039978588,-123.35417179975 48.4229151379279,-123.354369811539 48.4220987102936,-123.355779071731 "
        "48.4222571534228,-123.357238860904 48.4224209369449,-123.35702791281 48.4232302445918)))";
    TDT(wkt,
        "MULTIPOLYGON(((-123.353452578038 48.4259519079838,-123.35072012771 48.4256699150083,-123.347337809991 "
        "48.4254740864963,-123.347469111645 48.4245757659326,-123.349409235923 48.4246224093429,-123.349966167324 "
        "48.4246562342604,-123.353650661317 48.4250703224683,-123.353452578038 48.4259519079838)))",
        0.0017144228293396);
...
test_lwgeom_segmentize2d(void)
{
    ...
    linein = lwgeom_from_wkt(
        "MULTIPOLYGON(((0 0,20 0,20 20,0 20,0 0),(2 2,2 4,4 4,4 2,2 2),(6 6,6 8,8 8,8 6,6 6)),((40 0,40 20,60 "
        "20,60 0,40 0),(42 2,42 4,44 4,44 2,42 2)))",
        LW_PARSER_CHECK_NONE);
    lwgeom_request_interrupt();
    lineout = lwgeom_segmentize2d(linein, 1e-100);
    CU_ASSERT_EQUAL(lineout, NULL);
    ...
    linein = lwgeom_from_wkt(
        "GEOMETRYCOLLECTION(MULTIPOLYGON(((0 0,20 0,20 20,0 20,0 0),(2 2,2 4,4 4,4 2,2 2),(6 6,6 8,8 8,8 6,6 "
        "6)),((40 0,40 20,60 20,60 0,40 0),(42 2,42 4,44 4,44 2,42 2))),MULTILINESTRING((0 0,10 0),(20 0, 30 "
        "0)),MULTIPOINT(0 0, 3 4))",
        LW_PARSER_CHECK_NONE);
...
test_lwgeom_locate_along(void)
{
    ...
    /* ST_Locatealong(ST_GeomFromText('MULTILINESTRING M ((1 2 3, 5 4 5), (50 50 1, 60 60 200))'), 105) */
    geom = lwgeom_from_wkt("MULTILINESTRING M ((1 2 3, 3 4 2, 9 4 3), (1 2 3, 5 4 5), (50 50 1, 60 60 200))",
                           LW_PARSER_CHECK_NONE);
    out = lwgeom_locate_along(geom, measure, 0.0);
    str = lwgeom_to_wkt(out, WKT_ISO, 6, NULL);
    lwgeom_free(geom);
...
{
    /* int lw_dist2d_seg_arc(const POINT2D *A1, const POINT2D *A2, const POINT2D *B1, const POINT2D *B2, const
     * POINT2D *B3, DISTPTS *dl) */
...
test_lw_dist2d_ptarray_ptarrayarc(void)
{
    ...
    ASSERT_STRING_EQUAL(cu_error_msg, "lw_dist2d_ptarray_ptarrayarc called with non-arc input");
...
test_lwgeom_tcpa(void)
{
    ...
    ASSERT_STRING_EQUAL(cu_error_msg, "Both input geometries must have a measure dimension");
    ...
    lwgeom_free(g2);
    ASSERT_DOUBLE_EQUAL(m, -1.0);
    ASSERT_STRING_EQUAL(cu_error_msg, "Both input geometries must be linestrings");
    ...
    ASSERT_DOUBLE_EQUAL(m, -1.0);
    ASSERT_STRING_EQUAL(cu_error_msg, "Both input lines must have at least 2 points" /* should be accepted
                                                                                        ? */
    ...
    lwgeom_free(g2);
    ASSERT_DOUBLE_EQUAL(m, -1.0);
    ASSERT_STRING_EQUAL(cu_error_msg, "Both input lines must have at least 2 points"
