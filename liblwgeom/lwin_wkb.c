...
bytes_from_hexbytes(const char *hexbuf, size_t hexsize)
{
  ...
  if (hexsize % 2) lwerror("Invalid hex string, length (%d) has to be a multiple of two!", hexsize);

  buf = lwalloc(hexsize / 2);

  if (!buf) lwerror("Unable to allocate memory buffer.");

  for (i = 0; i < hexsize / 2; i++)
  {
    h1 = hex2char[(int)hexbuf[2 * i]];
    h2 = hex2char[(int)hexbuf[2 * i + 1]];
    if (h1 > 15) lwerror("Invalid hex character (%c) encountered", hexbuf[2 * i]);
    if (h2 > 15) lwerror("Invalid hex character (%c) encountered", hexbuf[2 * i + 1]);
 ...
 */
static void
lwtype_from_wkb_state(wkb_parse_state *s, uint32_t wkb_type)
{
 ...
 */
static POINTARRAY *
ptarray_from_wkb_state(wkb_parse_state *s)
{
  POINTARRAY *pa = NULL;
  size_t pa_size;
  uint32_t ndims = 2;
  ...
  /* Empty! */
  if (npoints == 0) return ptarray_construct(s->has_z, s->has_m, npoints);
 ...
 */
static LWPOINT *
lwpoint_from_wkb_state(wkb_parse_state *s)
{
 ...
 */
static LWLINE *
lwline_from_wkb_state(wkb_parse_state *s)
{
 ...
 */
static LWCIRCSTRING *
lwcircstring_from_wkb_state(wkb_parse_state *s)
{
  POINTARRAY *pa = ptarray_from_wkb_state(s);

  if (pa == NULL || pa->npoints == 0) return lwcircstring_construct_empty(s->srid, s->has_z, s->has_m);
 ...
 */
static LWPOLY *
lwpoly_from_wkb_state(wkb_parse_state *s)
{
 ...
 */
static LWTRIANGLE *
lwtriangle_from_wkb_state(wkb_parse_state *s)
{
  uint32_t nrings = integer_from_wkb_state(s);
  ...
  if (tri->points) ptarray_free(tri->points);
 ...
 */
static LWCURVEPOLY *
lwcurvepoly_from_wkb_state(wkb_parse_state *s)
{
 ...
 */
static LWCOLLECTION *
lwcollection_from_wkb_state(wkb_parse_state *s)
{
  uint32_t ngeoms = integer_from_wkb_state(s);
  ...
  /* Empty collection? */
  if (ngeoms == 0) return col;

  /* Be strict in polyhedral surface closures */
  if (s->lwtype == POLYHEDRALSURFACETYPE) s->check |= LW_PARSER_CHECK_ZCLOSURE;
 ...
 */
LWGEOM *
lwgeom_from_wkb_state(wkb_parse_state *s)
{
 ...
 */
LWGEOM *
lwgeom_from_wkb(const uint8_t *wkb, const size_t wkb_size, const char check)
{
  wkb_parse_state s;
...
LWGEOM *
lwgeom_from_hexwkb(const char *hexwkb, const char check)
{
  int hexwkb_len;
  uint8_t *wkb;
  LWGEOM *lwgeom;
