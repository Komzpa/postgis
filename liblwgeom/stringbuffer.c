...
{
  if (s->str_end == s->str_start) return 0;
...
stringbuffer_avprintf(stringbuffer_t *s, const char *fmt, va_list ap)
{
  ...
  if (len < 0)
    ...
    len = _vscprintf(fmt, ap2); /**Assume windows flaky vsnprintf that returns -1 if initial buffer to small
                                   and add more space **/
#else
    return len;
#endif
