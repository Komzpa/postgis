...
gbox_same(const GBOX *g1, const GBOX *g2)
{
  ...
  if (FLAGS_GET_Z(g1->flags) && (g1->zmin != g2->zmin || g1->zmax != g2->zmax)) return LW_FALSE;
  if (FLAGS_GET_M(g1->flags) && (g1->mmin != g2->mmin || g1->mmax != g2->mmax)) return LW_FALSE;
...
{
  if (g1->xmin == g2->xmin && g1->ymin == g2->ymin && g1->xmax == g2->xmax && g1->ymax == g2->ymax) return LW_TRUE;
  return LW_FALSE;
}
...
gbox_is_valid(const GBOX *gbox)
{
  ...
  {
    if (!isfinite(gbox->zmin) || isnan(gbox->zmin) || !isfinite(gbox->zmax) || isnan(gbox->zmax)) return LW_FALSE;
  }

  /* M */
  if (FLAGS_GET_M(gbox->flags))
  {
    if (!isfinite(gbox->mmin) || isnan(gbox->mmin) || !isfinite(gbox->mmax) || isnan(gbox->mmax)) return LW_FALSE;
  }
...
gbox_merge(const GBOX *new_box, GBOX *merge_box)
{
  ...
  if (FLAGS_GET_ZM(merge_box->flags) != FLAGS_GET_ZM(new_box->flags)) return LW_FAILURE;
...
gbox_overlaps(const GBOX *g1, const GBOX *g2)
{
  ...
  /* Check X/Y first */
  if (g1->xmax < g2->xmin || g1->ymax < g2->ymin || g1->xmin > g2->xmax || g1->ymin > g2->ymax) return LW_FALSE;
...
size_t
gbox_serialized_size(uint8_t flags)
{
...
lw_arc_calculate_gbox_cartesian_2d(const POINT2D *A1, const POINT2D *A2, const POINT2D *A3, GBOX *gbox)
{
  ...
  if (A2_side == lw_segment_side(A1, A3, &xmin)) gbox->xmin = xmin.x;

  if (A2_side == lw_segment_side(A1, A3, &ymin)) gbox->ymin = ymin.y;

  if (A2_side == lw_segment_side(A1, A3, &xmax)) gbox->xmax = xmax.x;

  if (A2_side == lw_segment_side(A1, A3, &ymax)) gbox->ymax = ymax.y;
...
lwcircstring_calculate_gbox_cartesian(LWCIRCSTRING *curve, GBOX *gbox)
{
  ...
  tmp.flags = gflags(FLAGS_GET_Z(curve->flags), FLAGS_GET_M(curve->flags), 0);
  ...
  for (i = 2; i < curve->points->npoints; i += 2)
  {
    ...
    if (lw_arc_calculate_gbox_cartesian(&p1, &p2, &p3, &tmp) == LW_FAILURE) continue;
...
lwcollection_calculate_gbox_cartesian(LWCOLLECTION *coll, GBOX *gbox)
{
  ...
  int first = LW_TRUE;
  assert(coll);
  if ((coll->ngeoms == 0) || !gbox) return LW_FAILURE;
