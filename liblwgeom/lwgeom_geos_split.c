...
lwline_split_by_line(const LWLINE *lwline_in, const LWGEOM *blade_in)
{
  ...
  assert(blade_in->type == LINETYPE || blade_in->type == MULTILINETYPE || blade_in->type == POLYGONTYPE ||
...
lwline_split_by_point(const LWLINE *lwline_in, const LWPOINT *blade_in)
{
  ...
  if (lwline_split_by_point_to(lwline_in, blade_in, out) < 2) { lwmline_add_lwline(out, lwline_clone_deep(lwline_in)); }
...
lwline_split_by_point_to(const LWLINE *lwline_in, const LWPOINT *blade_in, LWMLINE *v)
{
  ...
  LWDEBUGF(3,
           "Projected point:(%g %g), seg:%d, p1:(%g %g), p2:(%g %g)",
           pt_projected.x,
           pt_projected.y,
           seg,
           p1.x,
  ...
  if (((seg == nsegs - 1) && p4d_same(&pt_projected, &p2)) || ((seg == 0) && p4d_same(&pt_projected, &p1)))
  { return 1; }
...
lwline_split(const LWLINE *lwline_in, const LWGEOM *blade_in)
{
  ...
  default:
    lwerror("Splitting a Line by a %s is unsupported", lwtype_name(blade_in->type));
    return NULL;
  }
  return NULL;
}
...
lwpoly_split(const LWPOLY *lwpoly_in, const LWGEOM *blade_in)
{
  ...
  default:
    lwerror("Splitting a Polygon by a %s is unsupported", lwtype_name(blade_in->type));
    return NULL;
  }
  return NULL;
}
