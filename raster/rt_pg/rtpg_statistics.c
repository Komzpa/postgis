...
Datum RASTER_summaryStats_transfn(PG_FUNCTION_ARGS)
{
        ...
        for (i = 2; i < nargs; i++)
        {
            ...
            else if ((calltype == FLOAT4OID || calltype == FLOAT8OID) && (i == 3 || i == 4))
            {
                ...
                if (state->sample < 0. || state->sample > 1.)
                {
                    ...
                    MemoryContextSwitchTo(oldcontext);
                    elog(ERROR, "Invalid sample percentage (must be between 0 and 1). Returning NULL");
    ...
    if (NULL == stats)
    {
        elog(NOTICE, "Cannot compute summary statistics for band at index %d. Returning NULL", state->band_index);
...
Datum RASTER_histogramCoverage(PG_FUNCTION_ARGS)
{
    ...
    if (SRF_IS_FIRSTCALL())
    {
        ...
        /* coverage stats */
        len = sizeof(char) * (strlen("SELECT min, max FROM _st_summarystats('','',,::boolean,)") + strlen(tablename) +
                              strlen(colname) + (MAX_INT_CHARLEN * 2) + MAX_DBL_CHARLEN + 1);
        sql = (char *)palloc(len);
        if (NULL == sql)
        {
...
Datum RASTER_quantileCoverage(PG_FUNCTION_ARGS)
{
    ...
    if (SRF_IS_FIRSTCALL())
    {
        ...
        len = sizeof(char) * (strlen("SELECT count FROM _st_summarystats('','',,::boolean,)") + strlen(tablename) +
                              strlen(colname) + (MAX_INT_CHARLEN * 2) + MAX_DBL_CHARLEN + 1);
        sql = (char *)palloc(len);
        if (NULL == sql)
        {
        ...
        while (SPI_processed == 1 && SPI_tuptable != NULL)
        {
            ...
            covquant = rt_band_get_quantiles_stream(
                band, exclude_nodata_value, sample, cov_count, &qlls, &qlls_count, quantiles, quantiles_count, &count);
...
Datum RASTER_valueCountCoverage(PG_FUNCTION_ARGS)
{
    ...
    if (SRF_IS_FIRSTCALL())
    {
        ...
        while (SPI_processed == 1 && SPI_tuptable != NULL)
        {
            ...
            /* get counts of values */
            vcnts = rt_band_get_value_count(
                band, (int)exclude_nodata_value, search_values, search_values_count, roundto, &total, &count);
            rt_band_destroy(band);
            rt_raster_destroy(raster);
            if (NULL == vcnts || !count)
            {
