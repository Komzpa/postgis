...
Datum RASTER_dumpValues(PG_FUNCTION_ARGS)
{
  ...
  if (call_cntr < max_calls)
  {
    ...
    mdValues = construct_md_array(
        arg2->values[call_cntr], arg2->nodata[call_cntr], ndim, dim, lbound, FLOAT8OID, typlen, typbyval, typalign);
...
Datum RASTER_pixelOfValue(PG_FUNCTION_ARGS)
{
  ...
  if (SRF_IS_FIRSTCALL())
  {
    ...
    {
      /* error */
      if (count < 0) elog(NOTICE, "Could not get the pixels of search values for band at index %d", nband);
      /* no nearest pixel */
      else
...
Datum RASTER_neighborhood(PG_FUNCTION_ARGS)
{
  ...
  {
    count = rt_band_get_nearest_pixel(band, _x, _y, distance[0], distance[1], exclude_nodata_value, &npixels);
    /* error */
    if (count < 0)
    {
