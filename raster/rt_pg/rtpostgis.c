...
rtpg_assignHookGDALEnabledDrivers(const char *enabled_drivers, void *extra)
{
  ...
  if (!enable_all)
  {
    ...
    for (i = 0; i < drv_count; i++)
    {
      ...
      {
        gdal_skip = repalloc(gdal_skip, sizeof(char) * (strlen(gdal_skip) + 1 + strlen(drv_set[i].short_name) + 1));
        strcat(gdal_skip, " ");
      }
...
_PG_init(void)
{
  ...
  */
  ...
  POSTGIS_RT_DEBUGF(4, "boot_postgis_enable_outdb_rasters = %s", boot_postgis_enable_outdb_rasters ? "TRUE" : "FALSE");
  ...
  else
  {
    DefineCustomStringVariable(
        ...
        "List of enabled GDAL drivers by short name. To enable/disable all drivers, use 'ENABLE_ALL' or "
        "'DISABLE_ALL' (sets the GDAL_SKIP config option).", /* long_desc */
  ...
  {
    DefineCustomBoolVariable("postgis.enable_outdb_rasters",                                  /* name */
