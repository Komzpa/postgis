  ...
  */
  ...
  for (i = 0; i < n; i++)
  {
    ...
    bandindex = rt_raster_generate_new_band(
        raster, arg[i].pixtype, arg[i].initialvalue, arg[i].hasnodata, arg[i].nodatavalue, arg[i].index - 1);
...
Datum RASTER_addBandOutDB(PG_FUNCTION_ARGS)
{
  ...
  for (i = 0, j = dstnband - 1; i < numsrcnband; i++, j++)
  {
    ...
    band = rt_band_new_offline_from_path(width, height, hasnodata, nodataval, srcnband[i], outdbfile, FALSE);
    if (band == NULL)
    {
      if (raster != NULL) rt_raster_destroy(raster);
      if (pgraster != NULL) PG_FREE_IF_COPY(pgraster, 0);
    ...
    */
    ...
    for (i = 0; i < arg2->numbands; i++)
    {
      ...
      else
      {
        ...
        band = rt_band_new_offline(width, height, pixtype, hasnodata, nodataval, bandnum, rt_band_get_ext_path(_band));
