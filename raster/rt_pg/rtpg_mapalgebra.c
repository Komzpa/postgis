    ...
    for (i = 0; i < iwr->numband; i++)
    {
        ...
        for (j = 0; j < iwr->bandarg[i].numraster; j++)
        {
            ...
            if (!isempty[0] && !isempty[1] && hasband[0] && hasband[1])
            {
                ...
                if (rt_raster_from_two_rasters(iwr->bandarg[i].raster[j], raster, ET_UNION, &iraster, _offset) !=
                    ES_NONE)
                {
                ...
                {
                    POSTGIS_RT_DEBUGF(
                        4, "Getting pixel line of iterator raster at (x, y, length) = (0, %d, %d)", y, _dim[0]);
                    if (rt_band_get_pixel_line(_band, 0, y, _dim[0], &vals, &nvals) != ES_NONE)
                    {
            ...
            for (b = 0; b < colon_n; b++)
            {
                ...
                for (c = 0; c < dash_n; c++)
                {
                         ...
                         strchr(dash_set[c], ')') != NULL || strchr(dash_set[c], ']') != NULL))
                    {
                        ...
                        {
                            dash_set[dash_it - 1] =
                                repalloc(dash_set[dash_it - 1], (strlen(dash_set[dash_it]) + 1) * sizeof(char));
                    ...
                    if (junk != colon_set[b])
                    {
                        ...
                        if (*(junk - 1) == '-')
                        {
                             ...
                             * negative number */
                            if (((junk - 1) == colon_set[b]) || (*(junk - 2) == '-') || (*(junk - 2) == '[') ||
                                (*(junk - 2) == '('))
                            { val *= -1.; }
                        }
                    }
...
Datum RASTER_mapAlgebraExpr(PG_FUNCTION_ARGS)
{
    ...
    if (!PG_ARGISNULL(3))
    {
        ...
        strncpy(
            initexpr + strlen("SELECT (") + strlen(expression), ")::double precision", strlen(")::double precision"));
        initexpr[len] = '\0';
