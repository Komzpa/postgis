...
Datum RASTER_nMapAlgebra(PG_FUNCTION_ARGS)
{
  ...
  if (!PG_ARGISNULL(1) || get_fn_expr_argtype(fcinfo->flinfo, 1) == REGPROCEDUREOID)
  {
    ...
    */
    ...
    InitFunctionCallInfoData(
        arg->callback.ufc_info, &(arg->callback.ufl_info), arg->callback.ufl_info.fn_nargs, InvalidOid, NULL, NULL);
  ...
  POSTGIS_RT_DEBUGF(
      4, "pixtype, hasnodata, nodataval: %s, %d, %f", rt_pixtype_name(arg->pixtype), arg->hasnodata, arg->nodataval);
    ...
    if (arg->callback.expr[i].spi_argcount)
    {
      ...
      for (j = 0, k = 0; j < argkwcount; j++)
      {
        ...
        if ((strstr(argkw[j], "[rast.x]") != NULL) || (strstr(argkw[j], "[rast.y]") != NULL) ||
            (strstr(argkw[j], "[rast1.x]") != NULL) || (strstr(argkw[j], "[rast1.y]") != NULL) ||
...
Datum RASTER_union_transfn(PG_FUNCTION_ARGS)
{
  ...
  for (i = 0; i < iwr->numband; i++)
  {
    ...
    for (j = 0; j < iwr->bandarg[i].numraster; j++)
    {
      ...
      POSTGIS_RT_DEBUGF(
          4, "(pixtype, hasnodata, nodataval) = (%s, %d, %f)", rt_pixtype_name(pixtype), hasnodata, nodataval);
      ...
      if (!isempty[0] && !isempty[1] && hasband[0] && hasband[1])
      {
        ...
        if (rt_raster_from_two_rasters(iwr->bandarg[i].raster[j], raster, ET_UNION, &iraster, _offset) != ES_NONE)
        {
        ...
        POSTGIS_RT_DEBUGF(4, "_offset = %f, %f, %f, %f", _offset[0], _offset[1], _offset[2], _offset[3]);
        ...
        if (!reuserast)
        {
          ...
          if (rt_raster_generate_new_band(iraster, pixtype, nodataval, hasnodata, nodataval, 0) == -1)
          {
          ...
          for (y = 0; y < _dim[1]; y++)
          {
            ...
            POSTGIS_RT_DEBUGF(
                4, "Setting pixel line at (x, y, length) = (%d, %d, %d)", (int)_offset[0], (int)_offset[1] + y, nvals);
            if (rt_band_set_pixel_line(iband, (int)_offset[0], (int)_offset[1] + y, vals, nvals) != ES_NONE)
            {
        ...
        noerr = rt_raster_iterator(
            itrset, 2, ET_LAST, NULL, pixtype, hasnodata, nodataval, 0, 0, NULL, &utype, rtpg_union_callback, &_raster);
        if (noerr != ES_NONE)
        {
        ...
        for (y = 0; y < _dim[1]; y++)
        {
          ...
          POSTGIS_RT_DEBUGF(
              4, "Setting pixel line at (x, y, length) = (%d, %d, %d)", (int)_offset[2], (int)_offset[3] + y, nvals);
          if (rt_band_set_pixel_line(iband, (int)_offset[2], (int)_offset[3] + y, vals, nvals) != ES_NONE)
          {
...
Datum RASTER_union_finalfn(PG_FUNCTION_ARGS)
{
  ...
  for (i = 0; i < iwr->numband; i++)
  {
    ...
    if (iwr->bandarg[i].uniontype == UT_MEAN || iwr->bandarg[i].uniontype == UT_RANGE) { rt_raster_destroy(_raster); }
      ...
      for (b = 0; b < colon_n; b++)
      {
        ...
        for (c = 0; c < dash_n; c++)
        {
          ...
          if (dash_n != 1)
          {
            ...
            if (c < 1)
            {
              if (strchr(dash_set[c], ')') != NULL || strchr(dash_set[c], ']') != NULL)
              {
                exc_val = 1;
                inc_val = 1;
              }
            ...
            {
              if (strrchr(dash_set[c], '(') != NULL || strrchr(dash_set[c], '[') != NULL)
              {
                exc_val = 1;
                inc_val = 0;
              }
          ...
          POSTGIS_RT_DEBUGF(
              4, "(colon_set[%d], dash_set[%d], junk) = (%s, %s, %s)", b, c, colon_set[b], dash_set[c], junk);
          ...
          if (junk != colon_set[b])
          {
            ...
            if (*(junk - 1) == '-')
            {
               ...
               * negative number */
              if (((junk - 1) == colon_set[b]) || (*(junk - 2) == '-') || (*(junk - 2) == '[') || (*(junk - 2) == '('))
              { val *= -1.; }
            }
          }
          POSTGIS_RT_DEBUGF(4, "RASTER_reclass: min/max (double) %f", val);
    ...
    for (i = 0; i < arg->nentry; i++)
    {
      ...
      for (j = 0; j < arg->nelement; j++)
      {
        ...
        if (j == 0)
        {
          ...
          /* NODATA */
          if (strcmp(_element, "NV") == 0 || strcmp(_element, "NULL") == 0 || strcmp(_element, "NODATA") == 0)
          {
          ...
          else if ((percent = strchr(_element, '%')) != NULL)
          {
            ...
            arg->colormap->entry[arg->colormap->nentry].value =
                ((value / 100.) * (arg->bandstats->max - arg->bandstats->min)) + arg->bandstats->min;
          }
          /* straight value */
          else
          {
...
Datum RASTER_mapAlgebraExpr(PG_FUNCTION_ARGS)
{
      ...
      !hasnodataval)
  {
    ...
    POSTGIS_RT_DEBUGF(4, "RASTER_mapAlgebraExpr: New raster has %d bands", rt_raster_get_num_bands(newrast));

    rt_raster_copy_band(newrast, raster, nband - 1, 0);

    POSTGIS_RT_DEBUGF(4, "RASTER_mapAlgebraExpr: New raster now has %d bands", rt_raster_get_num_bands(newrast));
  ...
  for (x = 0 + ngbwidth; x < width - ngbwidth; x++)
  {
    for (y = 0 + ngbheight; y < height - ngbheight; y++)
    {
      ...
      { /* neighborhood should replace NODATA cells with the central pixel value, and a NODATA cell
           was detected */
        POSTGIS_RT_DEBUGF(3, "RASTER_mapAlgebraFctNgb: (%dx%d), %dx%d neighborhood", x, y, winwidth, winheight);

        neighborDatum = construct_md_array(
            (void *)neighborData, neighborNulls, 2, neighborDims, neighborLbs, FLOAT8OID, typlen, typbyval, typalign);
      ...
      if (!PG_ARGISNULL(spi_exprpos[i]))
      {
        ...
        if (spi_argcount[i])
        {
          ...
          for (j = 0, k = 0; j < argkwcount; j++)
          {
            ...
            /* positions are INT4 */
            if ((strstr(argkw[j], "[rast1.x]") != NULL) || (strstr(argkw[j], "[rast1.y]") != NULL) ||
                (strstr(argkw[j], "[rast2.x]") != NULL) || (strstr(argkw[j], "[rast2.y]") != NULL))
            { argtype[k] = INT4OID; }
            /* everything else is FLOAT8 */
            else
    ...
    for (x = 0; x < dim[0]; x++)
    {
      for (y = 0; y < dim[1]; y++)
      {
        ...
        for (i = 0; i < set_count; i++)
        {
          ...
          else if (!_isempty[i] && (_x >= 0 && _x < _dim[i][0]) && (_y >= 0 && _y < _dim[i][1]))
          {
          ...
          POSTGIS_RT_DEBUGF(5, "pixel r%d(%d, %d) = %d, %f", i, _x, _y, _haspixel[i], _pixel[i]);
        }
          ...
          else if (spi_plan[i] != NULL)
          {
            ...
            if (spi_argcount[i])
            {
              ...
              for (j = 0; j < argkwcount; j++)
              {
                ...
                if (strstr(argkw[j], "[rast1.x]") != NULL) { values[idx] = _pos[0][0]; }
                else if (strstr(argkw[j], "[rast1.y]") != NULL)
                {
                  values[idx] = _pos[0][1];
                }
                else if ((strstr(argkw[j], "[rast1.val]") != NULL) || (strstr(argkw[j], "[rast1]") != NULL))
                {
                ...
                else if ((strstr(argkw[j], "[rast2.val]") != NULL) || (strstr(argkw[j], "[rast2]") != NULL))
                {
