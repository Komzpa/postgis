                              ...
                              int force)
{
    ...
    {
        rterror("rt_band_new_offline_from_path: Specified band %d not found in offline raster: %s", bandNum, path);
        GDALClose(hdsSrc);
        return NULL;
    }
...
rt_band_duplicate(rt_band band)
{
    ...
    else
    {
        ...
        rtn = rt_band_new_inline(band->width, band->height, band->pixtype, band->hasnodata, band->nodataval, data);
...
rt_band_load_offline_data(rt_band band)
{
    ...
    RASTER_DEBUGF(3, "Offline geotransform (%f, %f, %f, %f, %f, %f)", ogt[0], ogt[1], ogt[2], ogt[3], ogt[4], ogt[5]);
                          ...
                          rt_pixel *npixels)
{
    ...
    *npixels = NULL;
    ...
    RASTER_DEBUGF(4, "Maximum Extent: (%d, %d, %d, %d)", max_extent[0], max_extent[1], max_extent[2], max_extent[3]);
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            for (j = 0; j < 2; j++)
            {
                ...
                for (k = 0; k < (uint32_t)_max; k++)
                {
                    ...
                    if (_x < max_extent[0] || _x > max_extent[2] || _y < max_extent[1] || _y > max_extent[3])
                    {
                        (*_min)++;
                        continue;
                    }
                    ...
                    if ((_x < 0 || _x >= band->width) || (_y < 0 || _y >= band->height))
                    {
                        ...
                        RASTER_DEBUGF(
                            4, "NODATA pixel outside band extent: (x, y, val) = (%d, %d, %f)", _x, _y, pixval);
                        inextent = 0;
                        isnodata = 1;
                    }
                    else
                    ...
                    if (!exclude_nodata_value || (exclude_nodata_value && !isnodata))
                    {
                        ...
                        RASTER_DEBUGF(
                            4, "Adding pixel to set of nearest pixels: (x, y, val) = (%d, %d, %f)", _x, _y, pixval);
                        count++;
