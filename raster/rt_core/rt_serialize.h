...
char *d_binary_to_hex(const uint8_t *const raw, uint32_t size, uint32_t *hexsize);

void d_print_binary_hex(const char *msg, const uint8_t *const raw, uint32_t size);
 ...
 *
 */
void setBits(char *ch, double val, int bits, int bitOffset);
