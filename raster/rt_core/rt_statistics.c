                          ...
                          double *cQ)
{
    ...
    */
    ...
    RASTER_DEBUGF(
        3, "sampling %d of %d available pixels w/ %d per set", sample_size, (band->width * band->height), sample_per);
...
rt_band_get_quantiles(rt_bandstats stats, double *quantiles, int quantiles_count, uint32_t *rtn_count)
{
    ...
    */
    for (i = 0; i < quantiles_count; i++)
    {
        ...
        if (h > hl) rtn[i].value = stats->values[hl - 1] + ((h - hl) * (stats->values[hl] - stats->values[hl - 1]));
                ...
                for (a = 0; a < *qlls_count; a++)
                {
                    ...
                    else if (qll->algeq)
                    {
                        ...
                        if (value < qll->head->value)
                        {
                            ...
                            if (qll->sum1 >= qll->tau) { RASTER_DEBUGF(4, "skipping %f", value); }
                            else
                            {
    ...
    *rtn_count = *qlls_count / 2;
    ...
    for (i = 0, k = 0; i < *qlls_count; i++)
    {
        ...
        if (exists)
        {
            if ((*qlls)[j].algeq)
            {
                rtn[k].value = ((qle->value * qle->count) + ((*qlls)[j].head->value * (*qlls)[j].head->count)) /
                               (qle->count + (*qlls)[j].head->count);
                RASTER_DEBUGF(
                    5, "qlls[%d].head: (value, count) = (%f, %d)", j, (*qlls)[j].head->value, (*qlls)[j].head->count);
            }
            else
            {
                rtn[k].value = ((qle->value * qle->count) + ((*qlls)[j].tail->value * (*qlls)[j].tail->count)) /
                               (qle->count + (*qlls)[j].tail->count);
                RASTER_DEBUGF(
                    5, "qlls[%d].tail: (value, count) = (%f, %d)", j, (*qlls)[j].tail->value, (*qlls)[j].tail->count);
            }
        }
