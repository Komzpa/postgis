        ...
        for (rowoffset = 0; rowoffset < 3; rowoffset++)
        {
            ...
            for (col = coloffset; col <= width1; col += 3)
            {
                ...
                for (row = rowoffset; row <= dimValue; row += 3)
                {
                    ...
                    RASTER_DEBUGF(
                        4, "line1(x1, y1, x2, y2) = (%f, %f, %f, %f)", line1[X1], line1[Y1], line1[X2], line1[Y2]);
                    RASTER_DEBUGF(
                        4, "line2(x1, y1, x2, y2) = (%f, %f, %f, %f)", line2[X1], line2[Y1], line2[X2], line2[Y2]);

                    /* intersection */
                    /* http://en.wikipedia.org/wiki/Line-line_intersection */
                    d = ((line1[X1] - line1[X2]) * (line2[Y1] - line2[Y2])) -
                         ...
                         (P[pY] > fmin(line2[Y1], line2[Y2]) && (P[pY] < fmax(line2[Y1], line2[Y2])))))
                    {
                        ...
                        for (i = 0; i < 8; i++)
                        {
                            ...
                            noval1 = 0;
                            if (rt_raster_geopoint_to_cell(rast1, Qw[pX], Qw[pY], &(Qr[pX]), &(Qr[pY]), igt1) !=
                                ES_NONE)
                            { noval1 = 1; }
                            ...
                            /* unable to convert point to cell */
                            noval2 = 0;
                            if (rt_raster_geopoint_to_cell(rast2, Qw[pX], Qw[pY], &(Qr[pX]), &(Qr[pY]), igt2) !=
                                ES_NONE)
                            { noval2 = 1; }
                            /* cell is outside bounds of grid */
                        ...
                        {
                            RASTER_DEBUGF(
                                4, "adjacent[%d] = %d, adjacent[%d] = %d", i, adjacent[i], i + 4, adjacent[i + 4]);
                ...
                for (col = coloffset; col < *widthS; col += 3)
                {
                    for (row = rowoffset; row < *heightS; row += 3)
                    {
                        ...
                        if ((hasnodataS == FALSE) || !isnodataS)
                        {
                            ...
                            if (rt_raster_geopoint_to_cell(rastL, lineS[X1], lineS[Y1], &(Qr[pX]), &(Qr[pY]), igtL) !=
                                ES_NONE)
                            { continue; }
