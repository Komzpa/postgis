        ...
        for (ytile = 0; ytile < ntiles[1]; ytile++)
        {
            ...
            for (xtile = 0; xtile < ntiles[0]; xtile++)
            {
                ...
                GDALApplyGeoTransform(
                    info->gt, xtile * info->tile_size[0], ytile * info->tile_size[1], &(gt[0]), &(gt[3]));
    ...
    else if (config->rt_file_count > 1)
    {
        ...
        if (drv == NULL)
        {
            ...
            if (ptr)
            {
                ...
                config->table = rtalloc(
                    sizeof(char) * (strlen(config->rt_file[config->rt_file_count - 1]) - strlen(config->schema) + 1));
