...
CreateStdHash(void)
{
  ...
  StdHash = hash_create(
      "PAGC Address Standardizer Backend MemoryContext Hash", STD_BACKEND_HASH_SIZE, &ctl, (HASH_ELEM | HASH_FUNCTION));
...
IsInStdPortalCache(StdPortalCache *STDCache, char *lextab, char *gaztab, char *rultab)
{
  ...
  for (i = 0; i < STD_CACHE_ITEMS; i++)
  {
    ...
    if (ci->lextab && !strcmp(ci->lextab, lextab) && ci->lextab && !strcmp(ci->gaztab, gaztab) && ci->lextab &&
        !strcmp(ci->rultab, rultab))
      return TRUE;
  }
...
#else
  STDMemoryContext = AllocSetContextCreate(STDCache->StdCacheContext, "PAGC STD Memory Context", ALLOCSET_SMALL_SIZES);
  ...
  while (moredata == TRUE)
  {
    ...
    if (ntuples > 0)
    {
      ...
      for (t = 0; t < ntuples; t++)
      {
        ...
        if (ret != 0)
        {
          elog(NOTICE, "load_roles: failed to add rule %d (%d): %s", total_tuples + t + 1, ret, rule);
          return -1;
        }
      }
      // DBG("calling SPI_freetuptable");
