...
main(int argc, char *argv[])
{
  ...
  while (!feof(in) && fgets(buf, 1024, in))
  {
    ...
    if (err != 0) printf("rules: Failed: %d (%d): %s", cnt, err, buf);
  }
  err = rules_ready(rules);
  if (err != 0) printf("rules: Failed: err=%d\n", err);
  fclose(in);
...
standardize_command_line(STANDARDIZER *std, char *input_str, int option)
{
  ...
  result = std_standardize_mm(std, unstandard_mic, unstandard_mac_left, (option & 4) ? 1 : 0);

  print_stdaddr(result);

  if (option & 8) output_rule_statistics(std->pagc_p->rules, std->err_p);
