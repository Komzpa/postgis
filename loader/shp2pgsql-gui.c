    ...
    for (i = 0; i < PQntuples(result); i++)
    {
        ...
        if (PQntuples(geocol_result) > 0)
        {
            ...
            for (j = 0; j < PQntuples(geocol_result); j++)
            {
                ...
                gtk_list_store_set(
                    dumper_geocol_combo_list, &geocol_iter, TABLECHOOSER_GEOCOL_COMBO_TEXT, geocol_name, -1);
            }
        }
        else
        {
            ...
            gtk_list_store_insert_before(
                dumper_geocol_combo_list, &geocol_iter, (GtkTreeIter *)TABLECHOOSER_GEOCOL_COMBO_TEXT);
            gtk_list_store_set(dumper_geocol_combo_list, &geocol_iter, TABLECHOOSER_GEOCOL_COMBO_TEXT, _("(None)"), -1);
        }
                                ...
                                GtkTreeIter *chooser_iter)
{
    ...
    if (chooser_liststore)
    {
        ...
        gtk_tree_model_get(
            GTK_TREE_MODEL(chooser_liststore), chooser_iter, TABLECHOOSER_GEO_LISTSTORE_COLUMN, &geocol_liststore, -1);

        gtk_list_store_set(export_table_list_store, &iter, EXPORT_GEOMETRY_LISTSTORE_COLUMN, geocol_liststore, -1);
    }
...
pgui_action_import(GtkWidget *widget, gpointer data)
{
    ...
    while (is_valid)
    {
        ...
        for (i = strlen(loader_file_config->shp_file);
             i >= 0 && loader_file_config->shp_file[i - 1] != '\\' && loader_file_config->shp_file[i - 1] != '/';
             i--)
            ;
...
pgui_create_tablechooser_dialog()
{
    ...
    g_signal_connect(
        G_OBJECT(checkbutton_chooser_geoonly), "toggled", G_CALLBACK(pgui_action_chooser_toggle_show_geocolumn), NULL);
...
pgui_create_import_file_table(GtkWidget *import_list_frame)
{
    ...
    import_table_column =
        gtk_tree_view_column_new_with_attributes(_("Table"), import_table_renderer, "text", IMPORT_TABLE_COLUMN, NULL);
    ...
    gtk_list_store_set(
        loader_mode_combo_list, &iter, LOADER_MODE_COMBO_TEXT, _("Prepare"), LOADER_MODE_COMBO_OPTION_CHAR, 'p', -1);
...
pgui_create_export_table_table(GtkWidget *export_list_frame)
{
    ...
    export_table_column =
        gtk_tree_view_column_new_with_attributes(_("Table"), export_table_renderer, "text", EXPORT_TABLE_COLUMN, NULL);
