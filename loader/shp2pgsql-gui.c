  ...
  for (i = 0; i < PQntuples(result); i++)
  {
    ...
    if (PQntuples(geocol_result) > 0)
    {
      ...
      for (j = 0; j < PQntuples(geocol_result); j++)
      {
        ...
        gtk_list_store_set(dumper_geocol_combo_list, &geocol_iter, TABLECHOOSER_GEOCOL_COMBO_TEXT, geocol_name, -1);
      }
    }
    else
    {
...
validate_shape_column_against_pg_column(int dbf_fieldtype, char *pg_fieldtype)
{
  ...
  case FTInteger:
    ...
    if (!strcmp(pg_fieldtype, "int2") || !strcmp(pg_fieldtype, "int4") || !strcmp(pg_fieldtype, "numeric")) return -1;
    break;
...
validate_remote_loader_columns(SHPLOADERCONFIG *config, PGresult *result)
{
  ...
  if (status == PGRES_TUPLES_OK)
  {
    ...
    case 'a':
      ...
      {
        pgui_seterr(
            _("ERROR: Destination table %s.%s could not be found for appending"), config->schema, config->table);
        response = SHPLOADERERR;
      }
      else
      {
  ...
  while (is_valid)
  {
    ...
    if (state->config->opt != 'p')
    {
      ...
      for (i = 0; i < numrecords && is_running; i++)
      {
        ...
        if (i % records_per_tick == 0) gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progress), (float)i / numrecords);
...
pgui_create_import_file_table(GtkWidget *import_list_frame)
{
  ...
  import_schema_column =
      gtk_tree_view_column_new_with_attributes(_("Schema"), import_schema_renderer, "text", IMPORT_SCHEMA_COLUMN, NULL);
...
pgui_create_export_table_table(GtkWidget *export_list_frame)
{
  ...
  export_schema_column =
      gtk_tree_view_column_new_with_attributes(_("Schema"), export_schema_renderer, "text", EXPORT_SCHEMA_COLUMN, NULL);
